...
**Full maintainer PR workflow (optional):** If you want the repo's end-to-end maintainer workflow (triage order, quality bar, rebase rules, commit/changelog conventions, co-contributor policy, and the `review-pr` > `prepare-pr` > `validate-pr` > `merge-pr` pipeline), see `.agents/skills/PR_WORKFLOW.md`. For fully automated, agent-driven workflows, use `run-pr` which chains all four stages with automatic handoff. Maintainers may use other workflows; when a maintainer specifies a workflow, follow that. If no workflow is specified, default to PR_WORKFLOW.
