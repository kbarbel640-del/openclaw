...
- Pre-commit hooks: auto-installed via `pnpm install` (runs oxlint/oxfmt on staged files). For full pre-commit framework (CI-level checks), use `pip install pre-commit && pre-commit install`.
