...
- Doctor/Telegram upgrades: warn when Telegram group auth is fail-closed (`groupPolicy="allowlist"`) but no sender allowlist is configured, with explicit migration guidance for `groupAllowFrom` / per-group `allowFrom`.
