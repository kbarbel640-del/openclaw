...
- **macOS**: `gateway stop` no longer unloads the LaunchAgent from launchd. It now sends `SIGTERM` while keeping the service registered, so `gateway start` can recover without requiring `gateway install` or `doctor --fix`. Fixes #25199.
