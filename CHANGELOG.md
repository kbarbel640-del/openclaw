...
- Slack/Config: align `replyToMode` runtime default to `"off"` to match docs. When `channels.slack.replyToMode` is omitted from config, the code was defaulting to `"all"` (always thread replies) while the documentation states the default is `"off"`. Thanks @arosstale.
