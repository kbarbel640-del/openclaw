...
- LLM/Task: fall back to `dist/` path for `llm-task` runner import when the source path is unavailable in production builds, fixing `MODULE_NOT_FOUND` on npm-installed gateways. (#18946) Thanks @arosstale.
