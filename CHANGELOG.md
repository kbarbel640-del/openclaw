...
- Slack: bot messages with empty `text` but content in `attachments` now trigger sessions when `allowBots: true` is set, so monitoring webhooks (Prometheus, Gatus, etc.) are no longer silently dropped. (#27616)
