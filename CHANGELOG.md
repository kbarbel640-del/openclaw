...
- Plugins/Extensions: auto-install missing dependencies for bundled extensions on enable, so extensions that ship without `node_modules/` no longer fail with `Cannot find module` on first load. (#26762)
