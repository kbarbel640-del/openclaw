...
- Matrix/Tool action routing: fix `sendMessage` tool action not forwarding `accountId` to the Matrix client resolver; isolated cron and subagent sessions were using the first active account by Map insertion order (non-deterministic across restarts in multi-account setups) instead of the agent's bound account, silently routing messages from the wrong Matrix identity and triggering `M_FORBIDDEN` errors in rooms only the correct account can access. (#26457)
