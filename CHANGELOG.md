...
- Agents/ImageTool: resolve image paths relative to media/inbound (sandbox-accessible path) before the absolute filesystem path, fixing image-tool failures when the sandbox media store uses a non-default location.
