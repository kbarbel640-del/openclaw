...
- Slack/Threading: make `replyToMode: "off"` consistently disable unwanted thread replies by removing status-thread fallback thread creation, honoring DM `off` mode when typing indicators provide thread context, stripping explicit reply tags in `off` mode, and aligning runtime defaults with docs. (#22944) Thanks @unboxed-ai and @theoseo.
