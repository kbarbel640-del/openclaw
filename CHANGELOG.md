...
- CLI/Gateway service token drift: stop embedding `OPENCLAW_GATEWAY_TOKEN` in generated daemon service environments, migrate legacy embedded-token units through service audits/doctor repair, make gateway-service local credential resolution prefer `gateway.auth.token` when running under `OPENCLAW_SERVICE_KIND=gateway`, and make local restart drift checks/TUI/status auth prefer `gateway.auth.token` over stale shell/.env token overrides to prevent post-rotation mismatch failures. (#17223)
