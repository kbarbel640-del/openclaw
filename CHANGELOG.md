...
- Cron/Telegram: when announce delivery targets a threaded destination (for example forum topics), bypass subagent announce flow and deliver directly so cron output is not dropped by intermediary `ANNOUNCE_SKIP`/`NO_REPLY` turns. (#23841) Thanks @AndrewArto.
