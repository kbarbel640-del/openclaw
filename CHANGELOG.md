...
- macOS/LaunchAgent: replace unconditional `KeepAlive=true` with `SuccessfulExit=false` dict and add `ThrottleInterval=30` to prevent tight restart loops when the gateway crashes at startup. (#26507)
