...
**OpenClaw**는 당신의 개인 기기에서 실행되는 _개인용 AI 어시스턴트_입니다.
이미 사용하고 있는 채널들(WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, Microsoft Teams, WebChat)과 BlueBubbles, Matrix, Zalo, Zalo Personal 같은 확장 채널에서 답변을 제공합니다. macOS/iOS/Android에서 음성 대화가 가능하며, 제어할 수 있는 라이브 캔버스를 렌더링할 수 있습니다. 게이트웨이는 단순히 제어 플레인이며, 제품의 핵심은 어시스턴트입니다.

로컬에서 빠르고 항상 작동하는 개인용, 단일 사용자 어시스턴트를 원한다면 바로 이것입니다.
...
권장 설정: 온보딩 마법사(`openclaw onboard`)를 실행하세요. 게이트웨이, 워크스페이스, 채널, 스킬 설정을 안내해줍니다. CLI 마법사가 권장 경로이며 **macOS, Linux, Windows (WSL2 강력 권장)**에서 작동합니다.
npm, pnpm, bun과 호환됩니다.
...
모델 참고사항: 모든 모델이 지원되지만, 긴 컨텍스트의 강력함과 더 나은 프롬프트 인젝션 저항성을 위해 **Anthropic Pro/Max (100/200) + Opus 4.5**를 강력히 권장합니다. [온보딩](https://docs.openclaw.ai/start/onboarding)을 참조하세요.

## 모델 (선택 + 인증)

- 모델 설정 + CLI: [모델](https://docs.openclaw.ai/concepts/models)
- 인증 프로필 회전 (OAuth vs API 키) + 폴백: [모델 장애조치](https://docs.openclaw.ai/concepts/model-failover)
...
# 어시스턴트와 대화 (연결된 모든 채널로 응답 전달 가능: WhatsApp/Telegram/Slack/Discord/Google Chat/Signal/iMessage/BlueBubbles/Microsoft Teams/Matrix/Zalo/Zalo Personal/WebChat)
...
- **beta**: 프리릴리스 태그 (`vYYYY.M.D-beta.N`), npm dist-tag `beta` (macOS 앱 누락 가능).
- **dev**: `main`의 이동하는 헤드, npm dist-tag `dev` (게시될 때).
...
소스에서 빌드할 때는 `pnpm`을 권장합니다. TypeScript를 직접 실행하는 경우 Bun은 선택사항입니다.
...
OpenClaw는 실제 메시징 서피스에 연결됩니다. 인바운드 DM을 **신뢰할 수 없는 입력**으로 취급하세요.
...
- **DM 페어링** (`dmPolicy="pairing"` / `channels.discord.dm.policy="pairing"` / `channels.slack.dm.policy="pairing"`): 알 수 없는 발신자는 짧은 페어링 코드를 받고 봇은 메시지를 처리하지 않습니다.
- 승인: `openclaw pairing approve <channel> <code>` (그러면 발신자가 로컬 허용목록 저장소에 추가됩니다).
- 공개 인바운드 DM은 명시적 옵트인이 필요합니다: `dmPolicy="open"`으로 설정하고 채널 허용목록에 `"*"`를 포함하세요 (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`).

위험하거나 잘못 구성된 DM 정책을 표시하려면 `openclaw doctor`를 실행하세요.

## 하이라이트

- **[로컬 우선 게이트웨이](https://docs.openclaw.ai/gateway)** — 세션, 채널, 도구, 이벤트를 위한 단일 제어 플레인.
- **[멀티 채널 인박스](https://docs.openclaw.ai/channels)** — WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, BlueBubbles, Microsoft Teams, Matrix, Zalo, Zalo Personal, WebChat, macOS, iOS/Android.
- **[멀티 에이전트 라우팅](https://docs.openclaw.ai/gateway/configuration)** — 인바운드 채널/계정/피어를 격리된 에이전트로 라우팅 (워크스페이스 + 에이전트별 세션).
- **[음성 깨우기](https://docs.openclaw.ai/nodes/voicewake) + [토크 모드](https://docs.openclaw.ai/nodes/talk)** — ElevenLabs와 함께 macOS/iOS/Android용 상시 음성 기능.
- **[라이브 캔버스](https://docs.openclaw.ai/platforms/mac/canvas)** — [A2UI](https://docs.openclaw.ai/platforms/mac/canvas#canvas-a2ui)가 있는 에이전트 기반 시각적 워크스페이스.
- **[일급 도구](https://docs.openclaw.ai/tools)** — 브라우저, 캔버스, 노드, cron, 세션, Discord/Slack 액션.
- **[동반 앱](https://docs.openclaw.ai/platforms/macos)** — macOS 메뉴 바 앱 + iOS/Android [노드](https://docs.openclaw.ai/nodes).
- **[온보딩](https://docs.openclaw.ai/start/wizard) + [스킬](https://docs.openclaw.ai/tools/skills)** — 번들/관리/워크스페이스 스킬이 있는 마법사 기반 설정.
...
- 세션, 존재감, 설정, cron, 웹훅, [제어 UI](https://docs.openclaw.ai/web), [캔버스 호스트](https://docs.openclaw.ai/platforms/mac/canvas#canvas-a2ui)가 포함된 [게이트웨이 WS 제어 플레인](https://docs.openclaw.ai/gateway).
- [CLI 서피스](https://docs.openclaw.ai/tools/agent-send): 게이트웨이, 에이전트, 전송, [마법사](https://docs.openclaw.ai/start/wizard), [닥터](https://docs.openclaw.ai/gateway/doctor).
- 도구 스트리밍과 블록 스트리밍이 있는 RPC 모드의 [Pi 에이전트 런타임](https://docs.openclaw.ai/concepts/agent).
- [세션 모델](https://docs.openclaw.ai/concepts/session): 직접 채팅을 위한 `main`, 그룹 격리, 활성화 모드, 큐 모드, 응답. 그룹 규칙: [그룹](https://docs.openclaw.ai/concepts/groups).
- [미디어 파이프라인](https://docs.openclaw.ai/nodes/images): 이미지/오디오/비디오, 전사 훅, 크기 제한, 임시 파일 라이프사이클. 오디오 세부사항: [오디오](https://docs.openclaw.ai/nodes/audio).
...
- [그룹 라우팅](https://docs.openclaw.ai/concepts/group-messages): 멘션 게이팅, 응답 태그, 채널별 청킹 및 라우팅. 채널 규칙: [채널](https://docs.openclaw.ai/channels).

### 앱 + 노드

- [macOS 앱](https://docs.openclaw.ai/platforms/macos): 메뉴 바 제어 플레인, [음성 깨우기](https://docs.openclaw.ai/nodes/voicewake)/PTT, [토크 모드](https://docs.openclaw.ai/nodes/talk) 오버레이, [WebChat](https://docs.openclaw.ai/web/webchat), 디버그 도구, [원격 게이트웨이](https://docs.openclaw.ai/gateway/remote) 제어.
- [iOS 노드](https://docs.openclaw.ai/platforms/ios): [캔버스](https://docs.openclaw.ai/platforms/mac/canvas), [음성 깨우기](https://docs.openclaw.ai/nodes/voicewake), [토크 모드](https://docs.openclaw.ai/nodes/talk), 카메라, 화면 녹화, Bonjour 페어링.
- [Android 노드](https://docs.openclaw.ai/platforms/android): [캔버스](https://docs.openclaw.ai/platforms/mac/canvas), [토크 모드](https://docs.openclaw.ai/nodes/talk), 카메라, 화면 녹화, 선택적 SMS.
- [macOS 노드 모드](https://docs.openclaw.ai/nodes): system.run/notify + 캔버스/카메라 노출.
...
- [스킬 플랫폼](https://docs.openclaw.ai/tools/skills): 설치 게이팅 + UI가 있는 번들, 관리, 워크스페이스 스킬.
...
- [존재감](https://docs.openclaw.ai/concepts/presence), [타이핑 표시기](https://docs.openclaw.ai/concepts/typing-indicators), [사용 추적](https://docs.openclaw.ai/concepts/usage-tracking).
- [모델](https://docs.openclaw.ai/concepts/models), [모델 장애조치](https://docs.openclaw.ai/concepts/model-failover), [세션 정리](https://docs.openclaw.ai/concepts/session-pruning).
- [보안](https://docs.openclaw.ai/gateway/security) 및 [문제해결](https://docs.openclaw.ai/channels/troubleshooting).

### 운영 + 패키징

- 게이트웨이에서 직접 제공되는 [제어 UI](https://docs.openclaw.ai/web) + [WebChat](https://docs.openclaw.ai/web/webchat).
- 토큰/패스워드 인증이 있는 [Tailscale Serve/Funnel](https://docs.openclaw.ai/gateway/tailscale) 또는 [SSH 터널](https://docs.openclaw.ai/gateway/remote).
- 선언적 설정을 위한 [Nix 모드](https://docs.openclaw.ai/install/nix); [Docker](https://docs.openclaw.ai/install/docker) 기반 설치.
- [닥터](https://docs.openclaw.ai/gateway/doctor) 마이그레이션, [로깅](https://docs.openclaw.ai/logging).
...
- **[게이트웨이 WebSocket 네트워크](https://docs.openclaw.ai/concepts/architecture)** — 클라이언트, 도구, 이벤트를 위한 단일 WS 제어 플레인 (운영: [게이트웨이 런북](https://docs.openclaw.ai/gateway)).
...
- **[캔버스 + A2UI](https://docs.openclaw.ai/platforms/mac/canvas)** — 에이전트 기반 시각적 워크스페이스 (A2UI 호스트: [캔버스/A2UI](https://docs.openclaw.ai/platforms/mac/canvas#canvas-a2ui)).
- **[음성 깨우기](https://docs.openclaw.ai/nodes/voicewake) + [토크 모드](https://docs.openclaw.ai/nodes/talk)** — 상시 음성 및 연속 대화.
- **[노드](https://docs.openclaw.ai/nodes)** — 캔버스, 카메라 스냅/클립, 화면 녹화, `location.get`, 알림, macOS 전용 `system.run`/`system.notify`.

## Tailscale 접근 (게이트웨이 대시보드)

OpenClaw는 게이트웨이가 루프백에 바인딩된 상태로 Tailscale **Serve** (tailnet 전용) 또는 **Funnel** (공개)을 자동 구성할 수 있습니다. `gateway.tailscale.mode`를 설정하세요:

- `off`: Tailscale 자동화 없음 (기본값).
- `serve`: `tailscale serve`를 통한 tailnet 전용 HTTPS (기본적으로 Tailscale 아이덴티티 헤더 사용).
- `funnel`: `tailscale funnel`을 통한 공개 HTTPS (공유 패스워드 인증 필요).

참고사항:

- Serve/Funnel이 활성화된 경우 `gateway.bind`는 `loopback`을 유지해야 합니다 (OpenClaw가 이를 강제합니다).
- `gateway.auth.mode: "password"` 또는 `gateway.auth.allowTailscale: false`를 설정하여 Serve가 패스워드를 요구하도록 강제할 수 있습니다.
- `gateway.auth.mode: "password"`가 설정되지 않으면 Funnel 시작을 거부합니다.
- 선택사항: `gateway.tailscale.resetOnExit`로 종료 시 Serve/Funnel 취소.

세부사항: [Tailscale 가이드](https://docs.openclaw.ai/gateway/tailscale) · [웹 서피스](https://docs.openclaw.ai/web)

## 원격 게이트웨이 (Linux가 훌륭합니다)

소규모 Linux 인스턴스에서 게이트웨이를 실행하는 것은 완전히 괜찮습니다. 클라이언트 (macOS 앱, CLI, WebChat)는 **Tailscale Serve/Funnel** 또는 **SSH 터널**을 통해 연결할 수 있으며, 필요할 때 기기별 액션을 실행하기 위해 기기 노드 (macOS/iOS/Android)를 페어링할 수 있습니다.

- **게이트웨이 호스트**는 기본적으로 exec 도구와 채널 연결을 실행합니다.
- **기기 노드**는 `node.invoke`를 통해 기기별 액션 (`system.run`, 카메라, 화면 녹화, 알림)을 실행합니다.
  간단히 말해: exec는 게이트웨이가 있는 곳에서 실행되고, 기기 액션은 기기가 있는 곳에서 실행됩니다.

세부사항: [원격 접근](https://docs.openclaw.ai/gateway/remote) · [노드](https://docs.openclaw.ai/nodes) · [보안](https://docs.openclaw.ai/gateway/security)

## 게이트웨이 프로토콜을 통한 macOS 권한

macOS 앱은 **노드 모드**로 실행될 수 있으며 게이트웨이 WebSocket을 통해 기능 + 권한 맵을 광고합니다 (`node.list` / `node.describe`). 그런 다음 클라이언트는 `node.invoke`를 통해 로컬 액션을 실행할 수 있습니다:

- `system.run`은 로컬 명령을 실행하고 stdout/stderr/종료 코드를 반환합니다; 화면 녹화 권한이 필요한 경우 `needsScreenRecording: true`를 설정하세요 (그렇지 않으면 `PERMISSION_MISSING`을 받게 됩니다).
- `system.notify`는 사용자 알림을 게시하고 알림이 거부되면 실패합니다.
- `canvas.*`, `camera.*`, `screen.record`, `location.get`도 `node.invoke`를 통해 라우팅되며 TCC 권한 상태를 따릅니다.

승격된 bash (호스트 권한)는 macOS TCC와 별개입니다:

- 활성화되고 허용목록에 있을 때 `/elevated on|off`를 사용하여 세션별 승격 접근을 토글하세요.
- 게이트웨이는 `thinkingLevel`, `verboseLevel`, `model`, `sendPolicy`, `groupActivation`과 함께 `sessions.patch` (WS 메서드)를 통해 세션별 토글을 지속합니다.

세부사항: [노드](https://docs.openclaw.ai/nodes) · [macOS 앱](https://docs.openclaw.ai/platforms/macos) · [게이트웨이 프로토콜](https://docs.openclaw.ai/concepts/architecture)

## 에이전트 간 통신 (sessions\_\* 도구)

- 채팅 서피스 간을 오가지 않고 세션 간 작업을 조정하는 데 사용합니다.
- `sessions_list` — 활성 세션 (에이전트)와 메타데이터를 찾습니다.
- `sessions_history` — 세션의 대화 로그를 가져옵니다.
- `sessions_send` — 다른 세션에 메시지; 선택적 응답 핑퐁 + 공지 단계 (`REPLY_SKIP`, `ANNOUNCE_SKIP`).

세부사항: [세션 도구](https://docs.openclaw.ai/concepts/session-tool)

## 스킬 레지스트리 (ClawHub)

ClawHub는 최소한의 스킬 레지스트리입니다. ClawHub가 활성화되면 에이전트는 자동으로 스킬을 검색하고 필요에 따라 새로운 것을 가져올 수 있습니다.

[ClawHub](https://clawhub.com)

## 채팅 명령어

WhatsApp/Telegram/Slack/Google Chat/Microsoft Teams/WebChat에서 이것들을 보내세요 (그룹 명령어는 소유자 전용):

- `/status` — 간편 세션 상태 (모델 + 토큰, 사용 가능한 경우 비용)
- `/new` 또는 `/reset` — 세션 재설정
- `/compact` — 세션 컨텍스트 압축 (요약)
- `/think <level>` — off|minimal|low|medium|high|xhigh (GPT-5.2 + Codex 모델만)
- `/verbose on|off`
- `/usage off|tokens|full` — 응답별 사용량 푸터
- `/restart` — 게이트웨이 재시작 (그룹에서 소유자 전용)
- `/activation mention|always` — 그룹 활성화 토글 (그룹만)

## 앱 (선택사항)

게이트웨이만으로도 훌륭한 경험을 제공합니다. 모든 앱은 선택사항이며 추가 기능을 제공합니다.

동반 앱을 빌드/실행할 계획이라면 아래 플랫폼 런북을 따르세요.

### macOS (OpenClaw.app) (선택사항)
...
참고: 리빌드 간 macOS 권한 유지를 위해 서명된 빌드가 필요합니다 (`docs/mac/permissions.md` 참조).

### iOS 노드 (선택사항)

- Bridge를 통해 노드로 페어링됩니다.
- 음성 트리거 전달 + 캔버스 서피스.
- `openclaw nodes …`를 통해 제어됩니다.

런북: [iOS 연결](https://docs.openclaw.ai/platforms/ios).

### Android 노드 (선택사항)

- iOS와 같은 Bridge + 페어링 플로우를 통해 페어링됩니다.
- 캔버스, 카메라, 화면 캡처 명령을 노출합니다.
- 런북: [Android 연결](https://docs.openclaw.ai/platforms/android).

## 에이전트 워크스페이스 + 스킬

- 워크스페이스 루트: `~/.openclaw/workspace` (`agents.defaults.workspace`를 통해 설정 가능).
...
[전체 설정 참조 (모든 키 + 예제).](https://docs.openclaw.ai/gateway/configuration)

## 보안 모델 (중요)

- **기본값:** 도구는 **main** 세션을 위해 호스트에서 실행되므로 에이전트는 당신만 있을 때 전체 접근 권한을 갖습니다.
- **그룹/채널 안전성:** `agents.defaults.sandbox.mode: "non-main"`을 설정하여 **비메인 세션** (그룹/채널)을 세션별 Docker 샌드박스 내에서 실행하세요; 그러면 해당 세션에서 bash가 Docker에서 실행됩니다.
- **샌드박스 기본값:** `bash`, `process`, `read`, `write`, `edit`, `sessions_list`, `sessions_history`, `sessions_send`, `sessions_spawn` 허용목록; `browser`, `canvas`, `nodes`, `cron`, `discord`, `gateway` 거부목록.

세부사항: [보안 가이드](https://docs.openclaw.ai/gateway/security) · [Docker + 샌드박싱](https://docs.openclaw.ai/install/docker) · [샌드박스 설정](https://docs.openclaw.ai/gateway/configuration)

### [WhatsApp](https://docs.openclaw.ai/channels/whatsapp)

- 기기 연결: `pnpm openclaw channels login` (`~/.openclaw/credentials`에 자격증명 저장).
- `channels.whatsapp.allowFrom`을 통해 어시스턴트와 대화할 수 있는 사람을 허용목록에 추가하세요.
- `channels.whatsapp.groups`가 설정되면 그룹 허용목록이 됩니다; 모든 것을 허용하려면 `"*"`를 포함하세요.

### [Telegram](https://docs.openclaw.ai/channels/telegram)

- `TELEGRAM_BOT_TOKEN` 또는 `channels.telegram.botToken` 설정 (env가 우선).
- 선택사항: `channels.telegram.groups` 설정 (`channels.telegram.groups."*".requireMention`와 함께); 설정되면 그룹 허용목록입니다 (모든 것을 허용하려면 `"*"` 포함). 또한 필요에 따라 `channels.telegram.allowFrom` 또는 `channels.telegram.webhookUrl` + `channels.telegram.webhookSecret`.
...
- `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN` 설정 (또는 `channels.slack.botToken` + `channels.slack.appToken`).

### [Discord](https://docs.openclaw.ai/channels/discord)

- `DISCORD_BOT_TOKEN` 또는 `channels.discord.token` 설정 (env가 우선).
- 선택사항: 필요에 따라 `commands.native`, `commands.text`, 또는 `commands.useAccessGroups`, `channels.discord.dm.allowFrom`, `channels.discord.guilds`, 또는 `channels.discord.mediaMaxMb` 설정.
...
- macOS 전용; Messages에 로그인되어 있어야 합니다.
- `channels.imessage.groups`가 설정되면 그룹 허용목록이 됩니다; 모든 것을 허용하려면 `"*"`를 포함하세요.

### [Microsoft Teams](https://docs.openclaw.ai/channels/msteams)

- Teams 앱 + Bot Framework를 구성한 다음 `msteams` 설정 섹션을 추가하세요.
- `msteams.allowFrom`을 통해 대화할 수 있는 사람을 허용목록에 추가하세요; `msteams.groupAllowFrom` 또는 `msteams.groupPolicy: "open"`을 통한 그룹 접근.

### [WebChat](https://docs.openclaw.ai/web/webchat)

- 게이트웨이 WebSocket 사용; 별도의 WebChat 포트/설정 없음.
...
온보딩 플로우를 지나 더 깊은 참조를 원할 때 사용하세요.

- [탐색과 "무엇이 어디에 있는지"를 위한 문서 인덱스로 시작하세요.](https://docs.openclaw.ai)
- [게이트웨이 + 프로토콜 모델의 아키텍처 개요를 읽어보세요.](https://docs.openclaw.ai/concepts/architecture)
- [모든 키와 예제가 필요할 때 전체 설정 참조를 사용하세요.](https://docs.openclaw.ai/gateway/configuration)
- [운영 런북으로 게이트웨이를 책대로 실행하세요.](https://docs.openclaw.ai/gateway)
- [제어 UI/웹 서피스가 작동하는 방식과 안전하게 노출하는 방법을 배우세요.](https://docs.openclaw.ai/web)
- [SSH 터널 또는 tailnet을 통한 원격 접근을 이해하세요.](https://docs.openclaw.ai/gateway/remote)
- [가이드된 설정을 위한 온보딩 마법사 플로우를 따르세요.](https://docs.openclaw.ai/start/wizard)
- [웹훅 서피스를 통해 외부 트리거를 연결하세요.](https://docs.openclaw.ai/automation/webhook)
- [Gmail Pub/Sub 트리거를 설정하세요.](https://docs.openclaw.ai/automation/gmail-pubsub)
- [macOS 메뉴 바 동반 세부사항을 배우세요.](https://docs.openclaw.ai/platforms/mac/menu-bar)
...
- [문제해결 가이드로 일반적인 실패를 디버그하세요.](https://docs.openclaw.ai/channels/troubleshooting)
...
- [macOS 음성 깨우기](https://docs.openclaw.ai/platforms/mac/voicewake)
...
OpenClaw는 우주 랍스터 AI 어시스턴트 **Molty**를 위해 구축되었습니다. 🦞
Peter Steinberger와 커뮤니티가 제작했습니다.
...
AI/vibe 코딩된 PR 환영합니다! 🤖

[pi-mono](https://github.com/badlogic/pi-mono)에 대한 지원과 기여를 해주신 [Mario Zechner](https://mariozechner.at/)님께 특별한 감사를 드립니다.
lobster.bot에 대해 Adam Doppelt님께 특별한 감사를 드립니다.

모든 clawtributor들에게 감사드립니다:
