...
This document describes how the Amigo ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`amigo acp` exposes an ACP agent over stdio and forwards prompts to a running
Amigo Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Amigo Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `amigo acp` over stdio.

Example config:

```bash
amigo config set gateway.remote.url wss://gateway-host:18789
amigo config set gateway.remote.token <token>
```

Example run:

```bash
amigo acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
amigo acp --session agent:main:main
amigo acp --session agent:design:main
amigo acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Amigo ACP” to start a thread.

## Execution Model

- ACP client spawns `amigo acp` and speaks ACP messages over stdio.
...
```bash
amigo acp --session agent:main:main
amigo acp --session-label "support inbox"
amigo acp --reset-session
```
...
`amigo acp` resolves the Gateway URL and auth from CLI flags or config:
