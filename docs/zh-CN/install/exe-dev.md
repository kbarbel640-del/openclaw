...
summary: 在 exe.dev 上运行 Amigo Gateway 网关（VM + HTTPS 代理）以实现远程访问
title: exe.dev
x-i18n:
...
目标：Amigo Gateway 网关运行在 exe.dev VM 上，可从你的笔记本电脑通过以下地址访问：`https://<vm-name>.exe.xyz`
...
Shelley，[exe.dev](https://exe.dev) 的智能体，可以使用我们的提示立即安装 Amigo。使用的提示如下：
...
提示：保持此 VM **有状态**。Amigo 在 `~/.amigo/` 和 `~/.amigo/workspace/` 下存储状态。
...
访问 `https://<vm-name>.exe.xyz/?token=YOUR-TOKEN-FROM-TERMINAL`（参阅新手引导中的控制 UI 输出）。使用 `amigo devices list` 和 `amigo devices approve <requestId>` 批准设备。如有疑问，从浏览器使用 Shelley！
