...
Amigo 有两个表面上看起来相似的不同问题：

1. **操作员远程控制**：macOS 菜单栏应用控制运行在其他地方的 Gateway 网关。
2. **节点配对**：iOS/Android（以及未来的节点）发现 Gateway 网关并安全配对。

设计目标是将所有网络发现/广播保留在 **Node Gateway 网关**（`amigo gateway`）中，并让客户端（mac 应用、iOS）作为消费者。
...
- TXT 键（非机密）：
  ...
  - `canvasPort=18793`（默认画布主机端口；服务于 `/__amigo__/canvas/`）
  - `cliPath=<path>`（可选；可运行的 `amigo` 入口点或二进制文件的绝对路径）
