...
Amigo 的 Gateway 网关可以提供兼容 OpenResponses 的 `POST /v1/responses` 端点。
...
底层实现中，请求作为正常的 Gateway 网关智能体运行执行（与 `amigo agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway 网关一致。
...
- `model: "amigo:<agentId>"`（示例：`"amigo:main"`、`"amigo:beta"`）
- `model: "agent:<agentId>"`（别名）

或通过头指定特定的 Amigo 智能体：

- `x-amigo-agent-id: <agentId>`（默认：`main`）

高级：

- `x-amigo-session-key: <sessionKey>` 完全控制会话路由。
