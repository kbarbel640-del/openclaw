...
summary: "`amigo models` 的 CLI 参考（status/list/set/scan、别名、回退、认证）"
title: models
x-i18n:
...
`amigo models status` 显示已解析的默认模型/回退配置以及认证概览。
...
- 如果省略提供商，Amigo 会将输入视为别名或**默认提供商**的模型（仅在模型 ID 不包含 `/` 时有效）。
