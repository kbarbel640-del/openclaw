...
你可以在 `~/.amigo/amigo.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/amigo.log"
  }
}
```
...
```bash
amigo channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.amigo/amigo.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/amigo/amigo-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Amigo 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `amigo plugins enable diagnostics-otel` 启用插件。
...
- `amigo.tokens`（计数器，属性：`amigo.token`、`amigo.channel`、`amigo.provider`、`amigo.model`）
- `amigo.cost.usd`（计数器，属性：`amigo.channel`、`amigo.provider`、`amigo.model`）
- `amigo.run.duration_ms`（直方图，属性：`amigo.channel`、`amigo.provider`、`amigo.model`）
- `amigo.context.tokens`（直方图，属性：`amigo.context`、`amigo.channel`、`amigo.provider`、`amigo.model`）

消息流：

- `amigo.webhook.received`（计数器，属性：`amigo.channel`、`amigo.webhook`）
- `amigo.webhook.error`（计数器，属性：`amigo.channel`、`amigo.webhook`）
- `amigo.webhook.duration_ms`（直方图，属性：`amigo.channel`、`amigo.webhook`）
- `amigo.message.queued`（计数器，属性：`amigo.channel`、`amigo.source`）
- `amigo.message.processed`（计数器，属性：`amigo.channel`、`amigo.outcome`）
- `amigo.message.duration_ms`（直方图，属性：`amigo.channel`、`amigo.outcome`）

队列 + 会话：

- `amigo.queue.lane.enqueue`（计数器，属性：`amigo.lane`）
- `amigo.queue.lane.dequeue`（计数器，属性：`amigo.lane`）
- `amigo.queue.depth`（直方图，属性：`amigo.lane` 或 `amigo.channel=heartbeat`）
- `amigo.queue.wait_ms`（直方图，属性：`amigo.lane`）
- `amigo.session.state`（计数器，属性：`amigo.state`、`amigo.reason`）
- `amigo.session.stuck`（计数器，属性：`amigo.state`）
- `amigo.session.stuck_age_ms`（直方图，属性：`amigo.state`）
- `amigo.run.attempt`（计数器，属性：`amigo.attempt`）

### 导出的 span（名称 + 关键属性）

- `amigo.model.usage`
  - `amigo.channel`、`amigo.provider`、`amigo.model`
  - `amigo.sessionKey`、`amigo.sessionId`
  - `amigo.tokens.*`（input/output/cache_read/cache_write/total）
- `amigo.webhook.processed`
  - `amigo.channel`、`amigo.webhook`、`amigo.chatId`
- `amigo.webhook.error`
  - `amigo.channel`、`amigo.webhook`、`amigo.chatId`、`amigo.error`
- `amigo.message.processed`
  - `amigo.channel`、`amigo.outcome`、`amigo.chatId`、`amigo.messageId`、`amigo.sessionKey`、`amigo.sessionId`、`amigo.reason`
- `amigo.session.stuck`
  - `amigo.state`、`amigo.ageMs`、`amigo.queueDepth`、`amigo.sessionKey`、`amigo.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `amigo doctor`。
