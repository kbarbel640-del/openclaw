...
如果您刚安装完 Amigo，可以直接运行向导，根据提示添加飞书：
...
- `amigo gateway status` - 查看网关运行状态
- `amigo logs --follow` - 查看实时日志
...
- `amigo gateway status` - 查看网关运行状态
- `amigo gateway restart` - 重启网关以应用新配置
- `amigo logs --follow` - 查看实时日志
...
1. 运行 `amigo channels add` 添加了 Feishu 渠道
2. 网关处于启动状态（可通过 `amigo gateway status` 检查状态）
...
```bash
amigo pairing approve feishu <配对码>
```
...
- **批准配对**：
  ```bash
  amigo pairing list feishu      # 查看待审批列表
  amigo pairing approve feishu <CODE>  # 批准
  ```
...
1. 启动网关并在群组中 @机器人发消息
2. 运行 `amigo logs --follow` 查看日志中的 `chat_id`
...
1. 启动网关并给机器人发消息
2. 运行 `amigo logs --follow` 查看日志中的 `open_id`
...
| -------------------------- | ----------------- |
| `amigo gateway status`  | 查看网关运行状态  |
| `amigo gateway install` | 安装/启动网关服务 |
| `amigo gateway stop`    | 停止网关服务      |
| `amigo gateway restart` | 重启网关服务      |
| `amigo logs --follow`   | 实时查看日志输出  |
...
3. 检查是否选择了 **长连接** 模式
4. 检查应用权限是否完整
5. 检查网关是否正在运行：`amigo gateway status`
6. 查看实时日志：`amigo logs --follow`
