...
| Group/channel | `conversation:<conversation-id>` | `conversation:19:[redacted-email]2` |
| Group/channel (raw) | `<conversation-id>` | `19:[redacted-email]2` (if contains `@thread`) |
...
clawdbot message send --channel msteams --target "conversation:19:[redacted-email]2" --message "Hello"

# Send an Adaptive Card to a conversation
clawdbot message send --channel msteams --target "conversation:19:[redacted-email]2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
...
**For config:**
- Team ID = path segment after `/team/` (URL-decoded, e.g., `19:[redacted-email]2`)
