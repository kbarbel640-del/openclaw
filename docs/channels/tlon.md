...
Tlon is a decentralized messenger built on Urbit. Amigo connects to your Urbit ship and can
...
```bash
amigo plugins install @amigo/tlon
```

Local checkout (when running from a git repo):

```bash
amigo plugins install ./extensions/tlon
```
...
Use these with `amigo message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Amigo replies in-thread.
