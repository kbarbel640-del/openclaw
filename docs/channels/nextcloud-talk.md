...
```bash
amigo plugins install @amigo/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
amigo plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Amigo" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `amigo pairing list nextcloud-talk`
  - `amigo pairing approve nextcloud-talk <CODE>`
