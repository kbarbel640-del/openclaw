...
Matrix is an open, decentralized messaging protocol. Amigo connects as a Matrix **user**
...
```bash
amigo plugins install @amigo/matrix
```

Local checkout (when running from a git repo):

```bash
amigo plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `amigo plugins install @amigo/matrix`
   - From a local checkout: `amigo plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Amigo calls the same
     login endpoint, stores the access token in `~/.amigo/credentials/matrix/credentials.json`,
...
- On first connection, Amigo requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Amigo logs a warning.
...
`~/.amigo/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `amigo pairing list matrix`
  - `amigo pairing approve matrix <CODE>`
...
- On startup, Amigo resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
