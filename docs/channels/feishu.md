...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Amigo to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
amigo plugins install @amigo/feishu
```

Local checkout (when running from a git repo):

```bash
amigo plugins install ./extensions/feishu
```
...
If you just installed Amigo, run the wizard:
...
2. Configuring app credentials in Amigo
...
1. You already ran `amigo channels add` for Feishu
2. The gateway is running (`amigo gateway status`)
...
```bash
amigo pairing approve feishu <CODE>
```
...
2. Run `amigo logs --follow` and look for `chat_id`
...
2. Run `amigo logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `amigo gateway status`  | Show gateway status           |
| `amigo gateway install` | Install/start gateway service |
| `amigo gateway stop`    | Stop gateway service          |
| `amigo gateway restart` | Restart gateway service       |
| `amigo logs --follow`   | Tail gateway logs             |
...
4. Check logs: `amigo logs --follow`
...
5. Ensure the gateway is running: `amigo gateway status`
6. Check logs: `amigo logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.amigo/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.amigo/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
