...
read_when:
  - You want Amigo to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Amigo to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`amigo onboard`) and `amigo channels add` list optional channel plugins.
...
```bash
amigo plugins install @amigo/nostr
```

Use a local checkout (dev workflows):

```bash
amigo plugins install --link <path-to-amigo>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "amigo@example.com",
        "lud16": "amigo@example.com"
      }
    }
  }
}
