...
read_when:
  - Setting up Twitch chat integration for Amigo
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Amigo connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
amigo plugins install @amigo/twitch
```

Local checkout (when running from a git repo):

```bash
amigo plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "amigo", // Bot's Twitch account
