...
3. Set tokens for Amigo and start the gateway.
...
7. Slash Commands → create `/amigo` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
Amigo can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, Amigo falls back to the user token.
...
```json
{
  "display_information": {
    "name": "Amigo",
    "description": "Slack connector for Amigo"
  },
  "features": {
    "bot_user": {
      "display_name": "Amigo",
    ...
    "slash_commands": [
      {
        "command": "/amigo",
        "description": "Send a message to Amigo",
        "should_escape": false
      }
    ]
  },
...
By default, Amigo replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, Amigo infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `amigo pairing approve slack <code>`.
...
- On startup, Amigo resolves channel/user names in allowlists to IDs (when tokens allow)
