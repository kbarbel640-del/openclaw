...
read_when:
  - You want to understand what “context” means in Amigo
...
“Context” is **everything Amigo sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Amigo-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Amigo builds the system prompt

The system prompt is **Amigo-owned** and rebuilt each run. It includes:
...
By default, Amigo injects a fixed set of workspace files (if present):
