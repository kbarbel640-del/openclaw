...
- `/usage cost` in chats: local cost summary aggregated from Amigo session logs.
- CLI: `amigo status --usage` prints a full per-provider breakdown.
- CLI: `amigo channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
