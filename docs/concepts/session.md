...
- Cron jobs reuse an existing session when it is still fresh per the configured reset policy; a new `sessionId` is minted when the session has expired or `forceNew` is set.
