---
summary: "Amigo on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Amigo on a Raspberry Pi
  - Running Amigo on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Amigo on Raspberry Pi

## Goal

Run a persistent, always-on Amigo Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Amigo features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled amigo

# Enable if not
sudo systemctl enable amigo

# Start on boot
sudo systemctl start amigo
```
...
```bash
# Check logs
journalctl -u amigo --no-pager -n 100

# Common fix: rebuild
cd ~/amigo  # if using hackable install
npm run build
sudo systemctl restart amigo
```
