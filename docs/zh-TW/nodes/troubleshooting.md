---
summary: "疑難排解節點配對、前景需求、權限與工具失敗問題"
read_when:
  - 節點已連線，但 camera/canvas/screen/exec 工具失敗
...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T08:15:05Z
---
...
若你看到 `NODE_BACKGROUND_UNAVAILABLE`，請將節點應用程式切換到前景後重試。
...
| `location.get`               | 使用期間或永遠（依模式而定） | 依模式為前景／背景定位     | 位置權限                 | `LOCATION_PERMISSION_REQUIRED` |
...
- `NODE_BACKGROUND_UNAVAILABLE` → 應用程式在背景；請切換到前景。
- `CAMERA_DISABLED` → 節點設定中已停用相機切換。
- `*_PERMISSION_REQUIRED` → 缺少／被拒絕的 OS 權限。
- `LOCATION_DISABLED` → 位置模式已關閉。
- `LOCATION_PERMISSION_REQUIRED` → 未授予所請求的位置模式。
- `LOCATION_BACKGROUND_UNAVAILABLE` → 應用程式在背景，但僅存在「使用期間」權限。
