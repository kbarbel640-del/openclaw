...
- 決定性路由：回覆一律回到 Signal。
- 私訊共用代理程式的主要工作階段；群組為隔離（`agent:<agentId>:signal:group:<groupId>`）。
...
- 若在**你的個人 Signal 帳號**上執行機器人，將忽略你自己的訊息（迴圈保護）。
- 若要「我傳訊給機器人，它回覆」，請使用**獨立的機器人號碼**。
...
多帳號支援：使用 `channels.signal.accounts`，搭配每個帳號的設定與選用的 `name`。共享模式請見 [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts)。

## 外部常駐程式模式（httpUrl）

若你想自行管理 `signal-cli`（JVM 冷啟動慢、容器初始化或共享 CPU），可獨立執行常駐程式並讓 OpenClaw 指向它：
...
這會略過 OpenClaw 內的自動啟動與啟動等待。若自動啟動時啟動很慢，請設定 `channels.signal.startupTimeoutMs`。
...
- 未知的寄件者會收到配對碼；在核准前訊息會被忽略（配對碼 1 小時後過期）。
- 核准方式：
  - `openclaw pairing list signal`
  - `openclaw pairing approve signal <CODE>`
- 配對是 Signal 私訊的預設權杖交換。詳情：[配對](/start/pairing)
- 僅 UUID 的寄件者（來自 `sourceUuid`）會以 `uuid:<id>` 形式儲存在 `channels.signal.allowFrom`。

群組：

- `channels.signal.groupPolicy = open | allowlist | disabled`。
- 當設定 `allowlist` 時，`channels.signal.groupAllowFrom` 控制哪些人可在群組中觸發。
...
- 外送文字會分段至 `channels.signal.textChunkLimit`（預設 4000）。
- 可選的換行分段：設定 `channels.signal.chunkMode="newline"`，在長度分段前先依空白行（段落邊界）切分。
- 支援附件（從 `signal-cli` 擷取 base64）。
- 預設媒體上限：`channels.signal.mediaMaxMb`（預設 8）。
- 使用 `channels.signal.ignoreAttachments` 可略過下載媒體。
- 群組歷史脈絡使用 `channels.signal.historyLimit`（或 `channels.signal.accounts.*.historyLimit`），回退為 `messages.groupChat.historyLimit`。設定 `0` 以停用（預設 50）。
...
- 目標：寄件者 E.164 或 UUID（使用配對輸出中的 `uuid:<id>`；僅 UUID 也可）。
...
- `channels.signal.allowFrom`：私訊允許清單（E.164 或 `uuid:<id>`）。`open` 需要 `"*"`。Signal 沒有使用者名稱；請使用電話／UUID 身分。
- `channels.signal.groupPolicy`：`open | allowlist | disabled`（預設：允許清單）。
- `channels.signal.groupAllowFrom`：群組寄件者允許清單。
- `channels.signal.historyLimit`：作為脈絡包含的最大群組訊息數（0 為停用）。
- `channels.signal.dmHistoryLimit`：以使用者回合數計的私訊歷史上限。每使用者覆寫：`channels.signal.dms["<phone_or_uuid>"].historyLimit`。
- `channels.signal.textChunkLimit`：外送分段大小（字元）。
- `channels.signal.chunkMode`：`length`（預設）或 `newline`，在長度分段前先依空白行（段落邊界）切分。
- `channels.signal.mediaMaxMb`：進／出站媒體上限（MB）。
