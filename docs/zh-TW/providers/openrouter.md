---
summary: "使用 OpenRouter 的統一 API，在 OpenClaw 中存取多種模型"
read_when:
  - 你想要一把 API 金鑰即可使用多個 LLM
  - 你想在 OpenClaw 中透過 OpenRouter 執行模型
title: "OpenRouter"
x-i18n:
  source_path: providers/openrouter.md
  source_hash: b7e29fc9c456c64d
  provider: openai
  model: gpt-5.2-chat-latest
  workflow: v1
  generated_at: 2026-02-08T06:54:17Z
---

# OpenRouter

OpenRouter 提供一個**統一 API**，可將請求路由到單一端點與 API 金鑰背後的多個模型。它與 OpenAI 相容，因此只需切換基底 URL，大多數 OpenAI SDK 即可運作。

## CLI 設定

```bash
openclaw onboard --auth-choice apiKey --token-provider openrouter --token "$OPENROUTER_API_KEY"
```

## 設定片段

```json5
{
  env: { OPENROUTER_API_KEY: "sk-or-..." },
  agents: {
    defaults: {
      model: { primary: "openrouter/anthropic/claude-sonnet-4-5" },
    },
  },
}
```

## 注意事項

- 模型參照為 `openrouter/<provider>/<model>`。
- 如需更多模型／提供者選項，請參閱 [/concepts/model-providers](/concepts/model-providers)。
- OpenRouter 在底層使用帶有你的 API 金鑰的 Bearer 權杖。
