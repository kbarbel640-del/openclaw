---
summary: 「用於入站語音備註的 Deepgram 轉錄」
read_when:
  - 「你想要為音訊附件使用 Deepgram 語音轉文字」
...
Deepgram 是一個語音轉文字 API。在 OpenClaw 中，它用於透過 `tools.media.audio` 進行 **入站音訊／語音備註
轉錄**。

啟用後，OpenClaw 會將音訊檔案上傳至 Deepgram，並把轉錄文字注入回覆流程
（`{{Transcript}}` + `[Audio]` 區塊）。這 **不是串流**；
而是使用預先錄製的轉錄端點。
...
- 驗證方式遵循標準的提供者驗證順序；`DEEPGRAM_API_KEY` 是最簡單的做法。
