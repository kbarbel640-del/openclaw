...
OpenClaw 可透過 pi‑ai 的 **Bedrock Converse** 串流提供者使用 **Amazon Bedrock** 模型。Bedrock 的驗證使用 **AWS SDK 預設憑證鏈**，而非 API 金鑰。

## pi‑ai 支援項目

- Provider：`amazon-bedrock`
- API：`bedrock-converse-stream`
- Auth：AWS 憑證（環境變數、共用設定，或執行個體角色）
...
2. 在你的設定中加入一個 Bedrock 提供者與模型（不需要 `apiKey`）：
...
當 OpenClaw 執行於已附加 IAM 角色的 EC2 執行個體上時，AWS SDK 會自動使用執行個體中繼資料服務（IMDS）進行驗證。然而，OpenClaw 目前的憑證偵測僅檢查環境變數，而不檢查 IMDS 憑證。

**因應方式：** 設定 `AWS_PROFILE=default` 以指示 AWS 憑證可用。實際的驗證仍會透過 IMDS 使用執行個體角色。
...
- 若你使用設定檔，請在 Gateway 閘道器 主機上設定 `AWS_PROFILE`。
- OpenClaw 會依下列順序呈現憑證來源：`AWS_BEARER_TOKEN_BEDROCK`，
  接著是 `AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY`，然後是 `AWS_PROFILE`，最後為
  AWS SDK 預設憑證鏈。
- 推理支援取決於模型；請查看 Bedrock 模型卡以取得最新能力。
- 若你偏好受管金鑰流程，也可以在 Bedrock 前方放置一個相容 OpenAI 的代理，並將其設定為 OpenAI 提供者。
