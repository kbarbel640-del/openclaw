---
summary: "「openclaw browser」的 CLI 參考（設定檔、分頁、動作、擴充功能轉送）"
read_when:
  - "你使用「openclaw browser」並想要常見任務的範例"
  - "你想要透過節點主機控制在另一台機器上執行的瀏覽器"
  - "你想要使用 Chrome 擴充功能轉送（透過工具列按鈕附加／解除）"
title: "browser"
x-i18n:
...
- Chrome 擴充功能轉送：[Chrome extension](/tools/chrome-extension)
...
- `openclaw`：啟動／附加到專用的 OpenClaw 管理之 Chrome 執行個體（隔離的使用者資料目錄）。
- `chrome`：透過 Chrome 擴充功能轉送控制你現有的 Chrome 分頁。
...
接著 Chrome → `chrome://extensions` → 啟用「Developer mode」→「Load unpacked」→ 選取列印出的資料夾。

完整指南：[Chrome extension](/tools/chrome-extension)

## 遠端瀏覽器控制（節點主機代理）

如果 Gateway 與瀏覽器不在同一台機器上，請在具備 Chrome／Brave／Edge／Chromium 的機器上執行 **node host**。Gateway 會將瀏覽器動作代理到該節點（不需要另行的瀏覽器控制伺服器）。

使用 `gateway.nodes.browser.mode` 來控制自動路由，並在連線多個節點時使用 `gateway.nodes.browser.node` 來釘選特定節點。
