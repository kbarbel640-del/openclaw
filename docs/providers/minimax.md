---
summary: "Use MiniMax M2.1 in Amigo"
read_when:
  - You want MiniMax models in Amigo
...
```bash
amigo plugins enable minimax-portal-auth  # skip if already loaded.
amigo gateway restart  # restart if gateway is already running
amigo onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/amigo/amigo/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `amigo.json`:
...
- Use `amigo models list` and `amigo models set minimax/MiniMax-M2.1` to switch.
...
- Running `amigo configure` and selecting **MiniMax M2.1**, or
