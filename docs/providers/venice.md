---
summary: "Use Venice AI privacy-focused models in Amigo"
read_when:
  - You want privacy-focused inference in Amigo
...
```bash
amigo onboard --auth-choice venice-api-key
```
...
```bash
amigo chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Amigo shows all available Venice models. Pick based on your needs:
...
```bash
amigo models set venice/claude-opus-45
amigo models set venice/llama-3.3-70b
```

List all available models:

```bash
amigo models list | grep venice
```

## Configure via `amigo configure`

1. Run `amigo configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Amigo automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
amigo chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
amigo chat --model venice/claude-opus-45

# Use uncensored model
amigo chat --model venice/venice-uncensored

# Use vision model with image
amigo chat --model venice/qwen3-vl-235b-a22b

# Use coding model
amigo chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `amigo models list` to see currently available models. Some models may be temporarily offline.
