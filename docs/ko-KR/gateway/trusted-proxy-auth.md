...
`gateway.auth.mode = "trusted-proxy"`가 활성화되고 요청이 신뢰할 수 있는 프록시 검사를 통과하면, 제어 UI WebSocket 세션은 장치 페어링 ID 없이도 연결할 수 있습니다.

영향:

- 이 모드에서 페어링은 더 이상 제어 UI 접근의 주요 게이트가 아닙니다.
- 리버스 프록시 인증 정책과 `allowUsers`가 실질적인 접근 제어가 됩니다.
- 게이트웨이 수신을 신뢰할 수 있는 프록시 IP로만 잠가두세요 (`gateway.trustedProxies` + 방화벽).
