...
- UPN/표시 이름은 변경 가능하며, 직접 매칭은 기본적으로 비활성화되어 있고 `channels.msteams.dangerouslyAllowNameMatching: true`로만 활성화됩니다.
- 마법사는 자격 증명이 허용될 때 Microsoft Graph를 통해 이름을 ID로 해석할 수 있습니다.
...
- `toolsBySender` 키는 명시적 접두사를 사용해야 합니다:
  `id:`, `e164:`, `username:`, `name:` (기존 접두사 없는 키는 여전히 `id:`로만 매핑됩니다).
