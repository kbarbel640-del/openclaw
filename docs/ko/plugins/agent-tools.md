...
read_when:
  - 플러그인에 새 에이전트 도구를 추가하려는 경우
  - 허용 목록을 통해 도구를 선택적으로 활성화해야 하는 경우
title: "플러그인 에이전트 도구"
x-i18n:
...
OpenClaw 플러그인은 에이전트 실행 중 LLM 에 노출되는 **에이전트 도구**(JSON‑schema 함수)를 등록할 수 있습니다. 도구는 **필수**(항상 사용 가능) 또는 **선택적**(옵트인)일 수 있습니다.

에이전트 도구는 메인 설정의 `tools` 아래에서, 또는 에이전트별로 `agents.list[].tools` 아래에서 설정합니다. 허용 목록/차단 목록 정책은 에이전트가 호출할 수 있는 도구를 제어합니다.
...
도구 가용성에 영향을 미치는 기타 설정 항목:

- 플러그인 도구만 명시한 허용 목록은 플러그인 옵트인으로 처리됩니다. 핵심 도구는 허용 목록에 핵심 도구나 그룹을 함께 포함하지 않는 한 계속 활성화됩니다.
...
- 허용 목록에 사용되는 플러그인 id 는 핵심 도구 이름과 충돌해서는 안 됩니다.
