---
summary: "OpenClaw 에서 Venice AI 의 개인정보 보호 중심 모델 사용"
read_when:
  - OpenClaw 에서 개인정보 보호 중심 추론이 필요할 때
...
**Venice** 는 개인정보 보호 우선 추론을 위한 당사의 하이라이트 Venice 설정으로, 선택적으로 독점 모델에 대한 익명화된 액세스를 제공합니다.

Venice AI 는 검열되지 않은 모델을 지원하며, 익명화된 프록시를 통해 주요 독점 모델에 접근할 수 있는 개인정보 보호 중심 AI 추론을 제공합니다. 모든 추론은 기본적으로 비공개입니다—데이터 학습 없음, 로깅 없음.

## OpenClaw 에서 Venice 를 사용하는 이유

- **비공개 추론**: 오픈 소스 모델에 대해 비공개 추론 제공 (로깅 없음).
- **검열되지 않은 모델**: 필요할 때 콘텐츠 제한 없는 모델 사용.
- **익명화된 액세스**: 품질이 중요할 때 독점 모델 (Opus/GPT/Gemini) 에 대한 익명화된 액세스.
- OpenAI 호환 `/v1` 엔드포인트.

## 개인정보 보호 모드

Venice 는 두 가지 개인정보 보호 수준을 제공합니다 — 모델 선택에 있어 이를 이해하는 것이 중요합니다:

| 모드           | 설명                                                                                                                | 모델                                        |
| -------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- |
| **Private**    | 완전 비공개. 프롬프트/응답은 **절대 저장되거나 로깅되지 않음**. 일시적.                                             | Llama, Qwen, DeepSeek, Venice Uncensored 등 |
| **Anonymized** | 메타데이터가 제거된 상태로 Venice 를 통해 프록시 처리. 기본 프로바이더 (OpenAI, Anthropic) 는 익명화된 요청을 확인. | Claude, GPT, Gemini, Grok, Kimi, MiniMax    |

## 기능

- **개인정보 보호 중심**: "private" (완전 비공개) 와 "anonymized" (프록시) 모드 중 선택
- **검열되지 않은 모델**: 콘텐츠 제한 없는 모델에 접근
- **주요 모델 접근**: Venice 의 익명화된 프록시를 통해 Claude, GPT-5.2, Gemini, Grok 사용
- **OpenAI 호환 API**: 손쉬운 통합을 위한 표준 `/v1` 엔드포인트
- **스트리밍**: ✅ 모든 모델에서 지원
- **함수 호출**: ✅ 일부 모델에서 지원 (모델 기능 확인)
- **비전**: ✅ 비전 기능이 있는 모델에서 지원
- **하드 레이트 제한 없음**: 극단적인 사용량에 대해서는 공정 사용 제한이 적용될 수 있음
...
- **기본값 (권장)**: 비공개이며 균형 잡힌 성능의 `venice/llama-3.3-70b`.
- **최고의 전반적 품질**: 고난도 작업에 `venice/claude-opus-45` (Opus 가 여전히 가장 강력함).
- **개인정보 보호**: 완전 비공개 추론을 위해 "private" 모델 선택.
- **기능**: Venice 프록시를 통해 Claude, GPT, Gemini 에 접근하려면 "anonymized" 모델 선택.
...
| **최고의 전반적 품질**          | `claude-opus-45`                 | 고난도 작업에서 Opus 가 여전히 최강 |
| **개인정보 보호 + Claude 품질** | `claude-opus-45`                 | 익명화된 프록시를 통한 최상의 추론  |
...
`VENICE_API_KEY` 가 설정되면 OpenClaw 는 Venice API 에서 모델을 자동으로 디스커버리합니다. API 에 접근할 수 없는 경우 정적 카탈로그로 폴백합니다.

`/models` 엔드포인트는 공개되어 있으며 (목록 조회에는 인증 불필요), 추론에는 유효한 API 키가 필요합니다.
...
| --------------- | ------------------------------------------------------------- |
| **스트리밍**    | ✅ 모든 모델                                                  |
| **함수 호출**   | ✅ 대부분의 모델 (`supportsFunctionCalling` 를 API 에서 확인) |
| **비전/이미지** | ✅ "Vision" 기능으로 표시된 모델                              |
| **JSON 모드**   | ✅ `response_format` 를 통해 지원                             |

## 가격

Venice 는 크레딧 기반 시스템을 사용합니다. 현재 요금은 [venice.ai/pricing](https://venice.ai/pricing) 에서 확인하십시오:

- **Private 모델**: 일반적으로 비용이 더 낮음
- **Anonymized 모델**: 직접 API 가격 + 소액의 Venice 수수료와 유사

## 비교: Venice vs Direct API

| 항목              | Venice (Anonymized)     | Direct API      |
| ----------------- | ----------------------- | --------------- |
| **개인정보 보호** | 메타데이터 제거, 익명화 | 계정 연결됨     |
| **지연 시간**     | +10-50ms (프록시)       | 직접            |
| **기능**          | 대부분의 기능 지원      | 전체 기능       |
| **청구**          | Venice 크레딧           | 프로바이더 과금 |
...
Venice 모델 카탈로그는 동적으로 업데이트됩니다. 현재 사용 가능한 모델을 확인하려면 `openclaw models list` 를 실행하십시오. 일부 모델은 일시적으로 오프라인일 수 있습니다.
