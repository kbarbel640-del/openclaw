---
summary: "Qianfan 의 통합 API 를 사용하여 OpenClaw 에서 다양한 모델에 접근합니다"
read_when:
  - 여러 LLM 을 위한 단일 API 키가 필요할 때
...
Qianfan 은 Baidu 의 MaaS 플랫폼으로, 단일 엔드포인트와 API 키 뒤에서 여러 모델로 요청을 라우팅하는 **통합 API** 를 제공합니다. OpenAI 와 호환되므로, 기본 URL 만 전환하면 대부분의 OpenAI SDK 가 작동합니다.
