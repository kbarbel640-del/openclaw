---
summary: "OpenClaw 에서 API 키 또는 Codex 구독을 통해 OpenAI 사용"
read_when:
...
OpenAI 는 GPT 모델을 위한 개발자 API 를 제공합니다. Codex 는 구독 기반 액세스를 위한 **ChatGPT 로그인** 또는 사용량 기반 액세스를 위한 **API 키** 로그인을 지원합니다. Codex 클라우드는 ChatGPT 로그인이 필요합니다.
...
- 모델 참조는 항상 `provider/model` 를 사용합니다 (자세한 내용은 [/concepts/models](/concepts/models) 참조).
