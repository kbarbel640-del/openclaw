---
summary: "페어링 개요: 누가 다이렉트 메시지로 연락할 수 있는지와 어떤 노드가 참여할 수 있는지 승인"
read_when:
  - 다이렉트 메시지 접근 제어 설정
...
## 1) 다이렉트 메시지 페어링 (수신 채팅 접근)

채널이 DM 정책 `pairing` 로 구성된 경우, 알 수 없는 발신자는 짧은 코드를 받으며 승인할 때까지 메시지가 **처리되지 않습니다**.

기본 DM 정책은 다음 문서에 설명되어 있습니다: [Security](/gateway/security)

페어링 코드:

- 8자, 대문자, 혼동되는 문자는 제외 (`0O1I`).
- **1시간 후 만료**됩니다. 봇은 새 요청이 생성될 때에만 페어링 메시지를 전송합니다 (발신자당 대략 시간당 1회).
...
이 항목들은 어시스턴트에 대한 접근을 제어하므로 민감 정보로 취급하십시오.

## 2) 노드 디바이스 페어링 (iOS/Android/macOS/헤드리스 노드)

노드는 `role: node` 와 함께 **디바이스**로서 Gateway(게이트웨이)에 연결됩니다. Gateway(게이트웨이)는 승인되어야 하는 디바이스 페어링 요청을 생성합니다.
...
- `pending.json` (단기 유지; 대기 요청은 만료됨)
- `paired.json` (페어링된 디바이스 + 토큰)

### 참고

- 레거시 `node.pair.*` API (CLI: `openclaw nodes pending/approve`) 는
  별도의 Gateway(게이트웨이) 소유 페어링 저장소입니다. WS 노드는 여전히 디바이스 페어링이 필요합니다.
