---
summary: "표면(WhatsApp/Telegram/Discord/Slack/Signal/iMessage/Microsoft Teams) 전반의 그룹 채팅 동작"
read_when:
...
OpenClaw 는 사용자의 메시징 계정 안에서 ‘동작’합니다. 별도의 WhatsApp 봇 사용자는 존재하지 않습니다.  
**사용자 본인**이 어떤 그룹에 속해 있다면, OpenClaw 는 해당 그룹을 보고 그 안에서 응답할 수 있습니다.
...
의미: 허용 목록에 있는 발신자는 OpenClaw 를 멘션하여 트리거할 수 있습니다.

> TL;DR
>
> - **DM 접근**은 `*.allowFrom` 에 의해 제어됩니다.
> - **그룹 접근**은 `*.groupPolicy` + 허용 목록 (`*.groups`, `*.groupAllowFrom`)에 의해 제어됩니다.
...
- 다이렉트 채팅은 메인 세션을 사용합니다 (또는 설정 시 발신자별 세션).
- 하트비트는 그룹 세션에서 건너뜁니다.

## 패턴: 개인 DM + 공개 그룹 (단일 에이전트)

네 — “개인” 트래픽이 **DM**, “공개” 트래픽이 **그룹**인 경우 잘 작동합니다.

이유: 단일 에이전트 모드에서 DM 은 일반적으로 **메인** 세션 키 (`agent:main:main`)에 들어가고, 그룹은 항상 **비메인** 세션 키 (`agent:main:<channel>:group:<id>`)를 사용합니다. `mode: "non-main"` 로 샌드박스 처리를 활성화하면, 그룹 세션은 Docker 에서 실행되고 메인 DM 세션은 호스트에 그대로 유지됩니다.

이렇게 하면 하나의 에이전트 “두뇌”(공유 작업 공간 + 메모리)를 유지하면서, 두 가지 실행 자세를 가질 수 있습니다:

- **DM**: 전체 도구 (호스트)
- **그룹**: 샌드박스 + 제한된 도구 (Docker)

> “개인”과 “공개”가 절대 섞이면 안 되는, 완전히 분리된 작업 공간/페르소나가 필요하다면 두 번째 에이전트 + 바인딩을 사용하십시오. [Multi-Agent Routing](/concepts/multi-agent) 참고.

예시 (DM 은 호스트, 그룹은 샌드박스 + 메시징 전용 도구):
...
“호스트 접근 없음” 대신 “그룹은 폴더 X 만 볼 수 있음”을 원하십니까? `workspaceAccess: "none"` 를 유지하고 허용 목록에 있는 경로만 샌드박스에 마운트하십시오:
...
- 바인드 마운트 상세: [Sandboxing](/gateway/sandboxing#custom-bind-mounts)

## 표시 레이블

- UI 레이블은 가능할 경우 `displayName` 를 사용하며, `<channel>:<token>` 형식으로 포맷됩니다.
...
- 멘션 게이팅은 멘션 감지가 가능한 경우에만 적용됩니다 (네이티브 멘션 또는 `mentionPatterns` 설정).
- Discord 기본값은 `channels.discord.guilds."*"` 에 있으며 (길드/채널별 재정의 가능).
- 그룹 히스토리 컨텍스트는 채널 전반에서 일관되게 래핑되며 **보류(pending) 전용**입니다 (멘션 게이팅으로 건너뛴 메시지). 전역 기본값은 `messages.groupChat.historyLimit`, 재정의는 `channels.<channel>.historyLimit` (또는 `channels.<channel>.accounts.*.historyLimit`)를 사용합니다. 비활성화하려면 `0` 를 설정하십시오.

## 그룹/채널 도구 제한 (선택 사항)

일부 채널 설정은 **특정 그룹/룸/채널 내부**에서 사용 가능한 도구를 제한할 수 있습니다.

- `tools`: 그룹 전체에 대해 도구 허용/차단.
- `toolsBySender`: 그룹 내 발신자별 재정의 (키는 채널에 따라 발신자 ID/사용자 이름/이메일/전화번호). 와일드카드로 `"*"` 사용.

해결 순서 (가장 구체적인 규칙이 우선):
...
`channels.whatsapp.groups`, `channels.telegram.groups`, 또는 `channels.imessage.groups` 이 설정되면 해당 키는 그룹 허용 목록으로 동작합니다. 기본 멘션 동작을 유지하면서 모든 그룹을 허용하려면 `"*"` 를 사용하십시오.
...
소유자는 `channels.whatsapp.allowFrom` 로 결정됩니다 (미설정 시 봇 자신의 E.164). 명령은 단독 메시지로 보내십시오. 다른 표면은 현재 `/activation` 를 무시합니다.
...
- Telegram 포럼 토픽은 추가로 `MessageThreadId` 및 `IsForum` 를 포함합니다.

에이전트 시스템 프롬프트에는 새 그룹 세션의 첫 턴에 그룹 소개가 포함됩니다. 이는 모델이 사람처럼 응답하고, Markdown 표를 피하며, 리터럴 `\n` 시퀀스를 입력하지 않도록 상기시킵니다.
...
WhatsApp 전용 동작(히스토리 주입, 멘션 처리 상세)은 [Group messages](/channels/group-messages)를 참고하십시오.
