...
OpenClaw 는 메시지가 들어온 **같은 채널로 응답을 다시 라우팅**합니다.  
모델이 채널을 선택하지 않으며, 라우팅은 결정적이고 호스트 설정에 의해 제어됩니다.

## 핵심 용어

- **채널**: `whatsapp`, `telegram`, `discord`, `slack`, `signal`, `imessage`, `webchat`.
- **AccountId**: 채널별 계정 인스턴스 (지원되는 경우).
- **AgentId**: 격리된 작업 공간 + 세션 저장소 ('두뇌').
...
6. **기본 에이전트** (`agents.list[].default`, 그렇지 않으면 목록의 첫 항목, 최종적으로 `main` 로 폴백).

매칭된 에이전트가 어떤 작업 공간과 세션 저장소가 사용되는지를 결정합니다.

## 브로드캐스트 그룹 (여러 에이전트 실행)

브로드캐스트 그룹을 사용하면 OpenClaw 가 **일반적으로 응답하는 경우**에 동일한 피어에 대해 **여러 에이전트**를 실행할 수 있습니다  
(예: WhatsApp 그룹에서 멘션/활성화 게이팅 이후).
...
- JSONL 트랜스크립트는 저장소와 동일한 위치에 저장됩니다

`session.store` 및 `{agentId}` 템플리팅을 통해 저장소 경로를 재정의할 수 있습니다.

## WebChat 동작

WebChat 은 **선택된 에이전트**에 연결되며 기본적으로 에이전트의 메인 세션을 사용합니다.  
이로 인해 WebChat 에서는 해당 에이전트의 교차 채널 컨텍스트를 한곳에서 확인할 수 있습니다.
