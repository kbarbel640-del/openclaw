...
Tlon 은 Urbit 기반의 탈중앙화 메신저입니다. OpenClaw 는 사용자의 Urbit ship 에 연결하여
다이렉트 메시지와 그룹 채팅 메시지에 응답할 수 있습니다. 그룹 응답은 기본적으로 @ 멘션이 필요하며,
allowlist 를 통해 추가로 제한할 수 있습니다.

상태: 플러그인을 통해 지원됩니다. 다이렉트 메시지, 그룹 멘션, 스레드 답글, 텍스트 전용 미디어 대체
(캡션에 URL 추가)가 지원됩니다. 반응, 설문, 네이티브 미디어 업로드는 지원되지 않습니다.
...
`openclaw message send` 또는 cron 전달과 함께 사용합니다:
...
- 그룹 응답에는 멘션이 필요합니다 (예: `~your-bot-ship`).
- 스레드 답글: 인바운드 메시지가 스레드에 있는 경우 OpenClaw 는 스레드 내로 답장합니다.
- 미디어: `sendMedia` 는 텍스트 + URL 로 대체됩니다 (네이티브 업로드 없음).
