---
summary: "`openclaw plugins` CLI 참조 (목록, 설치, 활성화/비활성화, 진단)"
read_when:
...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T06:35:31Z
---

# `openclaw plugins`

Gateway(게이트웨이) 플러그인/확장(인프로세스로 로드됨)을 관리합니다.
...
보안 참고: 플러그인 설치는 코드 실행과 동일하게 취급해야 합니다. 고정된 버전을 사용하는 것을 권장합니다.

지원되는 아카이브: `.zip`, `.tgz`, `.tar.gz`, `.tar`.

로컬 디렉토리를 복사하지 않으려면 `--link`를 사용하십시오(`plugins.load.paths`에 추가됨):
