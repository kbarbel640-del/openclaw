...
- `--update`: `openclaw update`의 약어입니다 (소스 설치 전용).
- `-V`, `--version`, `-v`: 버전을 출력하고 종료합니다.

## 출력 스타일링

- ANSI 색상과 진행 표시기는 TTY 세션에서만 렌더링됩니다.
- OSC-8 하이퍼링크는 지원되는 터미널에서 클릭 가능한 링크로 렌더링되며, 그렇지 않으면 일반 URL 로 대체됩니다.
- `--json` (지원되는 경우 `--plain` 포함)은 깔끔한 출력을 위해 스타일링을 비활성화합니다.
- `--no-color`는 ANSI 스타일링을 비활성화하며, `NO_COLOR=1`도 존중됩니다.
- 장시간 실행되는 명령은 진행 표시기를 표시합니다 (지원되는 경우 OSC 9;4).

## 색상 팔레트

OpenClaw 는 CLI 출력에 로브스터 팔레트를 사용합니다.
...
팔레트의 단일 진실 소스: `src/terminal/palette.ts` (일명 'lobster seam').
...
- `openclaw security audit` — 일반적인 보안 실수를 대상으로 설정 + 로컬 상태를 감사합니다.
- `openclaw security audit --deep` — 최선 노력 방식의 실시간 Gateway(게이트웨이) 프로브.
...
- `openclaw plugins list` — 플러그인을 검색합니다 (머신 출력은 `--json` 사용).
- `openclaw plugins info <id>` — 플러그인 상세 정보를 표시합니다.
...
대부분의 플러그인 변경 사항은 Gateway 재시작이 필요합니다. [/plugin](/plugin)을 참고하십시오.
...
- 빠른 진단을 위한 `/status`.
- 영구 설정 변경을 위한 `/config`.
- 런타임 전용 설정 오버라이드를 위한 `/debug` (디스크가 아닌 메모리; `commands.debug: true` 필요).
...
게이트웨이, 워크스페이스, Skills 를 설정하는 대화형 마법사입니다.

옵션:

- `--workspace <dir>`
- `--reset` (마법사 전에 설정 + 자격 증명 + 세션 + 워크스페이스를 재설정)
- `--non-interactive`
- `--mode <local|remote>`
...
- `--skip-health`
- `--skip-ui`
- `--node-manager <npm|pnpm|bun>` (pnpm 권장; bun 은 Gateway 런타임에 권장되지 않음)
- `--json`

### `configure`

대화형 설정 마법사 (모델, 채널, Skills, 게이트웨이).

### `config`

비대화형 설정 헬퍼 (get/set/unset). 하위 명령 없이 `openclaw config`를 실행하면 마법사가 시작됩니다.
...
- `--deep`: 추가 게이트웨이 설치를 위해 시스템 서비스를 스캔합니다.
...
- `channels list`: 설정된 채널과 인증 프로필을 표시합니다.
- `channels status`: 게이트웨이 도달 가능성과 채널 상태를 확인합니다 (`--probe`은 추가 점검을 실행; 게이트웨이 상태 프로브에는 `openclaw health` 또는 `openclaw status --deep` 사용).
...
- `channels add`: 플래그가 없으면 마법사 방식으로 설정하며, 플래그를 전달하면 비대화형 모드로 전환됩니다.
...
- `skills list`: Skills 를 나열합니다 (하위 명령이 없을 때 기본).
- `skills info <name>`: 단일 Skill 의 상세 정보를 표시합니다.
- `skills check`: 준비됨 vs 누락 요구 사항 요약.

옵션:

- `--eligible`: 준비된 Skills 만 표시합니다.
- `--json`: JSON 출력 (스타일 없음).
- `-v`, `--verbose`: 누락 요구 사항 상세를 포함합니다.

팁: Skills 를 검색, 설치, 동기화하려면 `npx clawhub`를 사용하십시오.

### `pairing`

채널 전반에서 다이렉트 메시지 페어링 요청을 승인합니다.
...
Gmail Pub/Sub 훅 설정 + 러너. [/automation/gmail-pubsub](/automation/gmail-pubsub)을 참고하십시오.
...
광역 디바이스 검색 DNS 헬퍼 (CoreDNS + Tailscale). [/gateway/discovery](/gateway/discovery)을 참고하십시오.
...
Gateway 를 통해 하나의 에이전트 턴을 실행합니다 (또는 `--local` 내장).
...
새 격리 에이전트를 추가합니다. 플래그(또는 `--non-interactive`)가 전달되지 않으면 안내형 마법사를 실행합니다; 비대화형 모드에서는 `--workspace`가 필요합니다.
...
바인딩 사양은 `channel[:accountId]`를 사용합니다. WhatsApp 에서 `accountId`가 생략되면 기본 계정 id 가 사용됩니다.
...
전체 옵션과 예시는 [`acp`](/cli/acp)을 참고하십시오.
...
OpenClaw 는 OAuth/API 자격 증명이 있는 경우 프로바이더 사용량/할당량을 노출할 수 있습니다.

노출 위치:

- `/status` (사용 가능한 경우 짧은 프로바이더 사용량 라인을 추가)
- `openclaw status --usage` (전체 프로바이더 분해를 출력)
- macOS 메뉴 막대 (컨텍스트 아래 Usage 섹션)
...
게이트웨이 서비스 + 로컬 데이터를 제거합니다 (CLI 는 유지).
...
- `--non-interactive`에는 `--yes`와 명시적 스코프(또는 `--all`)가 필요합니다.
...
- `--allow-unconfigured`
- `--dev`
- `--reset` (개발 설정 + 자격 증명 + 세션 + 워크스페이스 재설정)
- `--force` (포트의 기존 리스너 종료)
- `--verbose`
- `--claude-cli-logs`
...
- `gateway status`은 서비스의 해결된 포트/설정을 사용하여 기본적으로 Gateway RPC 를 프로브합니다 (`--url/--token/--password`로 재정의).
- `gateway status`은 스크립팅을 위해 `--no-probe`, `--deep`, `--json`을 지원합니다.
- `gateway status`는 감지 가능한 경우 레거시 또는 추가 게이트웨이 서비스도 노출합니다 (`--deep`은 시스템 수준 스캔을 추가). 프로필 이름의 OpenClaw 서비스는 일급으로 취급되며 'extra'로 표시되지 않습니다.
...
- `gateway install`는 Node 런타임이 기본값이며, bun 은 **권장되지 않음** (WhatsApp/Telegram 버그).
...
- TTY 세션에서는 색상화된 구조적 뷰를 렌더링하며, 비 TTY 에서는 일반 텍스트로 대체됩니다.
- `--json`는 라인 단위 JSON 을 출력합니다 (한 줄당 하나의 로그 이벤트).
...
Gateway CLI 헬퍼 (`--url`, `--token`, `--password`, `--timeout`, `--expect-final`을 RPC 하위 명령으로 사용).
`--url`를 전달하면 CLI 는 설정이나 환경 자격 증명을 자동 적용하지 않습니다.
`--token` 또는 `--password`을 명시적으로 포함하십시오. 명시적 자격 증명이 없으면 오류입니다.
...
- `config.apply` (설정 검증 + 기록 + 재시작 + 웨이크)
- `config.patch` (부분 업데이트 병합 + 재시작 + 웨이크)
- `update.run` (업데이트 실행 + 재시작 + 웨이크)

팁: `config.set`/`config.apply`/`config.patch`를 직접 호출할 때 설정이 이미 존재한다면
`config.get`에서 `baseHash`를 전달하십시오.

## 모델

폴백 동작과 스캐닝 전략은 [/concepts/models](/concepts/models)을 참고하십시오.
...
- `--probe-provider <name>`
- `--probe-profile <id>` (반복 또는 콤마 구분)
- `--probe-timeout <ms>`
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`

항상 인증 스토어의 프로필에 대한 인증 개요와 OAuth 만료 상태를 포함합니다.
`--probe`은 라이브 요청을 실행합니다 (토큰을 소모하고 속도 제한을 트리거할 수 있음).
...
예약 작업을 관리합니다 (Gateway RPC). [/automation/cron-jobs](/automation/cron-jobs)을 참고하십시오.

하위 명령:

- `cron status [--json]`
- `cron list [--all] [--json]` (기본적으로 테이블 출력; 원시 출력은 `--json` 사용)
- `cron add` (별칭: `create`; `--name` 필요 및 `--at` | `--every` | `--cron` 중 정확히 하나, 그리고 `--system-event` | `--message` 중 정확히 하나의 페이로드 필요)
- `cron edit <id>` (필드 패치)
...
[`openclaw node`](/cli/node)을 참고하십시오.
...
`nodes`는 Gateway 와 통신하며 페어링된 노드를 대상으로 합니다. [/nodes](/nodes)을 참고하십시오.
...
브라우저 제어 CLI (전용 Chrome/Brave/Edge/Chromium). [`openclaw browser`](/cli/browser) 및 [Browser tool](/tools/browser)을 참고하십시오.
