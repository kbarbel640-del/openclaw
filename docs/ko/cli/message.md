...
- 둘 이상의 채널이 구성된 경우 `--channel` 가 필요합니다.
...
- Discord: `channel:<id>` 또는 `user:<id>` (`<@id>` 멘션도 가능; 순수 숫자 id 는 채널로 처리됨)
- Google Chat: `spaces/<spaceId>` 또는 `users/<userId>`
- Slack: `channel:<id>` 또는 `user:<id>` (원시 채널 id 허용)
- Mattermost (plugin): `channel:<id>`, `user:<id>`, 또는 `@username` (순수 id 는 채널로 처리됨)
...
- 지원되는 프로바이더 (Discord/Slack 등)의 경우, `Help` 또는 `#help` 과 같은 채널 이름은 디렉토리 캐시를 통해 해석됩니다.
- 캐시 미스 시, 프로바이더가 지원하는 경우 OpenClaw 가 실시간 디렉토리 조회를 시도합니다.
...
- `send`
  ...
  - Telegram 전용: `--buttons` (`channels.telegram.capabilities.inlineButtons` 를 허용해야 사용 가능)
...
- `react`
  ...
  - 참고: `--remove` 는 `--emoji` 가 필요합니다 (지원되는 경우 자신의 반응을 제거하려면 `--emoji` 를 생략; /tools/reactions 참고).
...
- `timeout`: `--guild-id`, `--user-id` (선택적으로 `--duration-min` 또는 `--until`; 둘 다 생략하면 타임아웃 해제)
