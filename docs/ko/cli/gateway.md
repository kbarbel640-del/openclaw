---
summary: "OpenClaw Gateway CLI(`openclaw gateway`) — 게이트웨이 실행, 조회 및 검색"
read_when:
...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T06:35:40Z
---

# Gateway CLI

Gateway 는 OpenClaw 의 WebSocket 서버(채널, 노드, 세션, 훅)입니다.
...
- `SIGUSR1` 는 인증된 경우 프로세스 내 재시작을 트리거합니다(`commands.restart` 을 활성화하거나 gateway 도구/config apply/update 를 사용).
- `SIGINT`/`SIGTERM` 핸들러는 gateway 프로세스를 중지하지만, 사용자 지정 터미널 상태는 복원하지 않습니다. CLI 를 TUI 또는 raw-mode 입력으로 감싸는 경우 종료 전에 터미널을 복원하십시오.

### 옵션

- `--port <port>`: WebSocket 포트(기본값은 config/env 에서 가져오며, 보통 `18789`).
...
- `--tailscale <off|serve|funnel>`: Tailscale 을 통해 Gateway 를 노출.
- `--tailscale-reset-on-exit`: 종료 시 Tailscale serve/funnel 설정을 재설정.
- `--allow-unconfigured`: config 에 `gateway.mode=local` 없이도 gateway 시작 허용.
- `--dev`: 누락 시 개발용 config + 워크스페이스 생성(BOOTSTRAP.md 건너뜀).
- `--reset`: 개발용 config + 자격 증명 + 세션 + 워크스페이스 재설정(`--dev` 필요).
...
- `--raw-stream`: 원시 모델 스트림 이벤트를 jsonl 로 기록.
...
- `--no-color`(또는 `NO_COLOR=1`): ANSI 를 비활성화하되 사람용 레이아웃 유지.
...
- `--expect-final`: 'final' 응답을 대기(에이전트 호출).

참고: `--url` 을 설정하면 CLI 는 config 또는 환경 변수 자격 증명으로 폴백하지 않습니다.
...
- 원격이 설정되어 있더라도 localhost(loopback).

여러 gateway 에 도달 가능하면 모두 출력합니다. 격리된 프로필/포트(예: 구조용 봇)를 사용할 때 여러 gateway 가 지원되지만, 대부분의 설치는 여전히 단일 gateway 를 실행합니다.
...
#### SSH 를 통한 원격(Mac 앱과의 동등성)

macOS 앱의 'Remote over SSH' 모드는 로컬 포트 포워드를 사용하여 loopback 에만 바인딩된 원격 gateway 도 `ws://127.0.0.1:<port>` 에서 접근 가능하게 합니다.
...
- `--ssh-auto`: 발견된 첫 번째 gateway 호스트를 SSH 대상으로 선택(LAN/WAB 전용).
...
- 멀티캐스트 DNS-SD: `local.`
- 유니캐스트 DNS-SD(Wide-Area Bonjour): 도메인 선택(예: `openclaw.internal.`) 후 split DNS + DNS 서버 설정; [/gateway/bonjour](/gateway/bonjour) 참조

Bonjour 검색이 활성화된 gateway 만(기본값) 비콘을 광고합니다.

Wide-Area 검색 레코드에는(TXT) 다음이 포함됩니다:
