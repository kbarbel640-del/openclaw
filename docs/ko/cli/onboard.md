---
summary: "대화형 온보딩 마법사를 위한 CLI 참조(`openclaw onboard`)"
read_when:
  - Gateway(게이트웨이), 워크스페이스, 인증, 채널, Skills 에 대한 안내형 설정이 필요할 때
title: "온보드"
x-i18n:
...
- `quickstart`: 최소한의 프롬프트로 진행하며 Gateway(게이트웨이) 토큰을 자동 생성합니다.
- `manual`: 포트/바인드/인증에 대한 전체 프롬프트(`advanced` 의 별칭).
- 가장 빠른 첫 채팅: `openclaw dashboard` (컨트롤 UI, 채널 설정 없음).
...
<Note>
`--json` 는 비대화형 모드를 의미하지 않습니다. 스크립트에는 `--non-interactive` 를 사용합니다.
</Note>
