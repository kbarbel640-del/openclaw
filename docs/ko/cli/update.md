---
summary: "`openclaw update` 에 대한 CLI 참조 (안전한 소스 업데이트 + Gateway 자동 재시작)"
read_when:
...
OpenClaw 를 안전하게 업데이트하고 stable/beta/dev 채널 간에 전환합니다.

**npm/pnpm** 으로 설치한 경우(전역 설치, git 메타데이터 없음) 업데이트는 [Updating](/install/updating) 의 패키지 매니저 흐름을 통해 진행됩니다.
...
- `--no-restart`: 업데이트가 성공한 후 Gateway(게이트웨이) 서비스를 재시작하지 않습니다.
...
참고: 다운그레이드는 이전 버전이 설정을 손상시킬 수 있으므로 확인이 필요합니다.
...
(기본값은 재시작). git 체크아웃 없이 `dev` 을 선택하면
체크아웃을 생성할지 제안합니다.

## What it does

채널을 명시적으로 전환할 때(`--channel ...`), OpenClaw 는 설치 방법도 함께
정렬 상태로 유지합니다:
...
- `stable`: 최신 non-beta 태그를 체크아웃한 다음 빌드 + doctor 를 실행합니다.
- `beta`: 최신 `-beta` 태그를 체크아웃한 다음 빌드 + doctor 를 실행합니다.
- `dev`: `main` 를 체크아웃한 다음 fetch + rebase 를 수행합니다.

High-level:

1. 깨끗한 작업 트리(커밋되지 않은 변경 사항 없음)가 필요합니다.
2. 선택한 채널(태그 또는 브랜치)로 전환합니다.
3. 업스트림을 fetch 합니다(dev 전용).
4. dev 전용: 임시 작업 트리에서 사전 검사 lint + TypeScript 빌드를 수행합니다; 최신 커밋이 실패하면 최대 10 커밋까지 거슬러 올라가 가장 최신의 깨끗한 빌드를 찾습니다.
5. 선택한 커밋으로 rebase 합니다(dev 전용).
6. 의존성을 설치합니다(pnpm 우선, npm 대체).
7. 빌드 + Control UI 를 빌드합니다.
8. 최종 “안전 업데이트” 검사로 `openclaw doctor` 를 실행합니다.
9. 플러그인을 활성 채널에 동기화합니다(dev 는 번들 확장을 사용; stable/beta 는 npm 사용) 그리고 npm 으로 설치된 플러그인을 업데이트합니다.
