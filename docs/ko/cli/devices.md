---
summary: "`openclaw devices`에 대한 CLI 참조(디바이스 페어링 + 토큰 회전/폐기)"
read_when:
  - 디바이스 페어링 요청을 승인할 때
  - 디바이스 토큰을 회전하거나 폐기해야 할 때
title: "디바이스"
x-i18n:
...
특정 역할에 대한 디바이스 토큰을 회전합니다(선택적으로 스코프 업데이트).
...
참고: `--url`를 설정하면 CLI는 구성이나 환경 변수 자격 증명으로 대체하지 않습니다.
`--token` 또는 `--password`를 명시적으로 전달하십시오. 명시적 자격 증명이 누락되면 오류입니다.

## Notes

- 토큰 회전은 새로운 토큰(민감 정보)을 반환합니다. 비밀 정보로 취급하십시오.
- 이러한 명령은 `operator.pairing`(또는 `operator.admin`) 스코프가 필요합니다.
