...
이 문서는 “어떻게 테스트하는지”에 대한 가이드입니다:

- 각 스위트가 다루는 범위 (그리고 의도적으로 다루지 않는 범위)
- 일반적인 워크플로우 (로컬, 푸시 전, 디버깅) 에서 실행할 명령
- 라이브 테스트가 자격 증명을 발견하고 모델/프로바이더를 선택하는 방식
- 실제 환경에서 발생한 모델/프로바이더 이슈에 대한 회귀 테스트 추가 방법
...
팁: 실패하는 단일 케이스만 필요할 때는, 아래에 설명된 allowlist 환경 변수를 사용해 라이브 테스트를 좁히는 것이 좋습니다.

## 테스트 스위트 (어디서 무엇이 실행되는가)

스위트는 “현실성 증가” (그리고 불안정성/비용 증가) 로 생각하면 됩니다:
...
- 파일: `src/**/*.e2e.test.ts`
- 범위:
  - 다중 인스턴스 게이트웨이의 엔드 투 엔드 동작
  - WebSocket/HTTP 인터페이스, 노드 페어링, 더 무거운 네트워킹
- 기대 사항:
  - 파이프라인에서 활성화된 경우 CI 에서 실행
  - 실제 키 불필요
  - unit 테스트보다 구성 요소가 많음 (느릴 수 있음)
...
- 범위:
  ...
  - 프로바이더 포맷 변경, 도구 호출 특이점, 인증 문제, rate limit 동작 포착
- 기대 사항:
  - 설계상 CI 안정적이지 않음 (실제 네트워크, 실제 프로바이더 정책, 쿼터, 장애)
  - 비용 발생 / rate limit 사용
  - “전체 실행” 대신 좁힌 부분집합 실행 권장
  - 라이브 실행은 누락된 API 키를 찾기 위해 `~/.profile` 를 소스로 사용
  - Anthropic 키 로테이션: `OPENCLAW_LIVE_ANTHROPIC_KEYS="sk-...,sk-..."` (또는 `OPENCLAW_LIVE_ANTHROPIC_KEY=sk-...`) 또는 여러 `ANTHROPIC_API_KEY*` 변수를 설정; 테스트는 rate limit 시 재시도

## 어떤 스위트를 실행해야 하나요?

다음 결정 표를 사용하세요:

- 로직/테스트 편집: `pnpm test` 실행 (많이 변경했다면 `pnpm test:coverage` 추가)
- 게이트웨이 네트워킹 / WS 프로토콜 / 페어링 수정: `pnpm test:e2e` 추가
- “내 봇이 다운됨” / 프로바이더별 실패 / 도구 호출 디버깅: 좁힌 `pnpm test:live` 실행

## Live: 모델 스모크 (프로파일 키)

라이브 테스트는 실패를 분리하기 위해 두 계층으로 나뉩니다:

- “직접 모델”은 해당 키로 프로바이더/모델이 최소한 응답 가능한지를 확인합니다.
- “게이트웨이 스모크”는 해당 모델에 대해 전체 Gateway(게이트웨이)+에이전트 파이프라인 (세션, 히스토리, 도구, 샌드박스 정책 등) 이 동작하는지를 확인합니다.

### Layer 1: 직접 모델 완료 (게이트웨이 없음)

- 테스트: `src/agents/models.profiles.live.test.ts`
- 목표:
  - 발견된 모델 열거
  - `getApiKeyForModel` 를 사용해 자격 증명이 있는 모델 선택
  - 모델별로 작은 completion 실행 (필요 시 타겟 회귀 포함)
- 활성화 방법:
  - `pnpm test:live` (또는 Vitest 를 직접 호출할 경우 `OPENCLAW_LIVE_TEST=1`)
- 이 스위트를 실제로 실행하려면 `OPENCLAW_LIVE_MODELS=modern` (또는 최신 별칭인 `all`) 을 설정해야 함; 그렇지 않으면 `pnpm test:live` 가 게이트웨이 스모크에 집중하도록 건너뜀
- 모델 선택 방법:
  ...
  - 또는 `OPENCLAW_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,..."` (콤마 allowlist)
- 프로바이더 선택 방법:
...
- 존재 이유:
  ...
  - 작고 고립된 회귀 포함 (예: OpenAI Responses/Codex Responses 추론 재생 + 도구 호출 플로우)

### Layer 2: Gateway(게이트웨이) + dev 에이전트 스모크 (“@openclaw” 의 실제 동작)
...
- 프로브 상세 (실패를 빠르게 설명하기 위함):
  ...
  - `exec+read` 프로브: 테스트가 에이전트에게 `exec` 로 temp 파일에 nonce 를 쓰게 한 뒤, 이를 `read` 하도록 요청
  - 이미지 프로브: 생성된 PNG (고양이 + 랜덤 코드) 를 첨부하고, 모델이 `cat <CODE>` 를 반환하는지 기대
...
- 모델 선택 방법:
  ...
  - 또는 `OPENCLAW_LIVE_GATEWAY_MODELS="provider/model"` (또는 콤마 리스트) 로 좁힘
- 프로바이더 선택 ( “OpenRouter 전체” 회피 ):
  - `OPENCLAW_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (콤마 allowlist)
- 이 라이브 테스트에서는 도구 + 이미지 프로브가 항상 활성화됨:
  - `read` 프로브 + `exec+read` 프로브 (도구 스트레스)
  - 이미지 프로브는 모델이 이미지 입력 지원을 광고할 때 실행
  - 흐름 (개요):
    - 테스트가 “CAT” + 랜덤 코드가 있는 작은 PNG 생성 (`src/gateway/live-image-probe.ts`)
    - 이를 `agent` `attachments: [{ mimeType: "image/png", content: "<base64>" }]` 를 통해 전송
    - 게이트웨이가 첨부 파일을 `images[]` (`src/gateway/server-methods/agent.ts` + `src/gateway/chat-attachments.ts`) 로 파싱
    - 내장 에이전트가 멀티모달 사용자 메시지를 모델로 전달
    - 단언: 응답에 `cat` + 코드가 포함됨 (OCR 허용 오차: 경미한 실수 허용)

팁: 로컬 머신에서 테스트 가능한 항목 (그리고 정확한 `provider/model` id) 을 보려면 다음을 실행하세요:
...
- 목표: Claude Code CLI setup-token (또는 붙여넣은 setup-token 프로파일) 로 Anthropic 프롬프트가 완료되는지 검증.
- 활성화:
  - `pnpm test:live` (또는 Vitest 를 직접 호출할 경우 `OPENCLAW_LIVE_TEST=1`)
...
- Claude Code CLI MCP 설정을 활성화 상태로 유지하려면 `OPENCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` (기본값은 임시 빈 파일로 MCP 설정을 비활성화).
...
좁고 명시적인 allowlist 가 가장 빠르고 불안정성이 적습니다:
...
- `google-gemini-cli/...` 는 로컬 머신의 Gemini CLI 를 사용합니다 (별도 인증 + 도구 특이점).
- Gemini API vs Gemini CLI:
  - API: OpenClaw 가 Google 이 호스팅하는 Gemini API 를 HTTP 로 호출 (API 키 / 프로파일 인증); 대부분의 사용자가 말하는 “Gemini”.
  - CLI: OpenClaw 가 로컬 `gemini` 바이너리를 실행; 자체 인증을 가지며 동작이 다를 수 있음 (스트리밍/도구 지원/버전 차이).

## Live: 모델 매트릭스 (다루는 범위)

고정된 “CI 모델 목록” 은 없지만 (라이브는 옵트인), 다음은 키를 가진 개발 머신에서 정기적으로 커버하기를 **권장** 하는 모델입니다.

### 최신 스모크 세트 (도구 호출 + 이미지)

다음은 계속 동작해야 하는 “공통 모델” 실행입니다:

- OpenAI (Codex 제외): `openai/gpt-5.2` (선택 사항: `openai/gpt-5.1`)
- OpenAI Codex: `openai-codex/gpt-5.3-codex` (선택 사항: `openai-codex/gpt-5.3-codex-codex`)
...
- Mistral: `mistral/`… (도구 가능 모델 중 하나 선택)
- Cerebras: `cerebras/`… (접근 권한이 있다면)
- LM Studio: `lmstudio/`… (로컬; 도구 호출은 API 모드에 따라 다름)

### Vision: 이미지 전송 (첨부 → 멀티모달 메시지)

이미지 프로브를 실행하기 위해 `OPENCLAW_LIVE_GATEWAY_MODELS` 에 이미지 지원 모델을 최소 하나 포함하세요 (Claude/Gemini/OpenAI 비전 지원 변형 등).
...
- `models.providers` 를 통한 (커스텀 엔드포인트): `minimax` (클라우드/API), 그리고 OpenAI/Anthropic 호환 프록시 (LM Studio, vLLM, LiteLLM 등)

팁: 문서에 “모든 모델” 을 하드코딩하려 하지 마세요. 권위 있는 목록은 로컬 머신에서 `discoverModels(...)` 가 반환하는 값 + 사용 가능한 키입니다.

## 자격 증명 (절대 커밋하지 마세요)

라이브 테스트는 CLI 와 동일한 방식으로 자격 증명을 발견합니다. 실질적인 의미:

- CLI 가 동작하면 라이브 테스트도 동일한 키를 찾을 수 있어야 합니다.
- 라이브 테스트가 “자격 증명 없음” 을 보고하면, `openclaw models list` / 모델 선택을 디버깅하는 것과 같은 방식으로 디버깅하세요.

- 프로파일 스토어: `~/.openclaw/credentials/` (권장; 테스트에서 말하는 “프로파일 키” 의 의미)
- 설정: `~/.openclaw/openclaw.json` (또는 `OPENCLAW_CONFIG_PATH`)

환경 변수 키에 의존하고 싶다면 (예: `~/.profile` 에 export), `source ~/.profile` 이후 로컬 테스트를 실행하거나, 아래의 Docker 러너를 사용하세요 (컨테이너에 `~/.profile` 를 마운트할 수 있음).
...
이들은 리포 Docker 이미지 내부에서 `pnpm test:live` 를 실행하며, 로컬 설정 디렉토리와 워크스페이스를 마운트합니다 (마운트된 경우 `~/.profile` 소싱):

- 직접 모델: `pnpm test:docker:live-models` (스크립트: `scripts/test-live-models-docker.sh`)
- Gateway(게이트웨이) + dev 에이전트: `pnpm test:docker:live-gateway` (스크립트: `scripts/test-live-gateway-models-docker.sh`)
...
- `OPENCLAW_PROFILE_FILE=...` (기본값: `~/.profile`) → `/home/node/.profile` 에 마운트되며 테스트 실행 전 소싱
- `OPENCLAW_LIVE_GATEWAY_MODELS=...` / `OPENCLAW_LIVE_MODELS=...` 로 실행 범위 축소
- 자격 증명이 환경 변수가 아닌 프로파일 스토어에서 오도록 보장하려면 `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1`

## 문서 정합성

문서 편집 후 문서 검사를 실행하세요: `pnpm docs:list`.

## 오프라인 회귀 (CI 안전)

실제 프로바이더 없이 “실제 파이프라인” 회귀 테스트입니다:

- 게이트웨이 도구 호출 (mock OpenAI, 실제 게이트웨이 + 에이전트 루프): `src/gateway/gateway.tool-calling.mock-openai.test.ts`
- 게이트웨이 마법사 (WS `wizard.start`/`wizard.next`, 설정 작성 + 인증 강제): `src/gateway/gateway.wizard.e2e.test.ts`

## 에이전트 신뢰성 평가 (Skills)

이미 “에이전트 신뢰성 평가” 처럼 동작하는 CI 안전 테스트가 몇 개 있습니다:

- 실제 게이트웨이 + 에이전트 루프를 통한 mock 도구 호출 (`src/gateway/gateway.tool-calling.mock-openai.test.ts`).
- 세션 연결과 설정 효과를 검증하는 엔드 투 엔드 마법사 플로우 (`src/gateway/gateway.wizard.e2e.test.ts`).

Skills 에 아직 부족한 부분 ([Skills](/tools/skills) 참고):

- **의사결정:** 프롬프트에 skills 가 나열될 때, 에이전트가 올바른 skill 을 선택하는가 (또는 관련 없는 것을 회피하는가)?
- **컴플라이언스:** 사용 전에 `SKILL.md` 를 읽고 필수 단계/인자를 따르는가?
- **워크플로 계약:** 도구 순서, 세션 히스토리 전달, 샌드박스 경계를 검증하는 다중 턴 시나리오.

향후 평가는 우선 결정론적으로 유지해야 합니다:

- mock 프로바이더를 사용해 도구 호출 + 순서, skill 파일 읽기, 세션 연결을 단언하는 시나리오 러너.
- skill 중심의 소규모 시나리오 스위트 (사용 vs 회피, 게이팅, 프롬프트 인젝션).
- CI 안전 스위트가 준비된 이후에만 선택적 라이브 평가 (옵트인, 환경 변수 게이팅).

## 회귀 추가 (가이드)

라이브에서 발견된 프로바이더/모델 이슈를 수정할 때:

- 가능하면 CI 안전 회귀를 추가 (프로바이더 mock/stub, 또는 정확한 요청 변환 캡처)
- 본질적으로 라이브 전용이라면 (rate limit, 인증 정책), 라이브 테스트를 좁게 유지하고 환경 변수로 옵트인
- 버그를 포착하는 가장 작은 계층을 타겟팅:
