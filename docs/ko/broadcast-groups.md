---
summary: "여러 에이전트에게 WhatsApp 메시지를 브로드캐스트합니다"
read_when:
  - 브로드캐스트 그룹 구성
...
브로드캐스트 그룹을 사용하면 여러 에이전트가 동일한 메시지를 동시에 처리하고 응답할 수 있습니다. 이를 통해 하나의 WhatsApp 그룹 또는 다이렉트 메시지에서, 단 하나의 전화번호만으로 협업하는 특화된 에이전트 팀을 구성할 수 있습니다.

현재 범위: **WhatsApp 전용** (웹 채널).

브로드캐스트 그룹은 채널 allowlist 및 그룹 활성화 규칙 이후에 평가됩니다. WhatsApp 그룹의 경우, 이는 OpenClaw 가 일반적으로 응답하는 시점(예: 그룹 설정에 따라 멘션 시점)에 브로드캐스트가 발생함을 의미합니다.
...
각 에이전트는 동일한 메시지를 처리하고, 자신의 전문 관점에서 응답을 제공합니다.
...
최상위에 `broadcast` 섹션을 추가합니다(`bindings` 옆). 키는 WhatsApp peer id 입니다:
...
**결과:** 이 채팅에서 OpenClaw 가 응답해야 할 때, 세 개의 에이전트가 모두 실행됩니다.
...
2. **브로드캐스트 확인**: 시스템이 peer ID 가 `broadcast` 에 포함되어 있는지 확인합니다
3. **브로드캐스트 목록에 있는 경우**:
   - 나열된 모든 에이전트가 메시지를 처리합니다
   - 각 에이전트는 자체 세션 키와 격리된 컨텍스트를 가집니다
   - 에이전트는 병렬(기본값) 또는 순차적으로 처리됩니다
4. **브로드캐스트 목록에 없는 경우**:
   - 일반 라우팅이 적용됩니다(첫 번째로 일치하는 바인딩)

참고: 브로드캐스트 그룹은 채널 allowlist 나 그룹 활성화 규칙(멘션, 명령 등)을 우회하지 않습니다. 메시지가 처리 대상이 될 때 _어떤 에이전트가 실행되는지_ 만 변경합니다.

### 세션 격리

브로드캐스트 그룹의 각 에이전트는 다음 항목을 완전히 분리하여 유지합니다:
...
- **도구 접근 권한** (서로 다른 허용/차단 목록)
- **메모리/컨텍스트** (분리된 IDENTITY.md, SOUL.md 등)
- **그룹 컨텍스트 버퍼** (컨텍스트로 사용되는 최근 그룹 메시지) 는 peer 단위로 공유되므로, 트리거 시 모든 브로드캐스트 에이전트가 동일한 컨텍스트를 봅니다

이를 통해 각 에이전트는 다음과 같은 차이를 가질 수 있습니다:
...
`120363403215116621@g.us` 그룹에 `["alfred", "baerbel"]` 에이전트가 있는 경우:
...
✅ **좋음:** 각 에이전트가 하나의 작업만 담당  
❌ **나쁨:** 하나의 범용 'dev-helper' 에이전트

### 2. 설명적인 이름 사용

각 에이전트가 무엇을 하는지 명확히 드러나도록 합니다:
...
**원인:** Peer ID 가 `bindings` 에는 있으나 `broadcast` 에는 없을 수 있습니다.
...
1. **최대 에이전트 수:** 하드 제한은 없으나, 10 개 이상의 에이전트는 느릴 수 있습니다
2. **공유 컨텍스트:** 에이전트는 서로의 응답을 보지 못합니다(설계상 의도됨)
3. **메시지 순서:** 병렬 응답은 어떤 순서로든 도착할 수 있습니다
4. **요율 제한:** 모든 에이전트는 WhatsApp 요율 제한에 포함됩니다
