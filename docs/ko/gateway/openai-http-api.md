...
read_when:
  - OpenAI Chat Completions 을 기대하는 도구 통합 시
...
x-i18n:
  ...
  model: gpt-5.2-pro
  workflow: v1
  generated_at: 2026-02-06T05:38:50Z
---
...
내부적으로 요청은 일반적인 Gateway(게이트웨이) 에이전트 실행으로 수행됩니다(`openclaw agent` 과 동일한 코드 경로). 따라서 라우팅/권한/설정이 Gateway(게이트웨이)와 일치합니다.
...
`gateway.http.endpoints.chatCompletions.enabled` 를 `true` 으로 설정하십시오:
...
`gateway.http.endpoints.chatCompletions.enabled` 를 `false` 으로 설정하십시오:
...
기본적으로 엔드포인트는 **요청별로 무상태**입니다(호출마다 새로운 세션 키가 생성됩니다).

요청에 OpenAI `user` 문자열이 포함되어 있으면, Gateway(게이트웨이)는 이를 기반으로 안정적인 세션 키를 파생하므로 반복 호출이 에이전트 세션을 공유할 수 있습니다.
