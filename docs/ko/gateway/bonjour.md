---
summary: "Bonjour/mDNS 디바이스 검색 + 디버깅 (Gateway 비콘, 클라이언트, 그리고 일반적인 실패 모드)"
read_when:
...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T06:39:56Z
---

# Bonjour / mDNS 디바이스 검색

OpenClaw 는 활성 Gateway (WebSocket 엔드포인트)를 발견하기 위한 **LAN 전용 편의 기능**으로 Bonjour (mDNS / DNS‑SD)를 사용합니다. 이는 최선 노력 방식이며 SSH 또는 Tailnet 기반 연결을 **대체하지 않습니다**.

## Tailscale 을 통한 광역 Bonjour (유니캐스트 DNS‑SD)

노드와 Gateway 가 서로 다른 네트워크에 있는 경우, 멀티캐스트 mDNS 는 경계를 넘어가지 않습니다. **유니캐스트 DNS‑SD**("광역 Bonjour")로 전환하여 Tailscale 위에서 동일한 디바이스 검색 UX 를 유지할 수 있습니다.

개략적인 단계:

1. Gateway 호스트에서 DNS 서버를 실행합니다 (Tailnet 을 통해 접근 가능).
2. 전용 존 아래에 `_openclaw-gw._tcp` 에 대한 DNS‑SD 레코드를 게시합니다
   (예: `openclaw.internal.`).
3. 선택한 도메인이 해당 DNS 서버를 통해 클라이언트( iOS 포함 )에서 해석되도록
   Tailscale **분할 DNS**를 구성합니다.
...
- Gateway 의 tailnet IP (UDP/TCP 53)를 가리키는 네임서버를 추가합니다.
...
Gateway WS 포트(기본값 `18789`)는 기본적으로 loopback 에 바인딩됩니다. LAN/tailnet 접근을 위해서는 명시적으로 바인딩하고 인증을 활성화한 상태를 유지하십시오.

tailnet 전용 설정의 경우:

- `~/.openclaw/openclaw.json` 에서 `gateway.bind: "tailnet"` 를 설정합니다.
- Gateway 를 재시작합니다 (또는 macOS 메뉴바 앱을 재시작합니다).

## 광고 대상

Gateway 만이 `_openclaw-gw._tcp` 를 광고합니다.
...
Gateway 는 UI 흐름을 편리하게 만들기 위해 작은 비밀이 아닌 힌트를 광고합니다:
...
- `tailnetDns=<magicdns>` (Tailnet 이 사용 가능한 경우의 선택적 힌트)
...
- 설정 → Gateway → 고급 → **디바이스 검색 디버그 로그**
- 설정 → Gateway → 고급 → **디바이스 검색 로그** → 재현 → **복사**

로그에는 브라우저 상태 전이와 결과 집합 변경 사항이 포함됩니다.

## 일반적인 실패 모드

- **Bonjour 는 네트워크를 넘지 않습니다**: Tailnet 또는 SSH 를 사용하십시오.
- **멀티캐스트 차단**: 일부 Wi‑Fi 네트워크는 mDNS 를 비활성화합니다.
- **절전 / 인터페이스 변경**: macOS 는 일시적으로 mDNS 결과를 드롭할 수 있으므로 재시도하십시오.
- **탐색은 되지만 해석이 실패**: 머신 이름을 단순하게 유지하십시오(이모지나
  문장부호 회피). 그런 다음 Gateway 를 재시작하십시오. 서비스 인스턴스 이름은
  호스트 이름에서 파생되므로, 지나치게 복잡한 이름은 일부 리졸버를 혼란스럽게 할 수 있습니다.

## 이스케이프된 인스턴스 이름 (`\032`)

Bonjour/DNS‑SD 는 종종 서비스 인스턴스 이름의 바이트를 10진수 `\DDD`
시퀀스로 이스케이프합니다 (예: 공백은 `\032` 로 변환).

- 이는 프로토콜 수준에서 정상 동작입니다.
- UI 는 표시를 위해 디코딩해야 합니다 (iOS 는 `BonjourEscapes.decode` 를 사용).

## 비활성화 / 구성

- `OPENCLAW_DISABLE_BONJOUR=1` 는 광고를 비활성화합니다 (레거시: `OPENCLAW_DISABLE_BONJOUR`).
- `~/.openclaw/openclaw.json` 의 `gateway.bind` 는 Gateway 바인딩 모드를 제어합니다.
- `OPENCLAW_SSH_PORT` 는 TXT 에 광고되는 SSH 포트를 재정의합니다 (레거시: `OPENCLAW_SSH_PORT`).
