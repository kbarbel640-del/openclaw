...
read_when:
  - 게이트웨이 프로토콜, 클라이언트 또는 전송 작업을 수행할 때
title: "Gateway 아키텍처"
x-i18n:
...
- 단일의 장시간 실행 **Gateway(게이트웨이)** 가 모든 메시징 표면을 소유합니다(WhatsApp 은
  Baileys 경유, Telegram 은 grammY 경유, Slack, Discord, Signal, iMessage, WebChat).
- 컨트롤 플레인 클라이언트(macOS 앱, CLI, 웹 UI, 자동화)는 구성된 바인드 호스트(기본값
  `127.0.0.1:18789`)에서 **WebSocket** 을 통해 Gateway(게이트웨이)에 연결합니다.
- **Nodes**(macOS/iOS/Android/headless)도 **WebSocket** 을 통해 연결하지만,
  명시적 caps/commands 와 함께 `role: node` 을 선언합니다.
- 호스트당 하나의 Gateway(게이트웨이); WhatsApp 세션을 여는 유일한 위치입니다.
- **canvas host**(기본값 `18793`)는 에이전트가 편집 가능한 HTML 과 A2UI 를 제공합니다.
...
- 수신 프레임을 JSON Schema 에 대해 검증합니다.
...
- `connect` 에서 디바이스 아이덴티티를 제공하며; 페어링은 **디바이스 기반**(역할 `node`)이고
...
- 부작용이 있는 메서드(`send`, `agent`)에는 안전한 재시도를 위해
  멱등성 키가 필요합니다; 서버는 단기 수명의 중복 제거 캐시를 유지합니다.
- Nodes 는 `role: "node"` 과 함께 `connect` 에 caps/commands/permissions 를 포함해야 합니다.

## 페어링 + 로컬 신뢰

- 모든 WS 클라이언트(운영자 + nodes)는 `connect` 에 **디바이스 아이덴티티** 를 포함합니다.
- 새로운 디바이스 ID 는 페어링 승인이 필요하며; Gateway(게이트웨이)는 이후 연결을 위해 **디바이스 토큰** 을 발급합니다.
...
자세한 내용: [Gateway protocol](/gateway/protocol), [Pairing](/start/pairing),
...
- 원격 설정에서는 WS 에 대해 TLS + 선택적 핀닝을 활성화할 수 있습니다.

## 운영 스냅샷

- 시작: `openclaw gateway` (포그라운드, stdout 으로 로그 출력).
- 헬스: WS 를 통한 `health` (또한 `hello-ok` 에 포함).
...
- 이벤트는 재생되지 않으므로; 클라이언트는 누락이 발생하면 새로고침해야 합니다.
