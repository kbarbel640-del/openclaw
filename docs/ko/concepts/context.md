...
'컨텍스트(Context)'는 **한 번의 실행에서 OpenClaw 가 모델로 보내는 모든 것**입니다. 이는 모델의 **컨텍스트 윈도우**(토큰 한도)에 의해 제한됩니다.

초보자를 위한 개념 모델:

- **시스템 프롬프트**(OpenClaw 가 생성): 규칙, 도구, Skills 목록, 시간/런타임, 그리고 주입된 워크스페이스 파일.
- **대화 기록**: 이 세션에서의 사용자 메시지 + 어시스턴트 메시지.
- **도구 호출/결과 + 첨부물**: 명령 출력, 파일 읽기, 이미지/오디오 등.

컨텍스트는 '메모리'와 _같은 것이 아닙니다_: 메모리는 디스크에 저장되어 나중에 다시 불러올 수 있지만, 컨텍스트는 모델의 현재 윈도우 안에 있는 것입니다.
...
- `/usage tokens` → 일반 답변에 응답별 사용량 푸터를 추가.
- `/compact` → 오래된 기록을 요약해 컴팩트한 항목으로 만들어 윈도우 공간을 확보.

참고: [Slash commands](/tools/slash-commands), [Token use & costs](/token-use), [Compaction](/concepts/compaction).

## 예시 출력

값은 모델, 프로바이더, 도구 정책, 그리고 워크스페이스에 무엇이 있는지에 따라 달라집니다.
...
- 첨부물/전사본(이미지/오디오/파일).
- 컴팩션 요약 및 프루닝 산출물.
- 프로바이더의 '래퍼' 또는 숨겨진 헤더(보이지 않지만 계산됨).
...
기본적으로 OpenClaw 는 다음과 같은 고정된 워크스페이스 파일 세트를 주입합니다(존재하는 경우):
...
큰 파일은 `agents.defaults.bootstrapMaxChars` 를 사용해 파일별로 잘립니다(기본값 `20000` 자). `/context` 는 **원본 vs 주입된** 크기와 잘림 여부를 보여줍니다.

## Skills: 주입되는 것 vs 필요 시 로드되는 것

시스템 프롬프트에는 간결한 **skills 목록**(이름 + 설명 + 위치)이 포함됩니다. 이 목록에는 실제 오버헤드가 있습니다.

스킬 지침은 기본적으로 포함되지 않습니다. 모델은 필요할 때만 해당 스킬의 `SKILL.md` 을 `read` 할 것으로 기대됩니다.
...
2. **도구 스키마**(JSON). 모델이 도구를 호출할 수 있도록 전송되며, 일반 텍스트로 보이지 않더라도 컨텍스트에 포함됩니다.

`/context detail` 는 가장 큰 도구 스키마를 분해해 무엇이 지배적인지 볼 수 있게 해줍니다.

## 명령, 지시문, 그리고 '인라인 단축키'

슬래시 명령은 Gateway(게이트웨이) 에서 처리됩니다. 몇 가지 서로 다른 동작이 있습니다:

- **단독 명령**: 메시지가 오직 `/...` 인 경우 명령으로 실행됩니다.
...
- **일반 기록**은 정책에 의해 컴팩트/프루닝될 때까지 세션 전사본에 유지됩니다.
...
`/context` 는 가능할 때 최신 **실행-빌드(run-built)** 시스템 프롬프트 보고서를 선호합니다:

- `System prompt (run)` = 마지막 임베디드(도구 사용 가능) 실행에서 캡처되어 세션 스토어에 유지됨.
- `System prompt (estimate)` = 실행 보고서가 없을 때(또는 보고서를 생성하지 않는 CLI 백엔드를 통해 실행할 때) 즉시 계산됨.

어느 경우든 크기와 상위 기여 항목을 보고하며, 전체 시스템 프롬프트나 도구 스키마를 덤프하지는 않습니다.
