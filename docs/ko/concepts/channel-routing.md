...
OpenClaw 는 메시지가 들어온 **해당 채널로 다시 응답을 라우팅**합니다.  
모델이 채널을 선택하지는 않으며, 라우팅은 결정적이고 호스트 설정에 의해 제어됩니다.
...
- Slack/Discord 스레드는 기본 키에 `:thread:<threadId>` 을(를) 추가합니다.
- Telegram 포럼 토픽은 그룹 키에 `:topic:<topicId>` 을(를) 포함합니다.
...
2. **길드 일치** (Discord) — `guildId` 을(를) 통해.
3. **팀 일치** (Slack) — `teamId` 을(를) 통해.
4. **계정 일치** (채널의 `accountId`).
5. **채널 일치** (해당 채널의 모든 계정).
6. **기본 에이전트** (`agents.list[].default`, 그렇지 않으면 목록의 첫 번째 항목, 최종적으로 `main` 로 폴백).

일치된 에이전트가 사용될 작업 공간과 세션 저장소를 결정합니다.
...
참고: [Broadcast Groups](/broadcast-groups).
...
WebChat 은 **선택된 에이전트**에 연결되며, 기본적으로 해당 에이전트의 메인 세션을 사용합니다.  
이로 인해 WebChat 에서는 해당 에이전트의 채널 간 컨텍스트를 한 곳에서 확인할 수 있습니다.
