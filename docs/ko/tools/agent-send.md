...
기본적으로 **Gateway(게이트웨이)** 를 **통과**하며, `--local`를 추가하면 현재 머신의
내장 런타임을 강제로 사용합니다.
...
- 세션 선택:
  ...
  - `--session-id <id>`이 id로 기존 세션을 재사용합니다, **또는**
  - `--agent <id>`이 구성된 에이전트를 직접 대상으로 지정합니다(해당 에이전트의 `main` 세션 키 사용)
- 일반 인바운드 응답과 동일한 내장 에이전트 런타임을 실행합니다.
- 사고/상세 플래그는 세션 스토어에 유지됩니다.
- 출력:
  - 기본값: 응답 텍스트를 출력합니다(`MEDIA:<url>` 줄 포함)
  - `--json`: 구조화된 페이로드 + 메타데이터를 출력합니다
- `--deliver` + `--channel`로 채널로의 선택적 전달(`openclaw message --target`와 대상 형식이 일치).
- 세션을 변경하지 않고 전달을 재정의하려면 `--reply-channel`/`--reply-to`/`--reply-account`를 사용합니다.

Gateway(게이트웨이)에 연결할 수 없는 경우, CLI는 내장 로컬 실행으로 **폴백**합니다.
...
- `--local`: 로컬에서 실행(셸에 모델 프로바이더 API 키가 필요)
...
- `--thinking <off|minimal|low|medium|high|xhigh>`: 사고 수준 유지(GPT-5.2 + Codex 모델 전용)
- `--verbose <on|full|off>`: 상세 수준 유지
