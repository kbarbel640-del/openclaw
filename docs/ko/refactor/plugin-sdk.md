...
목표: 모든 메시징 커넥터는 하나의 안정적인 API 를 사용하는 플러그인(번들 또는 외부)입니다.
어떤 플러그인도 `src/**` 를 직접 임포트하지 않습니다. 모든 의존성은 SDK 또는 런타임을 통해 접근합니다.

## 지금 필요한 이유

- 현재 커넥터는 패턴이 혼재되어 있습니다: 코어 직접 임포트, dist 전용 브리지, 커스텀 헬퍼.
- 이로 인해 업그레이드가 취약해지고, 깔끔한 외부 플러그인 표면을 제공하지 못합니다.
...
- `openclaw/plugin-sdk` 로 퍼블리시(또는 `openclaw/plugin-sdk` 아래에서 코어로부터 export).
- 명시적인 안정성 보장을 포함한 semver 적용.

### 2) 플러그인 런타임 (실행 표면, 주입 방식)

범위: 코어 런타임 동작에 닿는 모든 것.
플러그인은 `OpenClawPluginApi.runtime` 를 통해 접근하므로 `src/**` 를 직접 임포트하지 않습니다.
...
- 전환 기간 동안 기존 임포트 유지(디프리케이션 경고).

### Phase 1: 브리지 정리 (저위험)

- 확장별 `core-bridge.ts` 를 `api.runtime` 로 교체.
- BlueBubbles, Zalo, Zalo Personal 을 먼저 마이그레이션(이미 근접).
- 중복된 브리지 코드 제거.

### Phase 2: 경량 직접 임포트 플러그인

- Matrix 를 SDK + 런타임으로 마이그레이션.
- 온보딩, 디렉토리, 그룹 멘션 로직 검증.

### Phase 3: 대규모 직접 임포트 플러그인

- MS Teams 마이그레이션(가장 큰 런타임 헬퍼 집합).
- 답장/타이핑 시맨틱이 현재 동작과 일치하는지 확인.

### Phase 4: iMessage 플러그인화

- iMessage 를 `extensions/imessage` 로 이동.
- 직접 코어 호출을 `api.runtime` 로 대체.
...
- 플러그인은 요구되는 런타임 범위를 선언(예: `openclawRuntime: ">=2026.2.0"`).

## 테스트 전략

- 어댑터 수준 단위 테스트(실제 코어 구현으로 런타임 함수 실행).
- 플러그인별 골든 테스트: 동작 드리프트 방지(라우팅, 페어링, 허용 목록, 멘션 게이팅).
- CI 에서 사용하는 단일 엔드투엔드 플러그인 샘플(설치 + 실행 + 스모크).

## 열린 질문

- SDK 타입을 어디에 호스팅할 것인가: 별도 패키지 vs 코어 export?
- 런타임 타입 배포: SDK(타입만) vs 코어?
- 번들 플러그인과 외부 플러그인을 위한 문서 링크를 어떻게 노출할 것인가?
- 전환 기간 동안 리포지토리 내 플러그인에 대해 제한적인 코어 직접 임포트를 허용할 것인가?

## 성공 기준

- 모든 채널 커넥터가 SDK + 런타임을 사용하는 플러그인입니다.
- `src/**` 에서 `extensions/**` 임포트가 없습니다.
- 신규 커넥터 템플릿은 SDK + 런타임에만 의존합니다.
- 외부 플러그인은 코어 소스 접근 없이 개발 및 업데이트가 가능합니다.

관련 문서: [Plugins](/plugin), [Channels](/channels/index), [Configuration](/gateway/configuration).
