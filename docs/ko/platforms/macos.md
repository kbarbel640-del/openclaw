---
summary: "OpenClaw macOS 컴패니언 앱(메뉴 막대 + 게이트웨이 브로커)"
read_when:
  - macOS 앱 기능 구현 시
  - macOS 에서 게이트웨이 라이프사이클 또는 노드 브리징 변경 시
title: "macOS 앱"
x-i18n:
...
# OpenClaw macOS 컴패니언(메뉴 막대 + 게이트웨이 브로커)

macOS 앱은 OpenClaw 의 **메뉴 막대 컴패니언**입니다. 권한을 소유하고,
로컬에서 Gateway(게이트웨이)에 연결/관리(launchd 또는 수동)하며, macOS
기능을 노드로서 에이전트에 노출합니다.

## 기능

- 메뉴 막대에 네이티브 알림과 상태를 표시합니다.
- TCC 프롬프트(알림, 접근성, 화면 기록, 마이크,
...
- 요청 시 npm/pnpm 을 통해 전역 CLI(`openclaw`)를 설치합니다(Gateway 런타임에는 bun 을 권장하지 않음).

## 로컬 모드 vs 원격 모드

- **로컬**(기본값): 실행 중인 로컬 Gateway(게이트웨이)가 있으면 앱이 연결하며,
  없으면 `openclaw gateway install` 를 통해 launchd 서비스를 활성화합니다.
- **원격**: 앱은 SSH/Tailscale 을 통해 Gateway(게이트웨이)에 연결하며 로컬 프로세스를 시작하지 않습니다.
  원격 Gateway(게이트웨이)가 이 Mac 에 접근할 수 있도록 로컬 **노드 호스트 서비스**를 시작합니다.
...
이름이 지정된 프로필을 실행할 때는 레이블을 `bot.molt.<profile>` 로 교체합니다.

LaunchAgent 가 설치되어 있지 않으면 앱에서 활성화하거나
`openclaw gateway install` 를 실행합니다.

## 노드 기능(mac)

macOS 앱은 노드로 동작합니다. 일반적인 명령은 다음과 같습니다.
...
- `system.run` 는 로컬 Unix 소켓을 통해 macOS 앱(UI/TCC 컨텍스트)에서 실행되며, 프롬프트와 출력은 앱 내부에 유지됩니다.
...
보안 + 요청 + 허용 목록은 Mac 로컬의 다음 위치에 저장됩니다.
...
- `system.run` 환경 변수 오버라이드는 필터링(`PATH`, `DYLD_*`, `LD_*`, `NODE_OPTIONS`, `PYTHON*`, `PERL*`, `RUBYOPT` 제거)된 뒤 앱의 환경과 병합됩니다.
...
- 유효한 `key` 이 있으면 무인 실행됩니다(개인 자동화를 의도).
...
앱을 실행하지 않고도 macOS 앱이 사용하는 것과 동일한 Gateway(게이트웨이)
WebSocket 핸드셰이크와 디바이스 검색 로직을 검증하려면 디버그 CLI 를 사용합니다.
...
macOS 앱이 **원격** 모드로 실행될 때, 로컬 UI 컴포넌트가 원격 Gateway(게이트웨이)를
localhost 에 있는 것처럼 통신할 수 있도록 SSH 터널을 엽니다.

### 제어 터널(Gateway WebSocket 포트)

- **목적:** 상태 확인, 상태 표시, 웹 채팅, 설정 및 기타 제어 플레인 호출.
...
- **IP 보고:** SSH 터널은 loopback 을 사용하므로, 게이트웨이는 노드 IP 를 `127.0.0.1` 로 인식합니다. 실제 클라이언트 IP 가 표시되길 원하면 **Direct (ws/wss)** 전송을 사용하십시오([macOS 원격 접근](/platforms/mac/remote) 참고).

설정 단계는 [macOS 원격 접근](/platforms/mac/remote)을, 프로토콜 상세는 [Gateway 프로토콜](/gateway/protocol)을 참조하십시오.
