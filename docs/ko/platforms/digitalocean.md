...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T06:39:59Z
---
...
DigitalOcean에서 **월 $6** (또는 예약 요금 사용 시 월 $4)로 지속 실행되는 OpenClaw Gateway(게이트웨이)를 운영합니다.

월 $0 옵션을 원하고 ARM + 프로바이더별 설정을 감수할 수 있다면 [Oracle Cloud 가이드](/platforms/oracle)를 참고하세요.
...
Gateway(게이트웨이)는 기본적으로 loopback에 바인딩됩니다. Control UI에 접근하려면 다음 중 하나를 사용하세요:
...
- Serve는 Gateway(게이트웨이)를 loopback 전용으로 유지하고 Tailscale 아이덴티티 헤더로 인증합니다.
- 대신 토큰/비밀번호를 요구하려면 `gateway.auth.allowTailscale: false`을 설정하거나 `gateway.auth.mode: "password"`을 사용하세요.
...
다른 프로바이더는 [Channels](/channels)를 참고하세요.

---

## 1GB RAM 최적화

$6 Droplet은 1GB RAM만 제공합니다. 원활한 실행을 위해 다음을 권장합니다:
...
Oracle Cloud는 여기의 어떤 유료 옵션보다도 훨씬 강력한 **Always Free** ARM 인스턴스를 월 $0로 제공합니다.
...
- ARM 아키텍처 — 대부분 작동하지만 일부 바이너리는 ARM 빌드가 필요합니다

전체 설정 가이드는 [Oracle Cloud](/platforms/oracle)를 참고하세요. 가입 팁과 등록 과정 문제 해결은 이 [커뮤니티 가이드](https://gist.github.com/rssnyder/51e3cfedd730e7dd5f4a816143b25dbd)를 참고하세요.
