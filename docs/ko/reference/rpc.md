---
summary: "외부 CLI(signal-cli, 레거시 imsg)를 위한 RPC 어댑터와 게이트웨이 패턴"
read_when:
...
- `signal-cli` 는 HTTP 상의 JSON-RPC 를 사용하는 데몬으로 실행됩니다.
- 이벤트 스트림은 SSE 입니다 (`/api/v1/events`).
...
> **참고:** 새로운 iMessage 설정에는 [BlueBubbles](/channels/bluebubbles)를 대신 사용하십시오.

- OpenClaw 가 `imsg rpc` 를 자식 프로세스로 생성합니다 (레거시 iMessage 통합).
- JSON-RPC 는 stdin/stdout 을 통해 라인 단위로 전달됩니다 (라인당 하나의 JSON 객체).
...
레거시 설정과 주소 지정은 [iMessage](/channels/imessage)를 참조하십시오 (`chat_id` 권장).
