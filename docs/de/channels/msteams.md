---
summary: "Supportstatus, Funktionen und Konfiguration für Microsoft-Teams-Bots"
read_when:
  - Arbeit an MS-Teams-Kanal-Funktionen
...
Status: Text + DM-Anhänge werden unterstützt; Dateisenden in Kanälen/Gruppen erfordert `sharePointSiteId` + Graph-Berechtigungen (siehe [Dateien in Gruppenchats senden](#dateien-in-gruppenchats-senden)). Umfragen werden über Adaptive Cards gesendet.

## Plugin erforderlich

Microsoft Teams wird als Plugin ausgeliefert und ist nicht Teil der Kerninstallation.
...
Wenn Sie Teams während der Konfiguration/Einführung auswählen und ein Git-Checkout erkannt wird,
...
- Deterministische Weiterleitung: Antworten gehen immer an den Kanal zurück, aus dem sie stammen.
...
- `channels.msteams.allowFrom` akzeptiert AAD-Objekt-IDs, UPNs oder Anzeigenamen. Der Assistent löst Namen bei entsprechenden Berechtigungen über Microsoft Graph zu IDs auf.
...
- Beim Start löst OpenClaw Team-/Kanal- und Benutzer-Allowlist-Namen zu IDs auf (wenn Graph-Berechtigungen vorliegen)
...
3. Erstellen Sie ein **Teams-App-Paket**, das den Bot referenziert und die unten aufgeführten RSC-Berechtigungen enthält.
4. Laden/installieren Sie die Teams-App in ein Team (oder den persönlichen Bereich für DMs).
...
2. Füllen Sie den Tab **Basics** aus:
   ...
   | **Pricing tier**   | **Free** für Entwicklung/Test                                |
   | **Type of App**    | **Single Tenant** (empfohlen – siehe Hinweis unten)          |
   | **Creation type**  | **Create new Microsoft App ID**                              |

> **Hinweis zur Abkündigung:** Die Erstellung neuer Multi-Tenant-Bots wurde nach dem 2025-07-31 abgekündigt. Verwenden Sie **Single Tenant** für neue Bots.
...
4. Unter **Certificates & secrets** → **New client secret** → kopieren Sie den **Value** → dies ist Ihre `appPassword`
5. Gehen Sie zu **Overview** → kopieren Sie die **Directory (tenant) ID** → dies ist Ihre `tenantId`
...
2. Setzen Sie **Messaging endpoint** auf Ihre Webhook-URL:
   ...
   - Lokale Entwicklung: Verwenden Sie einen Tunnel (siehe [Lokale Entwicklung](#lokale-entwicklung-tunneling) unten)
...
3. Dies bestätigt, dass Ihr Webhook-Endpunkt funktioniert, bevor Sie Teams einrichten
...
3. **Teams-App-Manifest**
   ...
   - Fügen Sie RSC-Berechtigungen hinzu (siehe unten).
   - Erstellen Sie Icons: `outline.png` (32×32) und `color.png` (192×192).
...
- DM-Verlauf kann mit `channels.msteams.dmHistoryLimit` (Benutzer-Turns) begrenzt werden. Benutzerbezogene Overrides: `channels.msteams.dms["<user_id>"].historyLimit`.
...
- `bots[].supportsFiles: true` ist für Dateihandling im persönlichen Bereich erforderlich.
- `authorization.permissions.resourceSpecific` muss Kanal-Lesen/-Senden enthalten, wenn Sie Kanal-Traffic wünschen.
...
3. **Neu zippen** Sie das Manifest mit Icons (`manifest.json`, `outline.png`, `color.png`)
4. Laden Sie das neue ZIP hoch:
   - **Option A (Teams Admin Center):** Teams Admin Center → Teams apps → Manage apps → App finden → Upload new version
   - **Option B (Sideload):** In Teams → Apps → Manage your apps → Upload a custom app
5. **Für Team-Kanäle:** Installieren Sie die App in jedem Team neu, damit neue Berechtigungen wirksam werden
...
- Lesen des Kanal-/Chat-Verlaufs über Graph.
...
| **Historische Nachrichten** | Nein               | Ja (Verlauf abfragbar)        |
| **Einrichtungsaufwand**     | Nur App-Manifest   | Admin-Zustimmung + Token-Flow |
| **Offline nutzbar**         | Nein (muss laufen) | Ja (Abfrage jederzeit)        |

**Kurzfassung:** RSC ist für Echtzeit-Zuhören; Graph API für historischen Zugriff. Um verpasste Nachrichten während Offline-Zeiten nachzuholen, benötigen Sie Graph API mit `ChannelMessage.Read.All` (erfordert Admin-Zustimmung).
...
1. In Entra ID (Azure AD) **App Registration** Microsoft Graph **Application-Berechtigungen** hinzufügen:
   - `ChannelMessage.Read.All` (Kanal-Anhänge + Verlauf)
   - `Chat.Read.All` oder `ChatMessage.Read.All` (Gruppenchats)
2. **Admin-Zustimmung erteilen** für den Tenant.
3. Die Teams-App-**Manifestversion erhöhen**, erneut hochladen und die **App in Teams neu installieren**.
...
Teams liefert Nachrichten über HTTP-Webhooks. Wenn die Verarbeitung zu lange dauert (z. B. langsame LLM-Antworten), können auftreten:

- Gateway-Timeouts
- Teams-Wiederholungen derselben Nachricht (Duplikate)
- Verlorene Antworten

OpenClaw begegnet dem, indem schnell geantwortet und Antworten proaktiv gesendet werden; sehr langsame Antworten können dennoch Probleme verursachen.
...
Zentrale Einstellungen (siehe `/gateway/configuration` für gemeinsame Kanal-Muster):
...
- `channels.msteams.allowFrom`: Allowlist für DMs (AAD-Objekt-IDs, UPNs oder Anzeigenamen). Der Assistent löst Namen während der Einrichtung zu IDs auf, wenn Graph-Zugriff verfügbar ist.
- `channels.msteams.textChunkLimit`: Ausgehende Text-Chunk-Größe.
- `channels.msteams.chunkMode`: `length` (Standard) oder `newline`, um vor der Längen-Chunking an Leerzeilen (Absatzgrenzen) zu trennen.
- `channels.msteams.mediaAllowHosts`: Allowlist für eingehende Anhang-Hosts (Standard: Microsoft/Teams-Domains).
- `channels.msteams.mediaAuthAllowHosts`: Allowlist zum Anhängen von Authorization-Headern bei Medien-Wiederholungen (Standard: Graph- + Bot-Framework-Hosts).
...
- `channels.msteams.teams.<teamId>.tools`: Standardmäßige Pro-Team-Tool-Policy-Overrides (`allow`/`deny`/`alsoAllow`), wenn ein Kanal-Override fehlt.
- `channels.msteams.teams.<teamId>.toolsBySender`: Standardmäßige Pro-Team-Pro-Absender-Tool-Policy-Overrides (`"*"` Wildcard unterstützt).
...
- `channels.msteams.teams.<teamId>.channels.<conversationId>.tools`: Pro-Kanal-Tool-Policy-Overrides (`allow`/`deny`/`alsoAllow`).
- `channels.msteams.teams.<teamId>.channels.<conversationId>.toolsBySender`: Pro-Kanal-Pro-Absender-Tool-Policy-Overrides (`"*"` Wildcard unterstützt).
...
- Sitzungs-Schlüssel folgen dem Standard-Agent-Format (siehe [/concepts/session](/concepts/session)):
...
| --------------------------- | --------------------------------------------------------------- | ------------------------ |
| **Posts** (klassisch)       | Nachrichten erscheinen als Karten mit darunterliegenden Threads | `thread` (Standard)      |
| **Threads** (Slack-ähnlich) | Nachrichten fließen linear, ähnlich wie Slack                   | `top-level`              |

**Das Problem:** Die Teams-API gibt nicht preis, welchen UI-Stil ein Kanal verwendet. Wenn Sie das falsche `replyStyle` nutzen:

- `thread` in einem Threads-Kanal → Antworten erscheinen unglücklich verschachtelt
- `top-level` in einem Posts-Kanal → Antworten erscheinen als separate Top-Level-Posts statt im Thread

**Lösung:** Konfigurieren Sie `replyStyle` pro Kanal entsprechend der Kanaleinstellung:
...
- **Kanäle/Gruppen:** Anhänge liegen im M365-Speicher (SharePoint/OneDrive). Das Webhook-Payload enthält nur einen HTML-Stub, nicht die eigentlichen Datei-Bytes. **Graph-API-Berechtigungen sind erforderlich**, um Kanal-Anhänge herunterzuladen.
...
Authorization-Header werden nur für Hosts in `channels.msteams.mediaAuthAllowHosts` angehängt (Standard: Graph- + Bot-Framework-Hosts). Halten Sie diese Liste strikt (vermeiden Sie mandantenübergreifende Suffixe).
...
| Kontext                 | Versandmethode                                      | Erforderliches Setup                                |
| ----------------------- | --------------------------------------------------- | --------------------------------------------------- |
| **DMs**                 | FileConsentCard → Nutzer akzeptiert → Bot lädt hoch | Funktioniert sofort                                 |
| **Gruppenchats/Kanäle** | Upload zu SharePoint → Freigabelink                 | Erfordert `sharePointSiteId` + Graph-Berechtigungen |
| **Bilder (beliebig)**   | Base64-kodiert inline                               | Funktioniert sofort                                 |

### Warum Gruppenchats SharePoint benötigen

Bots haben kein persönliches OneDrive-Laufwerk (der `/me/drive`-Graph-API-Endpunkt funktioniert nicht für Application-Identitäten). Um Dateien in Gruppenchats/Kanälen zu senden, lädt der Bot in eine **SharePoint-Site** hoch und erstellt einen Freigabelink.
...
1. **Graph-API-Berechtigungen hinzufügen** in Entra ID (Azure AD) → App Registration:
   ...
   - `Chat.Read.All` (Application) – optional, ermöglicht benutzerbezogene Freigabelinks
...
| --------------------------------------- | -------------------------------------------------- |
| `Sites.ReadWrite.All` nur               | Organisationsweite Freigabe (jeder im Org-Zugriff) |
| `Sites.ReadWrite.All` + `Chat.Read.All` | Benutzerbezogene Freigabe (nur Chat-Teilnehmer)    |

Benutzerbezogene Freigaben sind sicherer, da nur die Chat-Teilnehmer Zugriff haben. Fehlt die Berechtigung `Chat.Read.All`, fällt der Bot auf organisationsweite Freigabe zurück.
...
- Umfragen posten noch keine automatischen Ergebniszusammenfassungen (bei Bedarf die Store-Datei prüfen).

## Adaptive Cards (beliebig)

Senden Sie beliebiges Adaptive-Card-JSON an Teams-Benutzer oder -Konversationen mit dem `message`-Werkzeug oder per CLI.
...
Siehe [Adaptive-Cards-Dokumentation](https://adaptivecards.io/) für Schema und Beispiele. Details zum Zielformat finden Sie unten unter [Zielformate](#zielformate).
...
| Gruppe/Kanal (raw)  | `<conversation-id>`              | `19:abc123...@thread.tacv2` (falls `@thread` enthalten) |
...
Der Abfrageparameter `groupId` in Teams-URLs ist **NICHT** die Team-ID für die Konfiguration. Extrahieren Sie stattdessen die IDs aus dem URL-Pfad:
...
- **Ignorieren** Sie den Abfrageparameter `groupId`
...
| @Erwähnungen                  | Ja             | Falls Bot zugänglich         |
...
2. DMs nutzen – Benutzer können den Bot immer direkt anschreiben
...
- **Bilder werden in Kanälen nicht angezeigt:** Graph-Berechtigungen oder Admin-Zustimmung fehlen. Teams-App neu installieren und Teams vollständig beenden/neu öffnen.
...
- **Versionskonflikt (Teams zeigt altes Manifest):** App entfernen + erneut hinzufügen und Teams vollständig beenden, um zu aktualisieren.
- **401 Unauthorized vom Webhook:** Erwartet bei manuellen Tests ohne Azure-JWT – bedeutet, der Endpunkt ist erreichbar, aber Authentifizierung fehlgeschlagen. Verwenden Sie Azure Web Chat für korrekte Tests.

### Manifest-Upload-Fehler

- **„Icon file cannot be empty“:** Das Manifest referenziert Icon-Dateien mit 0 Bytes. Erstellen Sie gültige PNG-Icons (32×32 für `outline.png`, 192×192 für `color.png`).
...
- **„Something went wrong“ beim Upload:** Laden Sie stattdessen über https://admin.teams.microsoft.com hoch, öffnen Sie die Browser-DevTools (F12) → Network-Tab und prüfen Sie den Response-Body auf den tatsächlichen Fehler.
- **Sideload schlägt fehl:** Versuchen Sie „Upload an app to your org's app catalog“ statt „Upload a custom app“ – dies umgeht oft Sideload-Beschränkungen.

### RSC-Berechtigungen funktionieren nicht

1. Prüfen Sie, dass `webApplicationInfo.id` exakt Ihrer Bot-App-ID entspricht
...
- [Teams-Bot-Dateihandling](https://learn.microsoft.com/en-us/microsoftteams/platform/bots/how-to/bots-filesv4) (Kanal/Gruppe erfordert Graph)
