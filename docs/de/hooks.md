...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T07:04:57Z
---
...
- **Hooks** (diese Seite): werden innerhalb des Gateway ausgeführt, wenn Agent-Ereignisse ausgelöst werden, z. B. `/new`, `/reset`, `/stop` oder Lebenszyklusereignisse.
...
Hooks können auch in Plugins gebündelt werden; siehe [Plugins](/plugin#plugin-hooks).
...
- Folgeautomatisierungen auslösen, wenn eine Sitzung startet oder endet
...
Das Hooks-System ermöglicht Ihnen:

- Sitzungs­kontext im Memory zu speichern, wenn `/new` ausgeführt wird
- Alle Befehle zur Auditierung zu protokollieren
- Benutzerdefinierte Automatisierungen bei Agent-Lebenszyklusereignissen auszulösen
...
- **😈 soul-evil**: Tauscht injizierte `SOUL.md`-Inhalte während eines Purge-Fensters oder mit zufälliger Wahrscheinlichkeit gegen `SOUL_EVIL.md` aus
...
Während der Einführung (`openclaw onboard`) werden Sie aufgefordert, empfohlene Hooks zu aktivieren. Der Assistent erkennt automatisch geeignete Hooks und stellt sie zur Auswahl bereit.
...
2. **Verwaltete Hooks**: `~/.openclaw/hooks/` (vom Benutzer installiert, über Workspaces hinweg geteilt)
3. **Gebündelte Hooks**: `<openclaw>/dist/hooks/bundled/` (mit OpenClaw ausgeliefert)

Verzeichnisse für verwaltete Hooks können entweder ein **einzelner Hook** oder ein **Hook-Paket** (Paketverzeichnis) sein.
...
Hook-Pakete sind standardmäßige npm-Pakete, die einen oder mehrere Hooks über `openclaw.hooks` in
...
- **`events`**: Array der Ereignisse, auf die gehört werden soll (z. B. `["command:new", "command:reset"]`)
...
### Tool-Ergebnis-Hooks (Plugin-API)
...
Speichert den Sitzungs­kontext im Memory, wenn Sie `/new` ausführen.
...
1. Verwendet den Sitzungs­eintrag vor dem Reset, um das korrekte Transkript zu finden
...
Tauscht injizierte `SOUL.md`-Inhalte während eines Purge-Fensters oder mit zufälliger Wahrscheinlichkeit gegen `SOUL_EVIL.md` aus.
...
3. Sendet angeforderte ausgehende Nachrichten über das Message-Werkzeug
...
Hooks laufen während der Befehlsverarbeitung. Halten Sie sie schlank:
...
Kapseln Sie riskante Operationen stets ab:
...
Protokollieren Sie in Ihrem Handler, wenn er aufgerufen wird:
...
- **`src/hooks/workspace.ts`**: Verzeichnisscanning und Laden
...
Auf TypeScript-/Importfehler prüfen:
