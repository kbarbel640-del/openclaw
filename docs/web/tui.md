...
```bash
amigo tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `amigo tui --deliver`
...
- Check the Gateway logs: `amigo logs --follow`.
- Confirm the agent can run: `amigo status` and `amigo models status`.
...
- No agents in picker: check `amigo agents list` and your routing config.
