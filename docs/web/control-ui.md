...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/clawdbrain`)
...
If the page fails to load, start the Gateway first: `clawdbrain gateway`.
...
```bash
clawdbrain gateway --tailscale serve
```
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Clawdbrain
...
```bash
clawdbrain gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
Clawdbrain **blocks** Control UI connections without device identity.
...
```bash
CLAWDBRAIN_CONTROL_UI_BASE_PATH=/clawdbrain/ pnpm ui:build
```
