...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/amigo`)
...
If the page fails to load, start the Gateway first: `amigo gateway`.
...
you revoke it with `amigo devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.amigo/amigo.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Amigo
...
Amigo **blocks** Control UI connections without device identity.
