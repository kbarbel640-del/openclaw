...
Amigo has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
### Layer 2: Gateway + dev agent smoke (what “@amigo” actually does)
...
```bash
amigo models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
...
- Gemini API vs Gemini CLI:
  - API: Amigo calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: Amigo shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
- OpenRouter: `openrouter/...` (hundreds of models; use `amigo models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `amigo models list` / model selection.

- Profile store: `~/.amigo/credentials/` (preferred; what “profile keys” means in the tests)
...
- `OPENCLAW_CONFIG_DIR=...` (default: `~/.amigo`) mounted to `/home/node/.amigo`
- `OPENCLAW_WORKSPACE_DIR=...` (default: `~/.amigo/workspace`) mounted to `/home/node/.amigo/workspace`
