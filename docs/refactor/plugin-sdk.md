...
- Publish as `amigo/plugin-sdk` (or export from core under `amigo/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `AmigoPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AmigoConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AmigoConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AmigoConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AmigoConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AmigoConfig): string;
  };
};
```
...
- Introduce `amigo/plugin-sdk`.
- Add `api.runtime` to `AmigoPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `amigoRuntime: ">=2026.2.0"`).
