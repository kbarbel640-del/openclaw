...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.amigo/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.amigo/node.json` (node id + pairing token).
...
Path: `~/.amigo/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.amigo/exec-approvals.sock",
...
- Unix socket at `~/.amigo/exec-approvals.sock` (0600).
