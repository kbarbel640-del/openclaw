---
summary: "Arcade.dev plugin: authorized tools across hundreds of services with automatic OAuth"
read_when:
...
Connect OpenClaw to [Arcade.dev](https://arcade.dev) for **authorized tools** across hundreds of services (even more with `useApiTools: true`). Arcade handles OAuth automatically—no credential management required.
