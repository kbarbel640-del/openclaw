---
summary: "Gateway（ゲートウェイ）プロトコルの単一の信頼できる情報源としての TypeBox スキーマ"
read_when:
  - プロトコルスキーマまたはコード生成を更新するとき
title: "TypeBox"
x-i18n:
...
# プロトコルの単一の信頼できる情報源としての TypeBox

最終更新日: 2026-01-10

TypeBox は TypeScript ファーストのスキーマライブラリです。私たちはこれを使って **Gateway（ゲートウェイ）WebSocket プロトコル**（ハンドシェイク、リクエスト/レスポンス、サーバーイベント）を定義します。これらのスキーマは、**ランタイム検証**、**JSON Schema のエクスポート**、および macOS アプリ向けの **Swift コード生成** を駆動します。単一の信頼できる情報源であり、それ以外はすべて生成されます。

より高レベルのプロトコルの文脈が必要な場合は、まず
[Gateway architecture](/concepts/architecture) から始めてください。

## メンタルモデル（30 秒）

すべての Gateway（ゲートウェイ）WS メッセージは、次の 3 種類のフレームのいずれかです。
...
| --------- | --------------------------------------------------------- | -------------------------------------------- |
| Core      | `connect`, `health`, `status`                             | `connect` は最初である必要があります         |
...
| Nodes     | `node.list`, `node.invoke`, `node.pair.*`                 | Gateway（ゲートウェイ）WS + ノードアクション |
| Events    | `tick`, `presence`, `agent`, `chat`, `health`, `shutdown` | サーバープッシュ                             |

権威ある一覧は `src/gateway/server.ts`（`METHODS`、`EVENTS`）にあります。
...
- `pnpm protocol:gen:swift`
  - Swift の Gateway（ゲートウェイ）モデルを生成します
- `pnpm protocol:check`
...
- **メソッドサーフェス**: Gateway（ゲートウェイ）は、`hello-ok` でサポートされる `methods` と `events` を通知します。
...
例: 新しい `system.echo` リクエストを追加し、`{ ok: true, text }` を返すようにします。
...
`ProtocolSchemas` に両方を追加し、型をエクスポートします:
...
`src/gateway/server-methods.ts` で登録します（すでに `systemHandlers` をマージしています）。次に、`src/gateway/server.ts` の `METHODS` に `"system.echo"` を追加します。
...
Swift ジェネレーターは次を生成します。

- `req`、`res`、`event`、および `unknown` ケースを持つ `GatewayFrame` enum
...
- Swift モデルは未知のフレームタイプを保持し、古いクライアントを壊さないようにします。

## スキーマのパターンと規約

- ほとんどのオブジェクトは、厳密なペイロードのために `additionalProperties: false` を使用します。
- `NonEmptyString` は、ID およびメソッド/イベント名のデフォルトです。
- トップレベルの `GatewayFrame` は、`type` に対する **discriminator** を使用します。
- 副作用のあるメソッドは通常、params に `idempotencyKey` を必要とします
  （例: `send`、`poll`、`agent`、`chat.send`）。

## ライブスキーマ JSON

生成された JSON Schema はリポジトリ内の `dist/protocol.schema.json` にあります。公開されている raw ファイルは通常、次で利用できます。
