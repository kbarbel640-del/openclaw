...
Hooks は「何かが起きたとき」に実行される小さなスクリプトです。2 種類があります。

- **Hooks**（このページ）: `/new`、`/reset`、`/stop`、またはライフサイクルイベントなど、エージェントのイベントが発火したときに Gateway（ゲートウェイ）内で実行されます。
...
Hooks はプラグイン内にバンドルすることもできます。詳しくは [Plugins](/plugin#plugin-hooks) を参照してください。

一般的な用途:

- セッションをリセットしたときにメモリスナップショットを保存する
- トラブルシューティングやコンプライアンスのために、コマンドの監査証跡を保持する
- セッションの開始や終了時に後続の自動化をトリガーする
- イベント発火時にエージェントのワークスペースへファイルを書き込んだり、外部 API を呼び出したりする

小さな TypeScript 関数を書ければ、Hook を作成できます。Hooks は自動的に検出され、CLI を使って有効化または無効化します。

## Overview

Hooks システムでは、次のことが可能です。

- `/new` が発行されたときにセッションコンテキストをメモリへ保存する
- すべてのコマンドをログに記録して監査に利用する
...
OpenClaw には、次の 4 つのバンドル済み Hook が同梱されており、自動的に検出されます。

- **💾 session-memory**: `/new` を実行した際に、セッションコンテキストをエージェントのワークスペース（デフォルトは `~/.openclaw/workspace/memory/`）へ保存します。
- **📝 command-logger**: すべてのコマンドイベントを `~/.openclaw/logs/commands.log` にログ出力します。
- **🚀 boot-md**: ゲートウェイ起動時に `BOOT.md` を実行します（内部フックを有効化する必要があります）。
- **😈 soul-evil**: パージウィンドウ中、またはランダムな確率で、注入された `SOUL.md` コンテンツを `SOUL_EVIL.md` に置き換えます。
...
オンボーディング（`openclaw onboard`）中に、推奨される Hook を有効化するかどうかを尋ねられます。ウィザードは対象となる Hook を自動的に検出し、選択肢として提示します。
...
Managed hook ディレクトリは、**単一の Hook** または **Hook パック**（パッケージディレクトリ）のいずれかです。

各 Hook は、次を含むディレクトリです。
...
## Hook Packs（npm / アーカイブ）

Hook パックは標準的な npm パッケージで、`package.json` 内の `openclaw.hooks` を通じて 1 つ以上の Hook をエクスポートします。次のようにインストールします。
...
各エントリは、`HOOK.md` と `handler.ts`（または `index.ts`）を含む Hook ディレクトリを指します。Hook パックは依存関係を同梱でき、それらは `~/.openclaw/hooks/<id>` 配下にインストールされます。
...
`metadata.openclaw` オブジェクトは、次をサポートします。
...
- **`requires`**: オプションの要件
  ...
  - **`anyBins`**: これらのうち少なくとも 1 つが存在する必要があります
  - **`env`**: 必須の環境変数
  - **`config`**: 必須の設定パス（例: `["workspace.dir"]`）
  - **`os`**: 必須のプラットフォーム（例: `["darwin", "linux"]`）
- **`always`**: 適格性チェックをバイパス（boolean）
- **`install`**: インストール方法（バンドル Hook の場合: `[{"id":"bundled","kind":"bundled"}]`）
...
- **`agent:bootstrap`**: ワークスペースのブートストラップファイルが注入される前（Hook は `context.bootstrapFiles` を変更できます）

### Gateway Events

ゲートウェイ起動時にトリガーされます。

- **`gateway:startup`**: チャンネルが起動し、Hook がロードされた後

### Tool Result Hooks（Plugin API）

これらはイベントストリームリスナーではありません。プラグインが、OpenClaw に保存される前にツール結果を同期的に調整できます。

- **`tool_result_persist`**: セッションのトランスクリプトに書き込まれる前にツール結果を変換します。同期処理である必要があります。更新後のツール結果ペイロード、またはそのまま保持する場合は `undefined` を返してください。[Agent Loop](/concepts/agent-loop) を参照してください。
...
**Migration**: 新しい Hook には、検出ベースの新システムを使用してください。レガシーハンドラーは、ディレクトリベースの Hook の後にロードされます。
...
パージウィンドウ中、またはランダムな確率で、注入された `SOUL.md` コンテンツを `SOUL_EVIL.md` に置き換えます。

**Events**: `agent:bootstrap`

**Docs**: [SOUL Evil Hook](/hooks/soul-evil)

**Output**: ファイルは書き込まれません。置き換えはメモリ内のみで行われます。
...
これを実行するには、内部 Hook を有効化する必要があります。
...
ゲートウェイログを監視して、Hook の実行状況を確認します。
...
- **`src/gateway/server-startup.ts`**: ゲートウェイ起動時に Hook をロード
...
2. Hook を再ロードするため、ゲートウェイプロセスを再起動します。
