...
- group chats: group JID (e.g. `[redacted-email]`)
- DMs: E.164 phone number (e.g. `+15551234567`)

```json
{
  "broadcast": {
    "[redacted-email]": ["alfred", "baerbel", "assistant3"]
  }
}
```
...
```json
{
  ...
  "broadcast": {
    "strategy": "parallel",
    "[redacted-email]": ["code-reviewer", "security-auditor", "docs-generator"],
    "[redacted-email]": ["support-en", "support-de"],
...
In group `[redacted-email]` with agents `["alfred", "baerbel"]`:

**Alfred's context:**
```
Session: agent:alfred:whatsapp:group:[redacted-email]
...
**Bärbel's context:**
```
Session: agent:baerbel:whatsapp:group:[redacted-email]  
...
2. Peer ID format is correct (e.g., `[redacted-email]`)
