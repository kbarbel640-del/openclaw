...
- ❌ **Missing:** Test that the fallback URL has a _different_ timestamp on each call (verifying it's `Date.now()` not cached). (Very minor.)

### Could this break hot-reload?

**No.** Hot-reload/dev workflows that watch for file changes will modify files → mtime changes → cache busted correctly. The only change is that _unchanged_ files aren't needlessly re-imported.
