...
type ChannelDock = {
  /* lightweight subset of ChannelPlugin for shared code */
};
...
type TelegramProbe = {
  ok: boolean;
  bot?: { id?; username?; canReadAllGroupMessages? };
  webhook?: { url? };
};
```
...
type LineInboundContext = {
  /* message context from webhook event */
};
```
  ...
  ```json
  {
    "channels": {
      "modelByChannel": {
        ...
        "discord": {
          "guild-name/channel-name": "anthropic/claude-opus-4-6",
...
_End of analysis. Total files analyzed: ~451 across 8 modules._
