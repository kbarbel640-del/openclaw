...
read_when:
  - Bạn muốn tự động hóa theo sự kiện cho /new, /reset, /stop và các sự kiện vòng đời của agent
...
Hooks cung cấp một hệ thống mở rộng theo sự kiện để tự động hóa các hành động phản hồi lại các lệnh và sự kiện của agent. Hooks được tự động phát hiện từ các thư mục và có thể được quản lý qua các lệnh CLI, tương tự như cách Skills hoạt động trong OpenClaw.
...
- **Webhooks**: các webhook HTTP bên ngoài cho phép các hệ thống khác kích hoạt công việc trong OpenClaw. Xem [Webhook Hooks](/automation/webhook) hoặc dùng `openclaw webhooks` cho các lệnh trợ giúp Gmail.

Hooks cũng có thể được đóng gói bên trong plugin; xem [Plugins](/plugin#plugin-hooks).

Các trường hợp sử dụng phổ biến:

- Lưu snapshot bộ nhớ khi bạn reset một phiên
- Duy trì audit trail của các lệnh để xử lý sự cố hoặc tuân thủ
...
- Ghi log tất cả các lệnh để audit
...
OpenClaw đi kèm bốn hook được đóng gói sẵn và tự động được phát hiện:

- **💾 session-memory**: Lưu ngữ cảnh phiên vào workspace của agent (mặc định `~/.openclaw/workspace/memory/`) khi bạn phát hành `/new`
...
- **😈 soul-evil**: Hoán đổi nội dung `SOUL.md` được chèn với `SOUL_EVIL.md` trong một khoảng purge hoặc theo xác suất ngẫu nhiên
...
Trong quá trình Huong Dan Ban Dau (`openclaw onboard`), bạn sẽ được nhắc bật các hook được khuyến nghị. Trình hướng dẫn sẽ tự động phát hiện các hook đủ điều kiện và trình bày để bạn lựa chọn.
...
3. **Bundled hooks**: `<openclaw>/dist/hooks/bundled/` (được phát hành cùng OpenClaw)

Các thư mục managed hook có thể là **một hook đơn** hoặc **một hook pack** (thư mục gói).
...
Hook pack có thể kèm theo dependencies; chúng sẽ được cài đặt dưới `~/.openclaw/hooks/<id>`.
...
- **`requires`**: Các yêu cầu tùy chọn
  - **`bins`**: Các binary bắt buộc có trên PATH (ví dụ: `["git", "node"]`)
  - **`anyBins`**: Ít nhất một trong các binary này phải tồn tại
  - **`env`**: Các bien moi truong bắt buộc
...
Được kích hoạt khi các lệnh của agent được phát hành:
...
Các hook này không phải listener của event-stream; chúng cho phép plugin điều chỉnh đồng bộ kết quả tool trước khi OpenClaw lưu chúng.

- **`tool_result_persist`**: chuyển đổi kết quả tool trước khi chúng được ghi vào transcript của phiên. Phải là đồng bộ; trả về payload kết quả tool đã cập nhật hoặc `undefined` để giữ nguyên. Xem [Agent Loop](/concepts/agent-loop).

### Sự kiện Tương lai

Các loại sự kiện được lên kế hoạch:
...
**Di chuyển**: Sử dụng hệ thống dựa trên phát hiện mới cho các hook mới. Legacy handlers được tải sau các hook dựa trên thư mục.
...
1. Sử dụng mục phiên trước reset để xác định transcript chính xác
2. Trích xuất 15 dòng hội thoại cuối cùng
3. Dùng LLM để tạo slug tên file mang tính mô tả
4. Lưu metadata của phiên vào một file bộ nhớ theo ngày
...
Ghi log tất cả các sự kiện lệnh vào một file audit tập trung.
...
2. Ghi thêm vào file log ở định dạng JSONL
...
Hoán đổi nội dung `SOUL.md` được chèn với `SOUL_EVIL.md` trong một khoảng purge hoặc theo xác suất ngẫu nhiên.
...
Cần bật internal hooks để chạy hook này.
...
Trong handler của bạn, log khi nó được gọi:
...
Kiểm tra vì sao một hook không đủ điều kiện:
...
Kiểm thử các handler của bạn một cách độc lập:
...
- **`src/auto-reply/reply/commands-core.ts`**: Kích hoạt các sự kiện lệnh
...
3. Liệt kê tất cả các hook đã được phát hiện:
   ```bash
   openclaw hooks list
   ```
