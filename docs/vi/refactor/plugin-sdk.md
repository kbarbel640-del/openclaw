...
# Kế hoạch tái cấu trúc SDK Plugin + Runtime
...
- Các connector hiện tại trộn lẫn nhiều mô hình: import trực tiếp từ core, bridge chỉ có ở dist, và helper tùy chỉnh.
- Điều này làm việc nâng cấp trở nên mong manh và cản trở một bề mặt plugin bên ngoài gọn gàng.

## Kiến trúc mục tiêu (hai lớp)

### 1) SDK Plugin (thời điểm biên dịch, ổn định, có thể phát hành)

Phạm vi: kiểu dữ liệu, helper và tiện ích cấu hình. Không có trạng thái runtime, không có side effect.
...
- Helper onboarding: `promptChannelAccessConfig`, `addWildcardAllowFrom`, các kiểu onboarding.
- Helper tham số tool: `createActionGate`, `readStringParam`, `readNumberParam`, `readReactionParams`, `jsonResult`.
...
- Semver với các cam kết ổn định rõ ràng.
...
- Mỗi phương thức runtime ánh xạ tới một triển khai core hiện có (không nhân bản).

## Kế hoạch di chuyển (theo giai đoạn, an toàn)

### Giai đoạn 0: dựng khung

- Giới thiệu `openclaw/plugin-sdk`.
- Thêm `api.runtime` vào `OpenClawPluginApi` với bề mặt ở trên.
- Duy trì các import hiện có trong một khoảng thời gian chuyển tiếp (cảnh báo ngừng dùng).

### Giai đoạn 1: dọn dẹp bridge (rủi ro thấp)

- Thay thế `core-bridge.ts` theo từng extension bằng `api.runtime`.
- Di chuyển BlueBubbles, Zalo, Zalo Personal trước (đã gần hoàn chỉnh).
- Loại bỏ mã bridge trùng lặp.

### Giai đoạn 2: plugin có import trực tiếp nhẹ

- Di chuyển Matrix sang SDK + runtime.
- Xác thực logic onboarding, directory, và nhắc đề cập nhóm.

### Giai đoạn 3: plugin có import trực tiếp nặng
...
### Giai đoạn 5: thực thi bắt buộc

- Thêm rule lint / kiểm tra CI: không cho phép import `extensions/**` từ `src/**`.
- Thêm kiểm tra tương thích SDK/phiên bản plugin (runtime + SDK semver).

## Tương thích và phiên bản

- SDK: semver, được phát hành, thay đổi có tài liệu.
- Runtime: được phiên bản hóa theo mỗi bản phát hành core. Thêm `api.runtime.version`.
- Plugin khai báo dải runtime yêu cầu (ví dụ: `openclawRuntime: ">=2026.2.0"`).

## Chiến lược kiểm thử

- Unit test mức adapter (các hàm runtime được chạy với triển khai core thực).
- Golden test cho mỗi plugin: đảm bảo không trôi hành vi (định tuyến, ghép cặp, allowlist, chặn đề cập).
- Một plugin mẫu end-to-end duy nhất dùng trong CI (cài đặt + chạy + smoke).

## Câu hỏi mở

- Nên lưu trữ kiểu SDK ở đâu: gói riêng hay export từ core?
- Phân phối kiểu runtime: trong SDK (chỉ kiểu) hay trong core?
- Cách phơi bày liên kết tài liệu cho plugin đóng gói sẵn so với plugin bên ngoài?
...
- Không có import `extensions/**` từ `src/**`.
...
Tài liệu liên quan: [Plugins](/plugin), [Channels](/channels/index), [Configuration](/gateway/configuration).
