...
read_when:
  - Bạn muốn tinh chỉnh thông tin xác thực, thiết bị hoặc mặc định của tác tu theo cách tương tác
title: "cau hinh"
x-i18n:
...
Lời nhắc tương tác để thiết lập thông tin xác thực, thiết bị và các mặc định của tác tu.

Lưu ý: Phần **Model** hiện bao gồm lựa chọn nhiều cho
danh sách cho phép `agents.defaults.models` (những gì hiển thị trong `/model` và bộ chọn model).

Mẹo: `openclaw config` không kèm tiểu lệnh sẽ mở cùng một trình hướng dẫn. Dùng
...
- CLI cấu hình: [Config](/cli/config)
