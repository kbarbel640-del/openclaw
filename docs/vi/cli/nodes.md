---
summary: "Tham chiếu CLI cho `openclaw nodes` (list/status/approve/invoke, camera/canvas/screen)"
read_when:
...
`nodes list` in ra các bảng đang chờ/đã ghép đôi. Các hàng đã ghép đôi bao gồm thời gian kết nối gần nhất (Last Connect).
Dùng `--connected` để chỉ hiển thị các node đang kết nối. Dùng `--last-connected <duration>` để
...
- Dùng phê duyệt exec (`exec.approval.request`) trước khi gọi `system.run`.
...
- `--env <key=val>`: ghi đè env (có thể lặp lại).
...
- `--agent <id>`: phê duyệt/danh sách cho phép theo phạm vi tác tử (mặc định theo tác tử đã cấu hình).
