...
Văn bản được hỗ trợ ở mọi nơi; media và reaction khác nhau tùy theo kênh.
...
- [Discord](/channels/discord) — Discord Bot API + Gateway; hỗ trợ server, kênh và tin nhắn trực tiếp.
...
- [BlueBubbles](/channels/bluebubbles) — **Được khuyến nghị cho iMessage**; dùng REST API của máy chủ BlueBubbles macOS với hỗ trợ đầy đủ tính năng (chỉnh sửa, thu hồi, hiệu ứng, reaction, quản lý nhóm — chỉnh sửa hiện đang bị lỗi trên macOS 26 Tahoe).
...
- Hành vi trong nhóm khác nhau theo từng kênh; xem [Groups](/concepts/groups).
- Ghép cặp tin nhắn trực tiếp và allowlist được áp dụng để đảm bảo an toàn; xem [Security](/gateway/security).
- Nội bộ Telegram: [ghi chú grammY](/channels/grammy).
- Xử lý sự cố: [Channel troubleshooting](/channels/troubleshooting).
- Các nhà cung cấp mô hình được tài liệu hóa riêng; xem [Model Providers](/providers/models).
