---
summary: "Trạng thái hỗ trợ ứng dụng Google Chat, khả năng và cấu hình"
read_when:
...
Trạng thái: sẵn sàng cho tin nhắn trực tiếp + space thông qua webhook Google Chat API (chỉ HTTP).
...
3. Tạo và tải về **JSON Key**:
   - Trong danh sách service account, nhấp vào account vừa tạo.
   - Vào tab **Keys**.
   - Nhấp **Add Key** > **Create new key**.
   - Chọn **JSON** và nhấn **Create**.
4. Lưu file JSON đã tải về trên máy chủ gateway của bạn (ví dụ: `~/.openclaw/googlechat-service-account.json`).
5. Tạo một ứng dụng Google Chat trong [Google Cloud Console Chat Configuration](https://console.cloud.google.com/apis/api/chat.googleapis.com/hangouts-chat):
   ...
   - Trong **Triggers**, chọn **Use a common HTTP endpoint URL for all triggers** và đặt thành URL công khai của gateway kèm theo `/googlechat`.
...
6. **Bật trạng thái ứng dụng**:
   ...
   - Đổi trạng thái sang **Live - available to users**.
   - Nhấp **Save** lại lần nữa.
...
8. Đặt loại và giá trị webhook audience (khớp với cấu hình ứng dụng Chat).
...
Webhook Google Chat yêu cầu một endpoint HTTPS công khai. Vì lý do bảo mật, **chỉ mở đường dẫn `/googlechat` ra internet**. Giữ dashboard OpenClaw và các endpoint nhạy cảm khác trong mạng riêng của bạn.

### Tùy chọn A: Tailscale Funnel (Khuyến nghị)

Dùng Tailscale Serve cho dashboard riêng tư và Funnel cho đường dẫn webhook công khai. Cách này giữ `/` ở chế độ riêng tư trong khi chỉ mở `/googlechat`.

1. **Kiểm tra gateway đang bind vào địa chỉ nào:**
...
3. **Chỉ mở đường dẫn webhook ra công khai:**
...
4. **Cấp quyền cho node sử dụng Funnel:**
   Nếu được yêu cầu, truy cập URL ủy quyền hiển thị trong output để bật Funnel cho node này trong chính sách tailnet của bạn.
...
Dashboard riêng tư của bạn vẫn chỉ доступ trong tailnet:
`https://<node-name>.<tailnet>.ts.net:8443/`

Dùng URL công khai (không kèm `:8443`) trong cấu hình ứng dụng Google Chat.

> Lưu ý: Cấu hình này được giữ nguyên qua các lần khởi động lại. Để gỡ bỏ sau này, chạy `tailscale funnel reset` và `tailscale serve reset`.
...
3. Tin nhắn được định tuyến theo space:
   - Tin nhắn trực tiếp dùng khóa session `agent:<agentId>:googlechat:dm:<spaceId>`.
   - Space dùng khóa session `agent:<agentId>:googlechat:group:<spaceId>`.
4. Truy cập DM mặc định là ghép cặp. Người gửi chưa biết sẽ nhận mã ghép cặp; phê duyệt bằng:
   - `openclaw pairing approve googlechat <code>`
5. Space nhóm mặc định yêu cầu @-mention. Dùng `botUser` nếu phát hiện mention cần tên người dùng của ứng dụng.

## Đích (Targets)

Dùng các định danh sau cho phân phối và allowlist:
...
- Thông tin xác thực service account cũng có thể truyền inline bằng `serviceAccount` (chuỗi JSON).
- Đường dẫn webhook mặc định là `/googlechat` nếu `webhookPath` chưa được đặt.
- Reaction khả dụng thông qua công cụ `reactions` và `channels action` khi `actions.reactions` được bật.
- `typingIndicator` hỗ trợ `none`, `message` (mặc định), và `reaction` (reaction yêu cầu OAuth người dùng).
- File đính kèm được tải xuống thông qua Chat API và lưu trong media pipeline (kích thước bị giới hạn bởi `mediaMaxMb`).
...
Điều này có nghĩa là webhook handler chưa được đăng ký. Các nguyên nhân thường gặp:

1. **Chưa cấu hình kênh**: Phần `channels.googlechat` bị thiếu trong config của bạn. Kiểm tra bằng:
...
2. **Plugin chưa được bật**: Kiểm tra trạng thái plugin:
   ...
   Nếu hiển thị "disabled", hãy thêm `plugins.entries.googlechat.enabled: true` vào config.

3. **Chưa khởi động lại gateway**: Sau khi thêm config, khởi động lại gateway:
   ```bash
...
- Nếu không nhận được tin nhắn, hãy xác nhận URL webhook + đăng ký sự kiện của ứng dụng Chat.
- Nếu cơ chế chặn theo mention ngăn trả lời, đặt `botUser` thành user resource name của ứng dụng và xác minh `requireMention`.
- Dùng `openclaw logs --follow` khi gửi tin nhắn thử để xem request có tới gateway hay không.
