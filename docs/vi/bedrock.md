...
tin xác thực mặc định của AWS SDK**, không phải khóa API.
...
`bedrock:ListFoundationModels` và được lưu cache (mặc định: 1 giờ).
...
- `refreshInterval` tính bằng giây; đặt `0` để tắt cache.
...
1. Đảm bảo thông tin xác thực AWS có sẵn trên **gateway host**:
...
**Giải pháp tạm thời:** Đặt `AWS_PROFILE=default` để báo hiệu rằng thông tin xác thực AWS
đã sẵn sàng. Việc xác thực thực tế vẫn sử dụng vai trò instance thông qua IMDS.
...
- `bedrock:ListFoundationModels` (cho khám phá tự động)
...
- Nếu bạn ưu tiên luồng khóa được quản lý, bạn cũng có thể đặt một proxy tương
  thích OpenAI trước Bedrock và cấu hình nó như một nhà cung cấp OpenAI.
