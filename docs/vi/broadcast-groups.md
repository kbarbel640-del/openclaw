...
Broadcast Groups cho phép nhiều tác tử xử lý và phản hồi cùng một tin nhắn đồng thời. Điều này giúp bạn tạo các nhóm tác tử chuyên biệt cùng làm việc trong một nhóm WhatsApp hoặc tin nhắn trực tiếp (DM) — tất cả chỉ dùng một số điện thoại.

Phạm vi hiện tại: **chỉ WhatsApp** (kênh web).

Broadcast groups được đánh giá sau allowlist của kênh và các quy tắc kích hoạt nhóm. Trong các nhóm WhatsApp, điều này có nghĩa là broadcast xảy ra khi OpenClaw bình thường sẽ phản hồi (ví dụ: khi được mention, tùy theo cài đặt nhóm của bạn).
...
Mỗi tác tử xử lý cùng một tin nhắn và cung cấp góc nhìn chuyên môn của mình.
...
Thêm một mục cấp cao `broadcast` (bên cạnh `bindings`). Các khóa là peer id của WhatsApp:
...
Các tác tử xử lý theo thứ tự (mỗi tác tử chờ tác tử trước hoàn tất):
...
3. **Nếu có trong danh sách broadcast**:
   ...
   - Mỗi tác tử có khóa phiên riêng và ngữ cảnh cách ly
...
Lưu ý: broadcast groups không bỏ qua allowlist của kênh hoặc các quy tắc kích hoạt nhóm (mentions/lệnh/etc). Chúng chỉ thay đổi _tác tử nào chạy_ khi một tin nhắn đủ điều kiện được xử lý.

### Cách ly phiên

Mỗi tác tử trong một broadcast group duy trì các thành phần hoàn toàn tách biệt:
...
- **Bộ đệm ngữ cảnh nhóm** (các tin nhắn nhóm gần đây dùng làm ngữ cảnh) được chia sẻ theo peer, nên tất cả tác tử broadcast thấy cùng một ngữ cảnh khi được kích hoạt
...
Chỉ cấp cho tác tử những công cụ cần thiết:
...
- Dùng `"strategy": "parallel"` (mặc định) để có tốc độ
...
3. Tác tử không nằm trong danh sách deny
...
- `[peerId]`: JID nhóm WhatsApp, số E.164, hoặc peer ID khác
  - Giá trị: Mảng ID tác tử cần xử lý tin nhắn
...
- [ ] Chế độ ngữ cảnh dùng chung (các tác tử thấy phản hồi của nhau)
- [ ] Điều phối tác tử (các tác tử có thể ra tín hiệu cho nhau)
- [ ] Chọn tác tử động (chọn tác tử dựa trên nội dung tin nhắn)
- [ ] Ưu tiên tác tử (một số tác tử phản hồi trước các tác tử khác)

## Xem thêm

- [Cấu hình đa tác tử](/multi-agent-sandbox-tools)
- [Cấu hình định tuyến](/concepts/channel-routing)
