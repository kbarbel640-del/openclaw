...
Este doc é um guia de “como testamos”:
...
Pense nas suites como “realismo crescente” (e flakiness/custo crescentes):
...
- Expectativas:
  ...
  - Execuções live vão buscar `~/.profile` para obter chaves de API ausentes
  - Rotação de chaves Anthropic: defina `OPENCLAW_LIVE_ANTHROPIC_KEYS="sk-...,sk-..."` (ou `OPENCLAW_LIVE_ANTHROPIC_KEY=sk-...`) ou múltiplas vars `ANTHROPIC_API_KEY*`; os testes vão tentar novamente em rate limits
...
- Defina `OPENCLAW_LIVE_MODELS=modern` (ou `all`, alias moderno) para realmente rodar esta suite; caso contrário ela é pulada para manter `pnpm test:live` focado no gateway smoke
- Como selecionar modelos:
  ...
  - ou `OPENCLAW_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,..."` (allowlist por vírgulas)
- Como selecionar provedores:
  - `OPENCLAW_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (allowlist por vírgulas)
...
### Camada 2: Gateway + agente dev smoke (o que “@openclaw” realmente faz)
...
- Objetivo:
  ...
  - Iterar modelos-com-chaves e validar:
    ...
    - probes extras opcionais de ferramenta (exec+read)
    - caminhos de regressão do OpenAI (apenas tool-call → follow-up) continuam funcionando
- Detalhes dos probes (para explicar falhas rapidamente):
  - Probe `read`: o teste grava um arquivo nonce no workspace e pede ao agente para `read` e ecoar o nonce de volta.
  - Probe `exec+read`: o teste pede ao agente para `exec`-gravar um nonce em um arquivo temporário, depois `read` de volta.
...
- Como selecionar modelos:
  ...
  - Ou defina `OPENCLAW_LIVE_GATEWAY_MODELS="provider/model"` (ou lista por vírgulas) para restringir
- Como selecionar provedores (evite “OpenRouter tudo”):
  - `OPENCLAW_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (allowlist por vírgulas)
- Probes de ferramenta + imagem estão sempre ativos neste teste live:
  ...
  - Fluxo (alto nível):
    ...
    - O gateway parseia anexos em `images[]` (`src/gateway/server-methods/agent.ts` + `src/gateway/chat-attachments.ts`)
...
- Objetivo: verificar se o setup-token do Claude Code CLI (ou um perfil com setup-token colado) consegue completar um prompt Anthropic.
- Habilitar:
  - `pnpm test:live` (ou `OPENCLAW_LIVE_TEST=1` se invocar o Vitest diretamente)
  - `OPENCLAW_LIVE_SETUP_TOKEN=1`
- Fontes do token (escolha uma):
...
- Overrides (opcional):
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE=1` para enviar um anexo de imagem real (paths são injetados no prompt).
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` para passar paths de arquivos de imagem como args do CLI em vez de injeção no prompt.
...
- `OPENCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` para manter a config MCP do Claude Code CLI habilitada (o padrão desabilita a config MCP com um arquivo vazio temporário).
...
- Gemini API vs Gemini CLI:
  - API: o OpenClaw chama a API Gemini hospedada do Google via HTTP (auth por chave de API / perfil); é o que a maioria dos usuários quer dizer com “Gemini”.
  - CLI: o OpenClaw executa um binário local `gemini`; ele tem auth própria e pode se comportar de forma diferente (streaming/suporte a ferramentas/desalinhamento de versão).
...
Inclua pelo menos um modelo com suporte a imagem em `OPENCLAW_LIVE_GATEWAY_MODELS` (variantes Claude/Gemini/OpenAI com visão, etc.) para exercitar o probe de imagem.

### Agregadores / gateways alternativos

Se você tiver chaves habilitadas, também suportamos testes via:

- OpenRouter: `openrouter/...` (centenas de modelos; use `openclaw models scan` para encontrar candidatos com tool+image)
...
Dica: não tente hardcodear “todos os modelos” nos docs. A lista autoritativa é o que `discoverModels(...)` retorna na sua máquina + quaisquer chaves disponíveis.

## Credenciais (nunca commit)

Os testes live descobrem credenciais do mesmo jeito que a CLI. Implicações práticas:

- Se a CLI funciona, os testes live devem encontrar as mesmas chaves.
- Se um teste live disser “sem credenciais”, depure do mesmo jeito que você depuraria `openclaw models list` / seleção de modelo.

- Armazenamento de perfis: `~/.openclaw/credentials/` (preferido; é o que “chaves de perfil” significa nos testes)
- Config: `~/.openclaw/openclaw.json` (ou `OPENCLAW_CONFIG_PATH`)

Se você quiser depender de chaves de env (ex.: exportadas no seu `~/.profile`), execute testes locais após `source ~/.profile`, ou use os runners Docker abaixo (eles podem montar `~/.profile` no container).
...
## Runners Docker (checks opcionais de “funciona no Linux”)
...
- Plugins (load de extensão customizada + smoke de registry): `pnpm test:docker:plugins` (script: `scripts/e2e/plugins-docker.sh`)
...
- `OPENCLAW_PROFILE_FILE=...` (padrão: `~/.profile`) montado em `/home/node/.profile` e feito source antes de rodar os testes
...
- Assistente do gateway (WS `wizard.start`/`wizard.next`, grava config + auth imposta): `src/gateway/gateway.wizard.e2e.test.ts`
...
- Tool-calling mockado através do gateway real + loop do agente (`src/gateway/gateway.tool-calling.mock-openai.test.ts`).
...
- **Decisão:** quando skills estão listadas no prompt, o agente escolhe a skill certa (ou evita as irrelevantes)?
- **Conformidade:** o agente lê `SKILL.md` antes de usar e segue os passos/args exigidos?
- **Contratos de workflow:** cenários multi-turn que afirmam ordem de ferramentas, carryover de histórico de sessão e limites de sandbox.

Evals futuras devem permanecer determinísticas primeiro:

- Um runner de cenários usando provedores mockados para afirmar chamadas de ferramentas + ordem, leituras de arquivos de skill e wiring de sessão.
- Uma pequena suite de cenários focados em skills (usar vs evitar, gating, prompt injection).
- Evals live opcionais (opt-in, controladas por env) apenas depois que a suite segura para CI estiver pronta.

## Adicionando regressões (orientações)
