...
configuração/estado obsoletos, verifica a saúde e fornece etapas de reparo acionáveis.
...
- Atualização opcional pré-execução para instalações via git (apenas interativo).
...
- Resumo do status de Skills (elegíveis/ausentes/bloqueadas).
- Normalização de configuração para valores legados.
- Avisos de override do provedor OpenCode Zen (`models.providers.opencode`).
- Migração de estado legado em disco (sessions/diretório do agente/auth do WhatsApp).
...
- Saúde de autenticação de modelos: verifica expiração de OAuth, pode renovar tokens prestes a expirar e relata estados de cooldown/desativado do perfil de autenticação.
...
- Verificações de instalação por source (incompatibilidade de workspace pnpm, ativos de UI ausentes, binário tsx ausente).
...
sem um override específico por canal), o doctor os normaliza para o esquema atual.
...
- Regravar `~/.openclaw/openclaw.json` com o esquema atualizado.
...
### 2b) Overrides de provedor OpenCode Zen
...
forçar todos os modelos para uma única API ou zerar custos. O doctor avisa para
que você possa remover o override e restaurar o roteamento de API + custos por modelo.

### 3) Migrações de estado legadas (layout em disco)
...
deixar quaisquer pastas legadas para trás como backups. O Gateway/CLI também migra
automaticamente as sessions legadas + diretório do agente na inicialização para que
histórico/auth/modelos cheguem ao caminho por agente sem uma execução manual do doctor.
A autenticação do WhatsApp é intencionalmente migrada apenas via `openclaw doctor`.
...
- **Permissões do diretório de estado**: verifica possibilidade de escrita; oferece reparar permissões
  (e emite uma dica `chown` quando é detectada incompatibilidade de proprietário/grupo).
- **Diretórios de session ausentes**: `sessions/` e o diretório de armazenamento de sessions são
  necessários para persistir histórico e evitar falhas `ENOENT`.
- **Incompatibilidade de transcript**: avisa quando entradas recentes de session têm arquivos
  de transcript ausentes.
- **Session principal “JSONL de 1 linha”**: sinaliza quando o transcript principal tem apenas uma
  linha (o histórico não está se acumulando).
- **Múltiplos diretórios de estado**: avisa quando múltiplas pastas `~/.openclaw` existem em
...
- **Permissões do arquivo de configuração**: avisa se `~/.openclaw/openclaw.json` é
...
expirando/expirados e pode renová-los quando seguro. Se o perfil Anthropic Claude Code
estiver obsoleto, sugere executar `claude setup-token` (ou colar um setup-token).
Prompts de renovação só aparecem ao executar de forma interativa (TTY); `--non-interactive`
...
catálogo e a allowlist e avisa quando não resolver ou for proibido.
...
Ele também pode varrer serviços extras semelhantes a gateway e imprimir dicas de limpeza.
...
Se estiver sendo executado como um serviço de usuário systemd, o doctor garante que o linger
esteja habilitado para que o gateway permaneça ativo após logout.
...
O Doctor avisa quando `gateway.auth` está ausente em um gateway local e oferece
gerar um token. Use `openclaw doctor --generate-gateway-token` para forçar a criação de token
em automação.
...
O Doctor verifica a configuração do supervisor instalado (launchd/systemd/schtasks) em busca de
padrões ausentes ou desatualizados (por exemplo, dependências network-online do systemd e
atraso de reinício). Quando encontra uma incompatibilidade, recomenda uma atualização e pode
...
O Doctor avisa quando o serviço do gateway é executado em Bun ou em um caminho de Node gerenciado
por versão (`nvm`, `fnm`, `volta`, `asdf`, etc.). Os canais WhatsApp + Telegram exigem Node,
e caminhos de gerenciadores de versão podem quebrar após upgrades porque o serviço não
carrega sua inicialização de shell. O Doctor oferece migrar para uma instalação de Node do sistema quando
disponível (Homebrew/apt/choco).

### 18) Gravação de configuração + metadados do assistente

O Doctor persiste quaisquer mudanças de configuração e carimba metadados do assistente para
registrar a execução do doctor.
