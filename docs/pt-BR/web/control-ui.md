...
- `connect.params.auth.password`
  ...
  O assistente de integracao inicial gera um token do gateway por padrão, então cole-o aqui na primeira conexão.
...
exige uma **aprovação de pareamento única** — mesmo que você esteja no mesmo Tailnet
...
Depois de aprovado, o dispositivo é lembrado e não exigirá nova aprovação, a menos que
você a revogue com `openclaw devices revoke --device <id> --role <role>`. Veja
...
- Transmitir chamadas de ferramentas + cartões de saída ao vivo das ferramentas no Chat (eventos do agente)
- Canais: status de WhatsApp/Telegram/Discord/Slack + canais de plugin (Mattermost, etc.) + login por QR + configuração por canal (`channels.status`, `web.login.*`, `config.patch`)
- Instâncias: lista de presença + atualização (`system-presence`)
- Sessões: lista + substituições por sessão de thinking/verbose (`sessions.list`, `sessions.patch`)
...
- Escritas de configuração incluem um guard de hash-base para evitar sobrescrever edições concorrentes
- Esquema de configuração + renderização de formulários (`config.schema`, incluindo esquemas de plugin + canal); o editor JSON bruto permanece disponível
...
- Para jobs isolados, a entrega padrão é anunciar um resumo. Você pode mudar para nenhum se quiser execuções apenas internas.
...
- `chat.inject` adiciona uma nota do assistente à transcrição da sessão e transmite um evento `chat` para atualizações apenas da UI (sem execução de agente, sem entrega em canal).
- Parar:
...
Por padrão, requisições do Serve podem autenticar via cabeçalhos de identidade do Tailscale
...
`tailscale whois` e correspondendo-o ao cabeçalho, e só aceita isso quando a
...
o OpenClaw **bloqueia** conexões da UI de Controle sem identidade de dispositivo.
...
**Exemplo de downgrade (somente token sobre HTTP):**
...
Isso desativa a identidade de dispositivo + pareamento para a UI de Controle (mesmo em HTTPS). Use
...
Depois, aponte a UI para a URL do WS do seu Gateway (por exemplo, `ws://127.0.0.1:18789`).

## Depuração/testes: servidor de dev + Gateway remoto

A UI de Controle são arquivos estáticos; o alvo do WebSocket é configurável e pode ser
diferente da origem HTTP. Isso é útil quando você quer o servidor de dev do Vite
...
- `token` é armazenado no localStorage; `password` é mantido apenas em memória.
...
- Para configurações de dev cross-origin (por exemplo, `pnpm ui:dev` para um Gateway remoto), adicione a origem da UI a `gateway.controlUi.allowedOrigins`.
