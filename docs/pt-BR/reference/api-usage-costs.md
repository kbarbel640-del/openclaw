...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T06:57:17Z
---
...
**Janelas de uso no CLI (cotas do provedor)**
...
Veja [Uso de tokens e custos](/token-use) para detalhes e exemplos.
...
Veja [Modelos](/providers/models) para configuração de preços e [Uso de tokens e custos](/token-use) para exibição.
...
- Fallback opcional para OpenAI se embeddings locais falharem
...
### 5) Ferramenta de fetch da web (Firecrawl)
...
Se o Firecrawl não estiver configurado, a ferramenta faz fallback para fetch direto + readability (sem API paga).

Veja [Ferramentas web](/tools/web).

### 6) Snapshots de uso do provedor (status/health)
...
`openclaw models scan` pode sondar modelos do OpenRouter e usa `OPENROUTER_API_KEY` quando a
sondagem está habilitada.
...
As Skills podem armazenar `apiKey` em `skills.entries.<name>.apiKey`. Se uma skill usar essa chave para APIs
