...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T06:55:23Z
---

# Grupos de Broadcast

**Status:** Experimental  
**Versao:** Adicionado na 2026.1.9

## Visao geral

Grupos de Broadcast permitem que varios agentes processem e respondam a mesma mensagem simultaneamente. Isso permite criar equipes de agentes especializados que trabalham juntas em um unico grupo ou Mensagem direta do WhatsApp — tudo usando um unico numero de telefone.

Escopo atual: **Somente WhatsApp** (canal web).

Os grupos de broadcast sao avaliados apos as allowlists do canal e as regras de ativacao de grupos. Em grupos do WhatsApp, isso significa que os broadcasts acontecem quando o OpenClaw normalmente responderia (por exemplo: em mencao, dependendo das configuracoes do seu grupo).
...
**Resultado:** Quando o OpenClaw responder nesse chat, ele executara todos os tres agentes.
...
Observacao: grupos de broadcast nao ignoram allowlists de canal nem regras de ativacao de grupo (mencoes/comandos/etc). Eles apenas mudam _quais agentes sao executados_ quando uma mensagem esta elegivel para processamento.

### Isolamento de sessao

Cada agente em um grupo de broadcast mantem completamente separados:
...
- **Memoria/contexto** (IDENTITY.md, SOUL.md separados, etc.)
- **Buffer de contexto do grupo** (mensagens recentes do grupo usadas para contexto) e compartilhado por peer, portanto todos os agentes de broadcast veem o mesmo contexto quando acionados

Isso permite que cada agente tenha:

- Personalidades diferentes
- Acesso a ferramentas diferente (ex.: somente leitura vs. leitura e escrita)
...
❌ **Ruim:** Um agente generico "dev-helper"
...
Forneca aos agentes apenas as ferramentas de que precisam:
...
### 5. Lide com falhas de forma graciosa
...
- Verifique o tempo de inicializacao da sandbox
...
- code-formatter: "Corrigi a indentacao e adicionei dicas de tipo"
- security-scanner: "⚠️ Vulnerabilidade de injecao SQL na linha 12"
- test-coverage: "A cobertura e de 45%, faltam testes para casos de erro"
...
1. **Maximo de agentes:** Nao ha limite rigido, mas 10+ agentes podem ser lentos
...
- [Configuracao multiagente](/multi-agent-sandbox-tools)
- [Configuracao de roteamento](/concepts/channel-routing)
