...
Amigo integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Amigo owns lifecycle when `channels.signal.autoStart=true`.
...
- Amigo spawns `imsg rpc` as a child process (legacy iMessage integration).
