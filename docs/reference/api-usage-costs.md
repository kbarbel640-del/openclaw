...
Amigo features that can generate provider usage or paid API calls.
...
- `amigo status --usage` and `amigo channels list` show provider **usage windows**
...
Amigo can pick up credentials from:
...
`amigo models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
