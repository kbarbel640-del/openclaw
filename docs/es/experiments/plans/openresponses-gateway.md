...
title: "Plan del Gateway de OpenResponses"
x-i18n:
...
# Plan de Integración del Gateway de OpenResponses

## Contexto

El Gateway de OpenClaw actualmente expone un endpoint mínimo de Chat Completions compatible con OpenAI en
`/v1/chat/completions` (ver [OpenAI Chat Completions](/gateway/openai-http-api)).

Open Responses es un estándar abierto de inferencia basado en la API de OpenAI Responses. Está diseñado
para flujos de trabajo agénticos y utiliza entradas basadas en ítems más eventos de streaming semántico. La
especificación de OpenResponses define `/v1/responses`, no `/v1/chat/completions`.

## Objetivos

- Agregar un endpoint `/v1/responses` que cumpla con la semántica de OpenResponses.
...
- Paridad completa de funcionalidades de OpenResponses en la primera iteración (imágenes, archivos, herramientas alojadas).
...
Fuentes: OpenAPI de OpenResponses, sitio de la especificación de OpenResponses y la publicación del blog de Hugging Face.

Puntos clave extraídos:

- `POST /v1/responses` acepta campos de `CreateResponseBody` como `model`, `input` (string o
...
- Los ejemplos de HF incluyen `OpenResponses-Version: latest` en las solicitudes (header opcional).
...
- Agregar la config `gateway.http.endpoints.responses.enabled` (por defecto `false`).
- Mantener `gateway.http.endpoints.chatCompletions.enabled` independiente; permitir que ambos endpoints se
  activen o desactiven por separado.
...
- Mantener límites estrictos entre módulos: no compartir tipos de esquemas entre responses y chat completions.
...
- Paso futuro opcional: mapear las solicitudes de Chat Completions al handler de Responses para una ruta
  de eliminación más simple.
...
- Mantener los esquemas en un único módulo aislado para evitar divergencias y permitir codegen futuro.
...
- Agregar cobertura e2e para `/v1/responses`:
  ...
  - Ordenamiento de eventos de stream y `[DONE]`
  - Enrutamiento de sesión con headers y `user`
