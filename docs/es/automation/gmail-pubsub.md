...
read_when:
  ...
  - Configurar push de Pub/Sub para despertar del agente
...
Objetivo: watch de Gmail -> push de Pub/Sub -> `gog gmail watch serve` -> webhook de OpenClaw.
...
- `tailscale` con sesión iniciada ([tailscale.com](https://tailscale.com/)). La configuración compatible usa Tailscale Funnel para el endpoint HTTPS público.
  ...
  En este momento, Tailscale es lo que soportamos.
...
Para establecer un modelo y nivel de pensamiento predeterminados específicamente para hooks de Gmail, agregue
...
- Si se establece `agents.defaults.models`, el modelo de Gmail debe estar en la allowlist.
...
Use el helper de OpenClaw para conectar todo (instala dependencias en macOS vía brew):
...
`hooks.gmail.tailscale.target` (o `--tailscale-target`) a una URL completa como
...
- Cuando `hooks.enabled=true` y `hooks.gmail.account` están configurados, el Gateway inicia
  `gog gmail watch serve` al arrancar y renueva automáticamente el watch.
- Establezca `OPENCLAW_SKIP_GMAIL_WATCHER=1` para optar por no participar (útil si ejecuta el daemon usted mismo).
...
1. Seleccione el proyecto de GCP **que posee el cliente OAuth** usado por `gog`.
...
Nota: el watch de Gmail requiere que el tema de Pub/Sub viva en el mismo proyecto que el cliente OAuth.
...
Ejemplo local (autenticación con token compartido):
...
Si necesita un túnel que no sea Tailscale, cablee manualmente y use la URL pública en la suscripción de push
...
Envíe un mensaje a la bandeja de entrada observada:
...
Verifique el estado del watch y el historial:
...
- Mensajes vacíos: el push de Gmail solo proporciona `historyId`; obtenga mediante `gog gmail history`.
