...
- **Configuración de sandbox** (`agents.list[].sandbox` sobrescribe `agents.defaults.sandbox`)
...
Si quiere compartir credenciales, copie `auth-profiles.json` en el `agentDir` del otro agente.

Para ver cómo se comporta el sandbox en tiempo de ejecución, consulte [Sandboxing](/gateway/sandboxing).
...
Cuando existen configuraciones tanto globales (`agents.defaults.*`) como específicas por agente (`agents.list[].*`):
...
- `agents.list[].sandbox.{docker,browser,prune}.*` sobrescribe `agents.defaults.sandbox.{docker,browser,prune}.*` para ese agente (se ignora cuando el alcance del sandbox se resuelve en `"shared"`).
...
Las políticas de herramientas (globales, por agente, de sandbox) admiten entradas `group:*` que se expanden a múltiples herramientas concretas:
...
`tools.elevated` es la línea base global (lista de permitidos basada en el remitente). `agents.list[].tools.elevated` puede restringir aún más Elevated para agentes específicos (ambos deben permitir).
...
- Deshabilitar Elevated globalmente (`tools.elevated.enabled: false`) si solo quiere ejecución en sandbox
- Deshabilitar Elevated por agente (`agents.list[].tools.elevated.enabled: false`) para perfiles sensibles
...
Las configuraciones heredadas `agent.*` son migradas por `openclaw doctor`; en adelante, prefiera `agents.defaults` + `agents.list`.
...
se tratan como non-main y se ejecutarán en sandbox. Si quiere que un agente nunca use sandbox, establezca `agents.list[].sandbox.mode: "off"`.
...
- Verifique si hay un `agents.defaults.sandbox.mode` global que lo sobrescriba
- La configuración específica por agente tiene precedencia, así que establezca `agents.list[].sandbox.mode: "all"`
...
- Establezca `scope: "agent"` en la configuración de sandbox específica por agente
