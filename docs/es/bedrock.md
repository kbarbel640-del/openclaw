...
read_when:
  ...
  - Necesita la configuracion de credenciales/región de AWS para llamadas al modelo
...
OpenClaw puede usar modelos de **Amazon Bedrock** a través del proveedor de streaming **Bedrock Converse** de pi‑ai. La autenticación de Bedrock usa la **cadena de credenciales predeterminada del SDK de AWS**, no una clave de API.
...
Cuando OpenClaw se ejecuta en una instancia de EC2 con un rol de IAM adjunto, el SDK de AWS usará automáticamente el servicio de metadatos de la instancia (IMDS) para la autenticación. Sin embargo, la detección de credenciales de OpenClaw actualmente solo verifica variables de entorno, no credenciales de IMDS.

**Solución alternativa:** Establezca `AWS_PROFILE=default` para indicar que las credenciales de AWS están disponibles. La autenticación real sigue usando el rol de la instancia mediante IMDS.
...
- OpenClaw muestra la fuente de credenciales en este orden: `AWS_BEARER_TOKEN_BEDROCK`,
  luego `AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY`, luego `AWS_PROFILE`, y finalmente la
  cadena predeterminada del SDK de AWS.
- El soporte de razonamiento depende del modelo; revise la tarjeta del modelo de Bedrock para conocer las capacidades actuales.
