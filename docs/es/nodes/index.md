...
read_when:
  ...
  - Usar el canvas/cámara del nodo para el contexto del agente
...
Un **nodo** es un dispositivo complementario (macOS/iOS/Android/sin interfaz) que se conecta al **WebSocket** del Gateway (el mismo puerto que los operadores) con `role: "node"` y expone una superficie de comandos (p. ej., `canvas.*`, `camera.*`, `system.*`) vía `node.invoke`. Detalles del protocolo: [Protocolo del Gateway](/gateway/protocol).

Transporte legado: [Protocolo Bridge](/gateway/bridge-protocol) (TCP JSONL; obsoleto/eliminado para nodos actuales).

macOS también puede ejecutarse en **modo nodo**: la app de la barra de menús se conecta al servidor WS del Gateway y expone sus comandos locales de canvas/cámara como un nodo (por lo que `openclaw nodes …` funciona contra este Mac).
...
crea una solicitud de emparejamiento de dispositivo para `role: node`. Apruebe mediante la CLI de dispositivos (o la UI).
...
- `node.pair.*` (CLI: `openclaw nodes pending/approve/reject`) es un almacén de emparejamiento de nodos independiente, propiedad del gateway; **no** controla el saludo WS de `connect`.
...
### Incluir comandos en la allowlist

Las aprobaciones de ejecución son **por host de nodo**. Agregue entradas a la allowlist desde el gateway:
...
Las aprobaciones viven en el host del nodo en `~/.openclaw/exec-approvals.json`.
...
Una vez establecido, cualquier llamada `exec` con `host=node` se ejecuta en el host del nodo (sujeta a la allowlist/aprobaciones del nodo).
...
Existen ayudas de nivel superior para los flujos comunes de “dar al agente un adjunto MEDIA”.
...
- Android mostrará el aviso del sistema de captura de pantalla antes de grabar.
...
Los nodos Android pueden exponer `sms.send` cuando el usuario concede el permiso de **SMS** y el dispositivo admite telefonía.
...
- Los nodos macOS descartan anulaciones de `PATH`; los hosts de nodo sin interfaz solo aceptan `PATH` cuando antepone el PATH del host de nodo.
- En modo nodo macOS, `system.run` está controlado por aprobaciones de exec en la app de macOS (Configuración → Aprobaciones de Exec).
  Preguntar/allowlist/completo se comportan igual que en el host de nodo sin interfaz; las solicitudes denegadas devuelven `SYSTEM_RUN_DENIED`.
...
Cuando hay varios nodos disponibles, puede enlazar exec a un nodo específico.
...
OpenClaw puede ejecutar un **host de nodo sin interfaz** (sin UI) que se conecta al
...
- El emparejamiento sigue siendo obligatorio (el Gateway mostrará un aviso de aprobación del nodo).
- El host de nodo almacena su id de nodo, token, nombre para mostrar y la información de conexión al gateway en `~/.openclaw/node.json`.
- Las aprobaciones de exec se aplican localmente mediante `~/.openclaw/exec-approvals.json`
  (consulte [Aprobaciones de Exec](/tools/exec-approvals)).
- En macOS, el host de nodo sin interfaz prefiere el host de exec de la app complementaria cuando es accesible y
  recurre a la ejecución local si la app no está disponible. Establezca `OPENCLAW_NODE_EXEC_HOST=app` para requerir
  la app, o `OPENCLAW_NODE_EXEC_FALLBACK=0` para desactivar el fallback.
