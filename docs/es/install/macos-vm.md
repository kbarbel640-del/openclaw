---
summary: "Ejecute OpenClaw en una VM de macOS en sandbox (local o alojada) cuando necesite aislamiento o iMessage"
read_when:
  ...
  - Desea comparar opciones de VM de macOS locales vs alojadas
title: "VMs de macOS"
x-i18n:
...
## Predeterminado recomendado (la mayoría de los usuarios)

- **VPS Linux pequeño** para un Gateway siempre activo y de bajo costo. Consulte [VPS hosting](/vps).
- **Hardware dedicado** (Mac mini o caja Linux) si desea control total y una **IP residencial** para automatización del navegador. Muchos sitios bloquean IPs de centros de datos, por lo que la navegación local suele funcionar mejor.
- **Híbrido:** mantenga el Gateway en un VPS económico y conecte su Mac como **node** cuando necesite automatización de navegador/UI. Consulte [Nodes](/nodes) y [Gateway remote](/gateway/remote).

Use una VM de macOS cuando necesite específicamente capacidades exclusivas de macOS (iMessage/BlueBubbles) o quiera un aislamiento estricto de su Mac de uso diario.
...
- Entorno macOS completo en aislamiento (su host se mantiene limpio)
...
Si desea macOS en la nube, los proveedores de Mac alojados también funcionan:

- [MacStadium](https://www.macstadium.com/) (Macs alojados)
- Otros proveedores de Mac alojados también funcionan; siga su documentación de VM + SSH
...
3. Complete el Asistente de Configuración, habilite Inicio de sesión remoto (SSH)
4. `lume run openclaw --no-display`
5. Ingrese por SSH, instale OpenClaw, configure canales
6. Listo
...
Documentación: [Lume Installation](https://cua.ai/docs/lume/guide/getting-started/installation)
...
2. Omitir Apple ID (o inicie sesión si desea iMessage más adelante)
...
1. Abra Ajustes del Sistema → General → Compartir
...
Reemplace `youruser` por la cuenta que creó, y la IP por la IP de su VM.
...
Siga los avisos de incorporacion para configurar su proveedor de modelo (Anthropic, OpenAI, etc.).
...
Esta es la función clave de ejecutar en macOS. Use [BlueBubbles](https://bluebubbles.app) para agregar iMessage a OpenClaw.
...
- Deshabilitar el reposo en Ajustes del Sistema → Ahorro de Energía
- Usar `caffeinate` si es necesario

Para un verdadero siempre activo, considere un Mac mini dedicado o un VPS pequeño. Consulte [VPS hosting](/vps).
...
| Problema                        | Solución                                                                                          |
| ------------------------------- | ------------------------------------------------------------------------------------------------- |
| No se puede hacer SSH a la VM   | Verifique que "Inicio de sesión remoto" esté habilitado en los Ajustes del Sistema de la VM       |
| La IP de la VM no aparece       | Espere a que la VM arranque por completo, ejecute `lume get openclaw` nuevamente                  |
| No se encuentra el comando Lume | Agregue `~/.local/bin` a su PATH                                                                  |
