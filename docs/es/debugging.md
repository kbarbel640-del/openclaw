---
summary: "Herramientas de depuración: modo watch, flujos de modelo en bruto y trazado de fugas de razonamiento"
read_when:
  - Necesita inspeccionar la salida cruda del modelo para detectar fugas de razonamiento
...
Esto es útil cuando necesita alternar ajustes poco comunes sin editar `openclaw.json`.
...
Para iteraciones rápidas, ejecute el gateway bajo el observador de archivos:
...
Agregue cualquier bandera de CLI del gateway después de `gateway:watch` y se pasarán
en cada reinicio.
...
- **`--dev` global (perfil):** aísla el estado bajo `~/.openclaw-dev` y
  establece el puerto del gateway de forma predeterminada en `19001` (los puertos derivados se desplazan con él).
...
Si aún no tiene una instalación global, ejecute el CLI mediante `pnpm openclaw ...`.
...
Flujo de reinicio (inicio desde cero):
...
Consejo: si ya hay un gateway no dev en ejecución (launchd/systemd), deténgalo primero:
...
OpenClaw puede registrar el **flujo crudo del asistente** antes de cualquier filtrado/formateo.
...
Para capturar **fragmentos crudos compatibles con OpenAI** antes de que se analicen en bloques,
...
- Mantenga los registros locales y elimínelos después de la depuración.
