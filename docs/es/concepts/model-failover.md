---
summary: "Cómo OpenClaw rota perfiles de autenticación y hace fallback entre modelos"
read_when:
  - Diagnóstico de la rotación de perfiles de autenticación, tiempos de enfriamiento o comportamiento de fallback de modelos
  - Actualización de reglas de failover para perfiles de autenticación o modelos
...
OpenClaw maneja las fallas en dos etapas:
...
- se completa una compactación (incrementa el contador de compactación)
...
Los perfiles fijados por el usuario permanecen bloqueados a ese perfil; si falla y hay fallbacks de modelos
configurados, OpenClaw pasa al siguiente modelo en lugar de cambiar de perfil.

### Por qué OAuth puede “parecer perdido”

Si tiene tanto un perfil OAuth como un perfil de clave de API para el mismo proveedor, el round‑robin puede alternar entre ellos a lo largo de los mensajes a menos que estén fijados. Para forzar un solo perfil:
...
Los errores de formato/solicitud inválida (por ejemplo, fallas de validación del ID de llamada de herramienta
de Cloud Code Assist) se tratan como aptos para failover y usan los mismos cooldowns.
...
Las fallas de facturación/crédito (por ejemplo, “créditos insuficientes” / “saldo de crédito demasiado bajo”) se tratan como aptas para failover, pero generalmente no son transitorias. En lugar de un cooldown corto, OpenClaw marca el perfil como **deshabilitado** (con un backoff más largo) y rota al siguiente perfil/proveedor.
...
- El backoff de facturación comienza en **5 horas**, se duplica por cada falla de facturación y se limita a **24 horas**.
...
`agents.defaults.model.fallbacks`. Esto aplica a fallas de autenticación, límites de tasa y
tiempos de espera que agotaron la rotación de perfiles (otros errores no avanzan el fallback).

Cuando una ejecución comienza con una anulación de modelo (hooks o CLI), los fallbacks igualmente terminan en
`agents.defaults.model.primary` después de intentar cualquier fallback configurado.

## Configuración relacionada

Vea [Configuración del Gateway](/gateway/configuration) para:
...
Vea [Modelos](/concepts/models) para una visión general más amplia de la selección de modelos y el fallback.
