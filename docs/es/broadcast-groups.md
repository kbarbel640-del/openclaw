...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T06:58:00Z
---

# Grupos de Difusión

**Estado:** Experimental  
**Versión:** Agregado en 2026.1.9

## Descripción general

Los Grupos de Difusión permiten que múltiples agentes procesen y respondan al mismo mensaje simultáneamente. Esto le permite crear equipos de agentes especializados que trabajan juntos en un solo grupo o Mensaje directo de WhatsApp — todo usando un único número de teléfono.

Alcance actual: **solo WhatsApp** (canal web).

Los grupos de difusión se evalúan después de las allowlists del canal y las reglas de activación de grupos. En grupos de WhatsApp, esto significa que las difusiones ocurren cuando OpenClaw normalmente respondería (por ejemplo: al mencionar, dependiendo de la configuración del grupo).
...
- Mensajes directos: número telefónico E.164 (p. ej., `+15551234567`)
...
Nota: los grupos de difusión no omiten las allowlists del canal ni las reglas de activación de grupos (menciones/comandos/etc). Solo cambian _qué agentes se ejecutan_ cuando un mensaje es elegible para procesamiento.
...
### 1. Mantenga los Agentes Enfocados

Diseñe cada agente con una única responsabilidad clara:
...
### 3. Configure Acceso a Herramientas Diferenciado
...
**Solución:** Agregue a la configuración de difusión o elimine de los bindings.
...
- code-formatter: "Corrigió la indentación y agregó sugerencias de tipo"
- security-scanner: "⚠️ Vulnerabilidad de inyección SQL en la línea 12"
- test-coverage: "La cobertura es 45%, faltan pruebas para casos de error"
...
- `[peerId]`: JID del grupo de WhatsApp, número E.164 u otro ID de par
...
- [Configuración Multiagente](/multi-agent-sandbox-tools)
- [Configuración de Enrutamiento](/concepts/channel-routing)
