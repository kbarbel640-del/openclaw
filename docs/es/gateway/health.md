---
summary: "Pasos de verificación de salud para la conectividad de canales"
read_when:
...
- `openclaw status --all` — diagnóstico local completo (solo lectura, con color, seguro para pegar en depuración).
- `openclaw status --deep` — también sondea el Gateway en ejecución (sondeos por canal cuando se admiten).
...
- Sin mensajes entrantes → confirme que el teléfono vinculado esté en línea y que el remitente esté permitido (`channels.whatsapp.allowFrom`); para chats grupales, asegúrese de que las reglas de allowlist + menciones coincidan (`channels.whatsapp.groups`, `agents.list[].groupChat.mentionPatterns`).

## Comando dedicado de "health"

`openclaw health --json` solicita al Gateway en ejecución su instantánea de salud (sin sockets de canal directos desde la CLI). Informa, cuando está disponible, las credenciales vinculadas/antigüedad de autenticación, resúmenes de sondeos por canal, resumen del almacén de sesiones y la duración del sondeo. Sale con código distinto de cero si el Gateway es inaccesible o el sondeo falla/expira. Use `--timeout <ms>` para sobrescribir el valor predeterminado de 10 s.
