...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T06:59:00Z
---
...
- `sessionKey` (string, opcional): clave de sesión de destino. Si se omite o `"main"`, el Gateway usa la clave de sesión principal configurada (respeta `session.mainKey` y el agente predeterminado, o `global` en el ámbito global).
...
Para ayudar a que las políticas de grupo resuelvan el contexto, puede configurar opcionalmente:
...
- `401` → no autorizado
- `404` → herramienta no disponible (no encontrada o no incluida en la lista permitida)
