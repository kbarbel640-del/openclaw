...
read_when:
  - Desea habilitar web_search o web_fetch
  - Necesita configurar la clave de Brave Search API
  - Desea usar Perplexity Sonar para búsqueda web
...
x-i18n:
  ...
  workflow: v1
  generated_at: 2026-02-08T07:00:26Z
---
...
- `web_search` — Busca en la web mediante Brave Search API (predeterminado) o Perplexity Sonar (directo o vía OpenRouter).
- `web_fetch` — Obtención HTTP + extracción legible (HTML → markdown/texto).

Estas **no** son automatización de navegador. Para sitios con mucho JS o inicios de sesión, use la
...
- `web_search` llama a su proveedor configurado y devuelve resultados.
  ...
  - **Perplexity**: devuelve respuestas sintetizadas por IA con citas de búsqueda web en tiempo real.
...
| Proveedor                  | Pros                                               | Contras                                   | Clave API                                   |
| -------------------------- | -------------------------------------------------- | ----------------------------------------- | ------------------------------------------- |
...
1. Cree una cuenta de Brave Search API en https://brave.com/search/api/
2. En el panel, elija el plan **Data for Search** (no “Data for AI”) y genere una clave API.
3. Ejecute `openclaw configure --section web` para guardar la clave en la configuración (recomendado), o establezca `BRAVE_API_KEY` en su entorno.

Brave ofrece un nivel gratuito más planes de pago; consulte el portal de Brave API para los
límites y precios actuales.

### Dónde establecer la clave (recomendado)
...
**Alternativa de entorno:** establezca `BRAVE_API_KEY` en el entorno del proceso del Gateway.
Para una instalación de gateway, colóquelo en `~/.openclaw/.env` (o en el entorno de su
servicio). Consulte [Env vars](/help/faq#how-does-openclaw-load-environment-variables).

## Uso de Perplexity (directo o vía OpenRouter)

Los modelos Perplexity Sonar tienen capacidades de búsqueda web integradas y devuelven
respuestas sintetizadas por IA con citas. Puede usarlos vía OpenRouter (no se requiere tarjeta
de crédito; admite cripto/prepago).

### Obtener una clave API de OpenRouter

1. Cree una cuenta en https://openrouter.ai/
2. Agregue créditos (admite cripto, prepago o tarjeta de crédito)
3. Genere una clave API en la configuración de su cuenta

### Configurar la búsqueda de Perplexity
...
**Alternativa de entorno:** establezca `OPENROUTER_API_KEY` o `PERPLEXITY_API_KEY` en el entorno del Gateway.
Para una instalación de gateway, colóquelo en `~/.openclaw/.env`.

Si no se establece una URL base, OpenClaw elige un valor predeterminado según el origen de la clave API:
...
| `perplexity/sonar-pro` (predeterminado) | Razonamiento de múltiples pasos con búsqueda web | Preguntas complejas    |
...
- Clave API para su proveedor elegido:
...
- Alternativa opcional con Firecrawl: establezca `tools.web.fetch.firecrawl.apiKey` o `FIRECRAWL_API_KEY`.
...
- Las solicitudes a Firecrawl usan modo de evasión de bots y almacenan en caché los resultados de forma predeterminada.
...
- Las respuestas se almacenan en caché (15 minutos por defecto) para reducir obtenciones repetidas.
