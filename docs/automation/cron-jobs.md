...
- Jobs persist under `~/.amigo/cron/` so restarts don’t lose schedules.
...
amigo cron list
amigo cron run <job-id>
amigo cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.amigo/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `amigo cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Amigo defaults to `announce`.
...
- Job store: `~/.amigo/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.amigo/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
amigo cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
amigo cron edit <jobId> --agent ops
amigo cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
amigo cron run <jobId>
amigo cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
amigo cron edit <jobId> \
...
```bash
amigo system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`amigo system event`](/cli/system).
...
- Amigo applies exponential retry backoff for recurring jobs after consecutive errors:
