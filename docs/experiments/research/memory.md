...
read_when:
  - Designing workspace memory (~/.amigo/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Amigo integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.amigo/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.amigo/workspace` as canonical human-readable memory.
...
```
~/.amigo/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Amigo**, but keep a separable core library.

### Why integrate into Amigo?
...
- You want the agent itself to call the tools:
  - `amigo memory recall "…" --k 25 --since 30d`
  - `amigo memory reflect --since 7d`
...
Pragmatic take for `~/.amigo/workspace`:
