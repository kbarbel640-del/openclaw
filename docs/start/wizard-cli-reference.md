...
read_when:
  - You need detailed behavior for amigo onboard
...
This page is the full reference for `amigo onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.amigo/amigo.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `amigo doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `amigo pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `amigo health`.
    - `amigo status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.amigo/.env` so launchd can read it.
...
- OAuth credentials: `~/.amigo/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.amigo/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`amigo onboard`](/cli/onboard)
