---
summary: "Chrome extension: let Amigo drive your existing Chrome tab"
read_when:
...
The Amigo Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate amigo-managed Chrome profile.
...
Amigo then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Amigo release (npm package) as static files. There is no separate “build” step.

After upgrading Amigo:

- Re-run `amigo browser extension install` to refresh the installed files under your Amigo state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Amigo ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Amigo to control.
...
- `ON`: attached; Amigo can drive that tab.
...
Debugging: `amigo sandbox explain`
...
`amigo browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `amigo browser extension install` first to copy the extension to a stable location under your Amigo state directory.
...
- **This is not isolated** like the dedicated amigo-managed profile.
