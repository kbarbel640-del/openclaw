---
summary: "Agent tool surface for Amigo (browser, canvas, nodes, message, cron) replacing legacy `amigo-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `amigo-*` skills
title: "Tools"
---

# Tools (Amigo)

Amigo exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `amigo-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `amigo.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Amigo logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`amigo node run`).
...
- Requires a Brave API key (recommended: `amigo configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Amigo-managed browser.
...
- Quick smoke: `amigo nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `amigo gateway` restart in-place)
