---
summary: "Web search + fetch tools (Brave Search API, Perplexity, Grok, Gemini Google Search grounding, Kimi, Linkup)"
read_when:
...
- `web_search` — Search the web via Brave Search API (default), Perplexity Sonar, xAI Grok, Gemini with Google Search grounding, Kimi, or Linkup.
...
- `web_search` calls your configured provider and returns results.
  ...
  - **Kimi**: returns AI-synthesized answers with citations via Moonshot's native web search.
...
| **Kimi**            | AI-synthesized answers, native web search                   | Requires Moonshot API key                | `KIMI_API_KEY` or `MOONSHOT_API_KEY`         |
...
```json5
{
  tools: {
    web: {
      search: {
        provider: "brave", // or "perplexity", "grok", "gemini", "kimi", "linkup"
      },
    },
  },
}
