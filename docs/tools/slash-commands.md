...
- `/subagents list|stop|log|info|send|steer|spawn` (inspect, stop, log, or message sub-agent runs for the current session)
- `/kill <target>` (kill a running subagent; alias for `/subagents kill`)
- `/steer <target> <message>` (send guidance to a running subagent; alias: `/tell`)
