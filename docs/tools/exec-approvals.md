...
- **gateway host** → `amigo` process on the gateway machine
...
`~/.amigo/exec-approvals.json` directly.

CLI: `amigo approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
