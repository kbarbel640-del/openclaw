...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Amigo
...
Running Amigo via Node with `tsx` fails at startup with:

```
[amigo] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `amigo status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch amigo.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node amigo.mjs status` works on Node 25.
