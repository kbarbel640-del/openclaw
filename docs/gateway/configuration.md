---
summary: "All configuration options for ~/.amigo/amigo.json with examples"
read_when:
...
Amigo reads an optional **JSON5** config from `~/.amigo/amigo.json` (comments + trailing commas allowed).

If the file is missing, Amigo uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.amigo/workspace`). You usually only need a config to:
...
Amigo only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `amigo doctor`, `amigo logs`, `amigo health`, `amigo status`, `amigo service`, `amigo help`).
- Run `amigo doctor` to see the exact issues.
- Run `amigo doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `amigo config set`. Keep a backup of `~/.amigo/amigo.json`.
...
```bash
amigo gateway call config.get --params '{}' # capture payload.hash
amigo gateway call config.apply --params '{
...
```bash
amigo gateway call config.get --params '{}' # capture payload.hash
amigo gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.amigo/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@amigo", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.amigo/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.amigo/workspace" }],
}
```
...
{ "$include": "/etc/amigo/base.json5" } // absolute
...
```json5
// ~/.amigo/clients/mueller/agents.json5
[
...
```json5
// ~/.amigo/clients/mueller/broadcast.json5
{
...
Amigo reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Amigo runs your login shell and imports only the missing expected keys (never overrides).
...
Amigo stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.amigo/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.amigo/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`)
...
- `~/.amigo/agent/*` (migrated by `amigo doctor` into `~/.amigo/agents/<defaultAgentId>/agent/*`)
...
On first use, Amigo imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Amigo derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/amigo/amigo-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/amigo/amigo.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.amigo/credentials/whatsapp/biz
          // authDir: "~/.amigo/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `amigo doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@amigo", "amigo"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@amigo"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.amigo/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.amigo/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.amigo/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.amigo/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.amigo/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.amigo/workspace-home" },
      { id: "work", workspace: "~/.amigo/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `amigo.json`).
...
Amigo starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["amigo-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-amigo",
...
Amigo starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `amigo plugins install @amigo/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Amigo starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Amigo spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Amigo
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/amigo" } },
}
```
...
When a file exceeds this limit, Amigo logs a warning and injects a truncated
...
message envelopes). If unset, Amigo uses the host timezone at runtime.
...
`[amigo]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[amigo]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[amigo]"`, Amigo will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Amigo generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.amigo/settings/tts.json",
      elevenlabs: {
...
Amigo also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Amigo currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `amigo configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:amigo`: all built-in Amigo tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.amigo/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "amigo-sandbox-browser:bookworm-slim",
          containerPrefix: "amigo-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Amigo uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.amigo/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Amigo config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Amigo writes/merges a `models.json` into
`~/.amigo/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Amigo uses
...
- Shortcut: `amigo onboard --auth-choice opencode-zen`.
...
Shortcut: `amigo onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `amigo onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `amigo onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `amigo onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `amigo onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `amigo onboard --auth-choice minimax-api`.
...
- Override the agent config root with `OPENCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.amigo/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.amigo/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.amigo/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Amigo stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.amigo/skills` (workspace skills
...
from `~/.amigo/extensions`, `<workspace>/.amigo/extensions`, plus any
...
### `browser` (amigo-managed browser)

Amigo can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for amigo and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Amigo.app menubar, or `amigo gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      amigo: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/amigo" }
...
- When a connection comes from one of these IPs, Amigo uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `amigo gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
...
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  ...
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Amigo
...
- Amigo.app watches `~/.amigo/amigo.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.amigo/amigo.json` (or `OPENCLAW_CONFIG_PATH`) and applies changes automatically.
...
- `~/.amigo/amigo.json` (or `OPENCLAW_CONFIG_PATH`)
...
- `amigo --dev …` → uses `~/.amigo-dev` + shifts ports from base `19001`
- `amigo --profile <name> …` → uses `~/.amigo-<name>` (port via config/env/flags)
...
```bash
OPENCLAW_CONFIG_PATH=~/.amigo/a.json \
OPENCLAW_STATE_DIR=~/.amigo-a \
amigo gateway --port 19001
```
...
Gmail helper config (used by `amigo webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "amigo@gmail.com",
...
Note: when `tailscale.mode` is on, Amigo defaults `serve.path` to `/` so
...
Default root: `~/.amigo/workspace/canvas`  
Default port: `18793` (chosen to avoid the amigo browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__amigo__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__amigo__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.amigo/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.amigo/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.amigo/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_amigo-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `amigo` (advertises `amigo.local`). Override with `OPENCLAW_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_amigo-gw._tcp` under `~/.amigo/dns/` using the configured discovery domain (example: `amigo.internal.`).
