...
- `contextEngine`: per-agent context-engine id. Overrides `agents.defaults.contextEngine` and `plugins.slots.contextEngine`.
...
- `plugins.slots.contextEngine`: slot-level context-engine fallback (`legacy` by default).
- Context-engine precedence: `agents.list[].contextEngine` -> `agents.defaults.contextEngine` -> `plugins.slots.contextEngine` -> built-in default (`legacy`).
