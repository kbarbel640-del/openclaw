...
- During heartbeat runs, Amigo treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `amigo sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Amigo skips the heartbeat run entirely (no model call).
...
```bash
amigo system event --text "Check for urgent follow-ups" --mode now
```
