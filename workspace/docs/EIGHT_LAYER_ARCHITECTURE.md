...
| Volume 掛載      | ✅   | workspace + media 完整掛載           |
| 統一路徑環境變數 | ✅   | MEDIA_ROOT, WORKSPACE_ROOT, TMP_ROOT |
...
| ---------------------- | ------ | -------------------------- |
| ~~統一路徑映射表~~     | ✅     | ~~每次都要猜路徑~~         |
| ~~downloads 目錄掛載~~ | ✅     | ~~無法讀取 Telegram 圖片~~ |
| ~~媒體統一入口~~       | ✅     | ~~不同 channel 媒體散落~~  |
| **權限標準化**         | P1     | 有時 permission denied     |
| ~~環境變數統一管理~~   | ✅     | ~~API key 散落各處~~       |
...
| telegram-userbot | ✅   | 路徑問題已修復    |
| 圖片讀取         | ✅   | Volume 已掛載     |
| media-ingestion  | ✅   | 統一媒體處理 hook |
| 音訊轉錄         | △    | 有配置但不穩定    |
...
| 缺失                 | 優先級 | 影響                                    |
| -------------------- | ------ | --------------------------------------- |
| ~~統一媒體處理管道~~ | ✅     | ~~每個 channel 各自處理~~               |
| ~~圖片 OCR/描述~~    | ✅     | ~~無法理解圖片內容~~（media-ingestion） |
| **音訊轉文字**       | P1     | 語音消息無法處理                        |
| **視頻摘要**         | P2     | 視頻內容無法理解                        |
...
### Phase 1: 穩固地基 (1-2 週) ✅ 進行中

```
1. 骨架層 35% → 60% ✅
   - ✅ 修復 Volume 映射（docker-compose.soul.yml）
   - ✅ 統一路徑管理（MEDIA_ROOT, WORKSPACE_ROOT, TMP_ROOT）
   - ✅ 媒體統一入口（/app/media/{channel}/）

2. 感知層 55% → 65% ✅
   - ✅ 圖片讀取修復（Volume 已掛載）
   - ✅ 媒體處理管道（media-ingestion hook）
   - △ 待驗證：實際圖片 OCR 效果
```
