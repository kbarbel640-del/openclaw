...
# Sheet1: same as before (INNER JOIN is correct - must have recharge)
...
# Sheet2: LEFT JOIN to include players with bets but no deposits
s2 = q("""SELECT sp.login_name AS account,
COALESCE(SUM(pro.pay_amount), 0) AS total_deposit,
COALESCE(SUM(CASE WHEN pro.create_time >= '2026-01-18' AND pro.create_time < '2026-01-26' THEN pro.pay_amount ELSE 0 END), 0) AS jan18_25_deposit
FROM sys_player sp
LEFT JOIN player_recharge_order pro ON sp.player_id = pro.player_id AND pro.order_status = 1
WHERE sp.player_id IN (
...
print(f"Sheet2: {len(s2)} rows (with LEFT JOIN)")
