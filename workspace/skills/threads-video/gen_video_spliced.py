#!/usr/bin/env python3
"""
Generate video with spliced voice (YunJhe + George's AI)
"""
import os
from moviepy import *
...
VOICE_FILE = f"{OUTPUT_DIR}/voice_spliced.mp3"
BGM_FILE = f"{SKILL_DIR}/assets/bgm.mp3"
OUTPUT_FILE = f"{OUTPUT_DIR}/video-05-spliced.mp4"
...
# Spliced voice timing (adjusted from whisper)
# Note: "你用" is very short at start, then George's "AI"
SUBTITLES = [
    (0.00, 0.45, "你用", False),
    (0.45, 1.72, "AI 省下的時間", False),
    (1.72, 3.52, "最後都去刷抖音了", False),
    (3.52, 5.24, "別騙自己", True),
    (5.24, 7.86, "AI 不是讓你變強", False),
    (7.86, 9.34, "是讓你更會逃避", False),
    (9.34, 11.52, "真正的用法？", False),
    (11.52, 13.58, "買回注意力", True),
    (13.58, 15.67, "不是時間", True),
]
...
print("🎬 Creating video with spliced voice...")
