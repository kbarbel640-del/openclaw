#!/bin/bash
# Synced version - timing based on actual audio analysis
set -e
...
echo "🎬 Creating SYNCED segments..."

# Timing based on actual silence detection:
# s1: 0-4s (voice at 0.42-2.99)
# s2: 4-8s (voice at 4.12-7.17)
# s3: 8-11s (voice at 7.53-10.40)
# s4: 11-14s (voice at 11.55-13.26)
...
create_seg() {
    ...
    ffmpeg -y -loop 1 -i "$img" -t $dur -r 30 \
        -vf "scale=8000:-1,zoompan=z='min(zoom+0.0003,1.05)':d=$frames:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=${WIDTH}x${HEIGHT},fade=t=in:st=0:d=0.2,fade=t=out:st=$(echo "$dur - 0.2" | bc):d=0.2" \
        -c:v libx264 -pix_fmt yuv420p "$out" 2>/dev/null
}

create_seg "$WORK_DIR/s1.png" "$WORK_DIR/v1.mp4" 4
create_seg "$WORK_DIR/s2.png" "$WORK_DIR/v2.mp4" 4
...
ffmpeg -y \
    ...
    -c:a aac -b:a 128k \
    -shortest \
    "$OUTPUT_DIR/video-02-synced.mp4" 2>/dev/null

echo ""
echo "✅ Done!"
ls -lh "$OUTPUT_DIR/video-02-synced.mp4"
