---
description: Update Amigo from upstream when branch has diverged (ahead/behind)
---

# Amigo Upstream Sync Workflow
...
# Move old version
mv /Applications/Amigo.app /tmp/Amigo-backup.app

# Install new build
cp -R dist/Amigo.app /Applications/

# Launch
open /Applications/Amigo.app
```
...
ps aux | grep -E "(amigo|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm amigo agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update amigo.json with fallback chains
...
pnpm amigo agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
