 ...
 * `amigo update`. Use it to verify the completion installation flow
...
\`amigo update\`, \`amigo doctor\`, and \`amigo onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Amigo Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`amigo completion --install\` later to enable.`));
    return;
  }
