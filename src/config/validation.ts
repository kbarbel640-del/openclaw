...
import type { AmigoConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: AmigoConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: AmigoConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AmigoSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AmigoConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AmigoConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as AmigoConfig)),
    ),
  };
}
