...
const ETC_AMIGO_DIR = path.join(ROOT_DIR, "etc", "amigo");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "amigo.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcAmigoPath(...parts: string[]) {
  return path.join(ETC_AMIGO_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcAmigoPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "amigo.json"))).toEqual({
      shared: true,
    });
  });
});
