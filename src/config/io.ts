...
import type { AmigoConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareAmigoVersions } from "./version.js";
...
function coerceConfig(value: unknown): AmigoConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AmigoConfig;
}
...
function stampConfigVersion(cfg: AmigoConfig): AmigoConfig {
...
function warnIfConfigFromFuture(cfg: AmigoConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareAmigoVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Amigo (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: AmigoConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AmigoConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as AmigoConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AmigoConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: AmigoConfig) {
    clearConfigCache();
...
// module scope. `AMIGO_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AMIGO_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.AMIGO_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): AmigoConfig {
...
export async function writeConfigFile(cfg: AmigoConfig): Promise<void> {
  clearConfigCache();
