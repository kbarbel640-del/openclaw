import type { AmigoConfig } from "./config.js";
...
function isTelegramConfigured(cfg: AmigoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: AmigoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: AmigoConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: AmigoConfig): boolean {
...
function isIMessageConfigured(cfg: AmigoConfig): boolean {
...
function isWhatsAppConfigured(cfg: AmigoConfig): boolean {
...
function isGenericChannelConfigured(cfg: AmigoConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: AmigoConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: AmigoConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: AmigoConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: AmigoConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: AmigoConfig, pluginId: string): AmigoConfig {
...
function registerPluginEntry(cfg: AmigoConfig, pluginId: string): AmigoConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
