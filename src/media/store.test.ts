...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "AMIGO_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.AMIGO_STATE_DIR = path.join(home, ".amigo");
    ...
    await fs.mkdir(path.join(home, ".amigo"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.amigo${path.sep}media`);
