...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".amigo-profile-decorated");
}
 ...
 */
export function decorateAmigoProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_AMIGO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_AMIGO_BROWSER_COLOR).toUpperCase();
