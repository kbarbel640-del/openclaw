...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const amigo = resolveProfile(resolved, "amigo");
    expect(amigo?.driver).toBe("amigo");
    expect(amigo?.cdpPort).toBe(18800);
    expect(amigo?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from AMIGO_GATEWAY_PORT when unset", () => {
    const prev = process.env.AMIGO_GATEWAY_PORT;
    process.env.AMIGO_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const amigo = resolveProfile(resolved, "amigo");
      expect(amigo?.cdpPort).toBe(19012);
      expect(amigo?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.AMIGO_GATEWAY_PORT;
      } else {
        process.env.AMIGO_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const amigo = resolveProfile(resolved, "amigo");
      expect(amigo?.cdpPort).toBe(19022);
      expect(amigo?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.AMIGO_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "amigo");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        amigo: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("amigo");
  });
});
