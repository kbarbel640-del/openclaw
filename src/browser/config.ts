import type { BrowserConfig, BrowserProfileConfig, AmigoConfig } from "../config/config.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_AMIGO_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "amigo" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_AMIGO_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_AMIGO_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Amigo browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for amigo/amigo CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: AmigoConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_AMIGO_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "amigo";
