...
describe("backward compatibility: peer.kind dm → direct", () => {
      ...
      bindings: [
        {
          ...
          match: {
            ...
            peer: { kind: "dm", id: "+15551234567" } as unknown as ChatType,
          },
        },
      ],
    };
  ...
  test("legacy dm in runtime peer matches direct in config", () => {
    const cfg: OpenClawConfig = {
      bindings: [
        {
          agentId: "alex",
    ...
    const route = resolveAgentRoute({
      ...
      channel: "whatsapp",
      accountId: null,
      // Runtime uses legacy "dm", but should be normalized
      peer: { kind: "dm", id: "+15551234567" } as unknown as ChatType,
    });
    expect(route.agentId).toBe("alex");
    expect(route.matchedBy).toBe("binding.peer");
  ...
  test("legacy dm in runtime parentPeer matches direct in config", () => {
    const cfg: OpenClawConfig = {
      bindings: [
        {
          agentId: "alex",
    ...
    const route = resolveAgentRoute({
      ...
      channel: "whatsapp",
      accountId: null,
      peer: { kind: "channel", id: "thread789" },
      // Runtime parent uses legacy "dm", but should be normalized
      parentPeer: { kind: "dm", id: "parent123" } as unknown as RoutePeer,
    });
    expect(route.agentId).toBe("alex");
    expect(route.matchedBy).toBe("binding.peer.parent");
  });
