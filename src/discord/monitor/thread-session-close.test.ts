...
const hoisted = vi.hoisted(() => {
    // Captures the mutator passed to updateSessionStore so we can invoke it
    // directly in tests without touching the filesystem or write-lock logic.
...
/**
 * Wire up updateSessionStore so that it runs the mutator against `store`
 * and returns whatever the mutator returns — identical to the real
 * implementation minus file I/O and the write lock.
 */
...
describe("closeDiscordThreadSessions", () => {
    ...
    it("does not match a key that contains the threadId as a substring of a longer snowflake", async () => {
        const longerSnowflake = `${THREAD_ID}00`; // e.g. "99900" — NOT "999"
