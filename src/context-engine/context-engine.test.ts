...
describe("Default engine selection", () => {
  // Ensure both legacy and lcm are registered before these tests.
  beforeEach(() => {
  ...
  it("resolveContextEngine() with config contextEngine='lcm' returns LCM engine", async () => {
    const engine = await resolveContextEngine(configWithSlot("lcm"));
