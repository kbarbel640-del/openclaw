...
async function collectSkillInstallScanWarnings(entry: SkillEntry): Promise<string[]> {
  ...
  const result = await scanSkillDirectory({
    skillDir,
    skillName,
    criticalMode: "warn",
    criticalMessage: (name, details) =>
      `WARNING: Skill "${name}" contains dangerous code patterns: ${details}`,
    warnMessage: (name, warnCount) =>
      `Skill "${name}" has ${warnCount} suspicious code pattern(s). Run "openclaw security audit --deep" for details.`,
    scanFailureMessage: (name, errorText) =>
      `Skill "${name}" code safety scan failed (${errorText}). Installation continues; run "openclaw security audit --deep" after install.`,
  });
...
async function installDownloadSpec(params: {
  ...
  let downloaded = 0;
  try {
    const result = await downloadUrlToFile({ url, destPath: archivePath, timeoutMs });
  ...
  const archiveType = resolveArchiveType(filename, spec.archive);
