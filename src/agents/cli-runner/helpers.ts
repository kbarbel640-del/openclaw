...
/**
 * Cleanup suspended Amigo CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: AmigoConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-cli-images-"));
  const paths: string[] = [];
