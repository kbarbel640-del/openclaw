...
import { __testing, createAmigoCodingTools } from "./pi-tools.js";
...
describe("createAmigoCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createAmigoCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "amigo-outside.txt");
