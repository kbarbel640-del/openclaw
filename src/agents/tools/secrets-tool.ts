...
export function createSecretsTool(opts?: { config?: OpenClawConfig }): AnyAgentTool {
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if ((action === "get" || action === "request" || action === "status" || action === "resolve") && !name) {
      ...
      switch (action) {
        case "get": {
          try {
            ...
            if (mode === "legacy" || mode === "yolo") {
              ...
              if (metadata.hint) lines.push(`Hint: ${metadata.hint}`);
        ...
        case "request": {
          ...
          return {
            ...
            details: { ok: true, action: "request", name, tier: secretDef.tier, description: secretDef.description },
          };
        }

        case "status": {
          const secretDef = getSecretDef(name!);
          if (!secretDef) throw new Error(`Secret '${name}' not registered`);
          ...
          if (grantStatus.status === "valid") {
            ...
            details = { ...baseDetails, status: "valid", expiresAt: grantStatus.expiresAt, remainingMinutes: mins };
        ...
        case "list": {
          ...
          if (secrets.length === 0) {
            return { content: [{ type: "text", text: "No secrets registered" }], details: { ok: true, secrets: [] } };
          }

          const lines = ["OpenClaw Secrets:", ""];
          const detailsList: Array<{ name: string; tier: string; description?: string; grantStatus: string }> = [];

          for (const secret of secrets) {
            const grantStatus = await checkGrant(secret.name);
            let status: string;
            if (secret.tier === "open") status = "always available";
            else if (grantStatus.status === "valid") status = `✅ ${Math.ceil(grantStatus.remaining! / 60000)}m left`;
            else if (grantStatus.status === "expired") status = "⏰ expired";
            else status = "🔒 needs approval";

            lines.push(`• ${secret.name} (${secret.tier}) — ${status}`);
            detailsList.push({ name: secret.name, tier: secret.tier, description: secret.description, grantStatus: status });
          }
        ...
        case "resolve": {
          ...
          if (mode === "legacy" || mode === "yolo") {
            ...
            return {
              content: [{ type: "text", text: `Use credentialRef: "secret:${name}" in tool parameters` }],
              details: { ok: true, ref: `secret:${name}`, name },
            };
          }
          const metadata = await getSecretMetadata(name!);
          return {
            content: [{ type: "text", text: `Use credentialRef: "${metadata.ref}" in tool parameters` }],
