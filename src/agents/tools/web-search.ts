...
type WebSearchConfig = NonNullable<AmigoConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: AmigoConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("amigo configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.amigo.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://amigo.ai",
      "X-Title": "Amigo Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
