...
describe("resolveSearchProxy", () => {
  ...
  it("returns undefined when ProxyAgent construction throws (bad URL)", () => {
    // undici's ProxyAgent will throw on an invalid URL like a plain word
    const fetcher = resolveSearchProxy({ proxy: "\x00not-a-url" });
