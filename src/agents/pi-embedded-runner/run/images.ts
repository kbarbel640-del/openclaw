...
export function detectImageReferences(prompt: string): DetectedImageRef[] {
  ...
  while ((match = mediaAttachedPattern.exec(prompt)) !== null) {
    ...
    // Relaxed match: capture everything until (mime), | url, or end ]
    const pathMatch = content.match(/^\s*([^|(\]]+?)\s*(?:\(|$|\||\])/i);
    if (pathMatch?.[1]) {
      const raw = pathMatch[1].trim();
      if (raw && !seen.has(raw.toLowerCase())) {
        seen.add(raw.toLowerCase());
        // For [media attached: ...], we skip the strict extension check
        // because this block is generated by our own system (buildInboundMediaNote)
        // and we want to trust the path provided by the channel handler.
