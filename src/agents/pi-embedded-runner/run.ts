...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      // Only bias profile order for user-locked profiles. Auto overrides should
      // respect the configured priority order (e.g., manual OAuth before API key)
      // rather than pinning whichever profile was auto-selected last. See #25510.
      const profileOrder = resolveAuthProfileOrder({
        cfg: params.config,
        store: authStore,
        provider,
        preferredProfile: lockedProfileId ?? undefined,
      });
