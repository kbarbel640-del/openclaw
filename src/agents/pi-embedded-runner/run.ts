...
export async function runEmbeddedPiAgent(
  ...
  // B1: Generate trace ID if not provided
  const traceId = params.traceId ?? getCurrentTraceId(generateTraceId());
  const requestId = params.runId ?? traceId;
  ...
  return withTraceContext(traceContext, () =>
    enqueueSession(() =>
      enqueueGlobal(async () => {
        ...
        // B6: Start telemetry session for this model call
        const telemetrySessionId = startTelemetrySession({
          traceId,
          requestId,
          sessionId: params.sessionId,
          sessionKey: params.sessionKey,
          ...
          provider,
          role: "executor",
          isSubagent: params.spawnedBy ? true : false,
        ...
        try {
          while (true) {
            ...
            // B6: Complete telemetry session on success
            ...
            log.debug(
              `embedded run done: runId=${params.runId} sessionId=${params.sessionId} durationMs=${runDuration} aborted=${aborted} telemetry=${telemetrySessionId}`,
            );
            if (lastProfileId) {
          ...
          // B6: Complete telemetry session on failure
          completeTelemetrySession(telemetrySessionId, {
            status: "failure",
            errorKind: error instanceof FailoverError ? "model_failure" : "unknown",
