    ...
    enqueueGlobal(async () => {
      ...
      try {
        const fallbackResult = await runWithModelFallback({
          ...
          onError: async ({ provider, model, error }) => {
            log.warn(
              `Model ${provider}/${model} failed: ${error instanceof Error ? error.message : String(error)}. Trying fallback...`,
            );
          },
        });

        const finalResult = fallbackResult.result;
        // Update the result meta with the actual provider/model if different
        if (finalResult.meta?.agentMeta) {
          finalResult.meta.agentMeta.provider = fallbackResult.provider;
          finalResult.meta.agentMeta.model = fallbackResult.model;
        }
        return finalResult;
      } catch (err) {
        throw err;
