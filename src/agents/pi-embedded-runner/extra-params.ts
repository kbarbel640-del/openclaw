...
function createProviderHandlerRegistry(): Map<string, ProviderHandler[]> {
  ...
  registry.set("siliconflow", [
    {
      enabledWhen: (context) => shouldApplySiliconFlowThinkingOffCompat(context),
      createWrapper: (baseStreamFn) => createSiliconFlowThinkingWrapper(baseStreamFn),
    },
  ]);
  ...
  registry.set("*", [
    {
      createWrapper: (baseStreamFn, context) =>
        createGoogleThinkingPayloadWrapper(baseStreamFn, context.thinkingLevel),
    },
    {
