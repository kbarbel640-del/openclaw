...
describe("dependency-manager", () => {
  ...
  describe("selectInstallSpec", () => {
    ...
    it("returns undefined when no specs are available", () => {
      ...
      const platform = { os: "win32" as NodeJS.Platform, preferredManager: "winget" as const, availableManagers: ["winget" as const] };
    ...
    it("falls back to download spec when no manager matches", () => {
      const downloadSpec: SkillInstallSpec = { kind: "download", url: "https://example.com/file.zip" };
      const brewSpec: SkillInstallSpec = { kind: "brew", formula: "test" };
    ...
    it("includes specs without OS restriction", () => {
      ...
      const platform = { os: "linux" as NodeJS.Platform, preferredManager: "apt" as const, availableManagers: ["apt" as const, "npm" as const] };
