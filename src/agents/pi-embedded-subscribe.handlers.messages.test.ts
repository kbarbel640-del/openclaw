...
describe("handleMessageUpdate – text_end streaming suppression", () => {
  it("suppresses block reply flush at text_end when message already contains tool calls", () => {
    const ctx = buildMinimalCtx({
      assistantTexts: [],
      assistantTextBaseline: 0,
    });
    ctx.state.blockReplyBreak = "text_end";
    (ctx as unknown as Record<string, unknown>).blockChunking = { minChars: 10, maxChars: 100 };
    ...
    handleMessageUpdate(ctx, {
      ...
      message: {
        ...
        content: [
          ...
          { type: "toolCall", id: "tc-early", name: "web_search", arguments: {} },
        ],
      },
      assistantMessageEvent: { type: "text_end", delta: "", content: "" },
    } as unknown as Parameters<typeof handleMessageUpdate>[1]);

    // flushBlockReplyBuffer should NOT have been called because tool calls are visible
    expect(ctx.flushBlockReplyBuffer).not.toHaveBeenCalled();
  });

  it("allows block reply flush at text_end when message has no tool calls", () => {
    const ctx = buildMinimalCtx({
      assistantTexts: [],
      assistantTextBaseline: 0,
    });
    ...
    // flushBlockReplyBuffer SHOULD be called for normal text-only messages
    expect(ctx.flushBlockReplyBuffer).toHaveBeenCalled();
  });
});
