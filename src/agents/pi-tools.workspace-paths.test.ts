...
import { createAmigoCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("amigo-ws-", async (workspaceDir) => {
      await withTempDir("amigo-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createAmigoCodingTools({ workspaceDir });
    ...
    await withTempDir("amigo-ws-", async (workspaceDir) => {
      await withTempDir("amigo-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello amigo");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("amigo-ws-", async (workspaceDir) => {
      const tools = createAmigoCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("amigo-ws-", async (workspaceDir) => {
      await withTempDir("amigo-override-", async (overrideDir) => {
        const tools = createAmigoCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("amigo-sandbox-", async (sandboxDir) => {
      await withTempDir("amigo-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createAmigoCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
