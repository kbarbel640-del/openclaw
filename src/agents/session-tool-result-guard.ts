...
/**
 * Sanitize tool_use/toolCall/functionCall IDs in an assistant message so that
 * only provider-safe characters (`[a-zA-Z0-9_-]`) are persisted to the session
 * JSONL.  Without this, compaction summaries generated by LLMs can contain
 * human-readable IDs like `"functions.message:0"` that violate provider
 * constraints and corrupt the session (see #15621).
 */
function sanitizeAssistantToolCallIds(message: AgentMessage): AgentMessage {
  ...
  const next = content.map((block) => {
    ...
    // Accept IDs that already conform to the safe pattern (alphanumeric + underscore + hyphen).
    if (/^[a-zA-Z0-9_-]+$/.test(id)) {
      return block;
    }
    const sanitized = sanitizeToolCallId(id);
    if (sanitized === id) {
      return block;
    }
    changed = true;
    return { ...(block as unknown as Record<string, unknown>), id: sanitized };
  });
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage) => {
    ...
    if (role === "assistant") {
      ...
      // Sanitize tool call IDs before persistence to prevent invalid characters
      // (spaces, dots, colons) from being written to session JSONL (#15621).
      nextMessage = sanitizeAssistantToolCallIds(sanitized[0]);
    }
