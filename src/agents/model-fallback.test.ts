...
describe("runWithModelFallback", () => {
  ...
  it("keeps configured fallback chain when an override model fails", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
  ...
  it("uses configured fallback chain for override credential validation errors", async () => {
    const cfg = makeCfg();
    const run = createOverrideFailureRun({
      overrideProvider: "anthropic",
  ...
  it("falls back on unknown model errors", async () => {
    ...
    // Override model failed with model_not_found → continues through configured fallbacks.
    expect(result.result).toBe("ok");
    expect(run).toHaveBeenCalledTimes(2);
    expect(run.mock.calls[1]?.[0]).toBe("anthropic");
    expect(run.mock.calls[1]?.[1]).toBe("claude-haiku-3-5");
