...
function resolveExecConfig(cfg: ClawdbrainConfig | undefined) {
...
export function createClawdbrainCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createClawdbrainReadTool(freshReadTool)];
    }
