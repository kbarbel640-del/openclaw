...
/**
 * Maximum number of send attempts per queue item before it is dropped.
 * Prevents infinite retry loops when `sendAnnounce` keeps throwing (#19197).
 */
...
function scheduleAnnounceDrain(key: string) {
  ...
  void (async () => {
    try {
      ...
      // Increment attempt counter on the failing item and drop it if exhausted.
      ...
      if (failedItem) {
        ...
        if (failedItem._sendAttempts >= MAX_SEND_ATTEMPTS_PER_ITEM) {
          ...
          // Keep item in queue and retry after debounce.
          queue.lastEnqueuedAt = Date.now();
          defaultRuntime.error?.(`announce queue drain failed for ${key} (attempt ${failedItem._sendAttempts}/${MAX_SEND_ATTEMPTS_PER_ITEM}): ${String(err)}`);
        }
      } else {
