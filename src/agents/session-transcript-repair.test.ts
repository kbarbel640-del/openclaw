...
describe("sanitizeToolUseResultPairing", () => {
  ...
  it("strips tool_use blocks from errored assistant messages instead of creating synthetic results", () => {
    const input = [
      {
        role: "assistant",
        content: [
    ...
    const assistant = result.messages[0] as { role: string; content: Array<{ type: string }> };
    expect(assistant.role).toBe("assistant");
    expect(assistant.content.every((b) => b.type !== "toolCall")).toBe(true);
    expect(assistant.content[0]).toEqual({ type: "text", text: "let me try" });
  });
