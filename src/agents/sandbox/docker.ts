...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "amigo.sandbox=1");
  args.push("--label", `amigo.sessionKey=${params.scopeKey}`);
  args.push("--label", `amigo.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `amigo.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("amigo.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`amigo sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`amigo sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("amigo sandbox recreate --all");
}
