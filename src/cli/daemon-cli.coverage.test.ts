...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    AMIGO_STATE_DIR: process.env.AMIGO_STATE_DIR,
    AMIGO_CONFIG_PATH: process.env.AMIGO_CONFIG_PATH,
    AMIGO_GATEWAY_PORT: process.env.AMIGO_GATEWAY_PORT,
    AMIGO_PROFILE: process.env.AMIGO_PROFILE,
  };

  beforeEach(() => {
    process.env.AMIGO_STATE_DIR = "/tmp/amigo-cli-state";
    process.env.AMIGO_CONFIG_PATH = "/tmp/amigo-cli-state/amigo.json";
    delete process.env.AMIGO_GATEWAY_PORT;
    delete process.env.AMIGO_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.AMIGO_STATE_DIR !== undefined) {
      process.env.AMIGO_STATE_DIR = originalEnv.AMIGO_STATE_DIR;
    } else {
      delete process.env.AMIGO_STATE_DIR;
    }

    if (originalEnv.AMIGO_CONFIG_PATH !== undefined) {
      process.env.AMIGO_CONFIG_PATH = originalEnv.AMIGO_CONFIG_PATH;
    } else {
      delete process.env.AMIGO_CONFIG_PATH;
    }

    if (originalEnv.AMIGO_GATEWAY_PORT !== undefined) {
      process.env.AMIGO_GATEWAY_PORT = originalEnv.AMIGO_GATEWAY_PORT;
    } else {
      delete process.env.AMIGO_GATEWAY_PORT;
    }

    if (originalEnv.AMIGO_PROFILE !== undefined) {
      process.env.AMIGO_PROFILE = originalEnv.AMIGO_PROFILE;
    } else {
      delete process.env.AMIGO_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        AMIGO_PROFILE: "dev",
        AMIGO_STATE_DIR: "/tmp/amigo-daemon-state",
        AMIGO_CONFIG_PATH: "/tmp/amigo-daemon-state/amigo.json",
        AMIGO_GATEWAY_PORT: "19001",
      },
