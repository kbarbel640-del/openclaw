...
${formatHelpExamples([
  ['amigo agent --to +15555550123 --message "status update"', "Start a new session."],
  ['amigo agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'amigo agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'amigo agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['amigo agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.amigo.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.amigo.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['amigo agents set-identity --agent main --name "Amigo" --emoji "🦞"', "Set name + emoji."],
  ["amigo agents set-identity --agent main --avatar avatars/amigo.png", "Set avatar path."],
  [
    "amigo agents set-identity --workspace ~/.amigo/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "amigo agents set-identity --identity-file ~/.amigo/workspace/IDENTITY.md --agent main",
