...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("amigo node install"), formatCliCommand("amigo node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("amigo node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    AMIGO_LOG_PREFIX: baseEnv.AMIGO_LOG_PREFIX ?? "node",
