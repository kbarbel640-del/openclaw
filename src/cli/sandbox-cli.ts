...
const SANDBOX_EXAMPLES = {
  main: [
    ["amigo sandbox list", "List all sandbox containers."],
    ["amigo sandbox list --browser", "List only browser containers."],
    ["amigo sandbox recreate --all", "Recreate all containers."],
    ["amigo sandbox recreate --session main", "Recreate a specific session."],
    ["amigo sandbox recreate --agent mybot", "Recreate agent containers."],
    ["amigo sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["amigo sandbox list", "List all sandbox containers."],
    ["amigo sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["amigo sandbox recreate --all", "Recreate all containers."],
    ["amigo sandbox recreate --session main", "Recreate a specific session."],
    ["amigo sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["amigo sandbox recreate --browser --all", "Recreate only browser containers."],
    ["amigo sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["amigo sandbox explain", "Show effective sandbox config."],
    ["amigo sandbox explain --session agent:main:main", "Explain a specific session."],
    ["amigo sandbox explain --agent work", "Explain an agent sandbox."],
    ["amigo sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.amigo.ai/cli/sandbox")}\n`,
    )
    .action(() => {
