    ...
    .action(async (command: string[], opts: ElevateOptions) => {
      ...
      if (grantStatus.status !== "valid") {
        if (opts.json) {
          ...
          defaultRuntime.error(
            theme.muted(`Hint: Use ${theme.command(`openclaw elevate exec <totp> ${command.join(" ")}`)} to create one`),
          );
        }
        defaultRuntime.exit(1);
        return;
      ...
      const remaining = grantStatus.status === "valid" ? Math.ceil(grantStatus.remaining / 60000) : 0;
      if (!opts.json) {
        defaultRuntime.log(
