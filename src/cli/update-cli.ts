...
import { resolveAmigoPackageRoot } from "../infra/amigo-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "amigo doctor entry": "Checking doctor entrypoint",
  "amigo doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "amigo";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const AMIGO_REPO_URL = "https://github.com/amigo/amigo.git";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("amigo@")) {
    return trimmed.slice("amigo@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "amigo.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("amigo completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.AMIGO_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", AMIGO_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `AMIGO_GIT_DIR points at a non-git directory: ${params.dir}. Set AMIGO_GIT_DIR to an empty folder or an amigo checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`AMIGO_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Amigo update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Amigo..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Amigo install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("amigo doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("amigo gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g amigo@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g amigo@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.AMIGO_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.AMIGO_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("amigo gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("amigo doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("amigo gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("amigo gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `amigo update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `AMIGO_GIT_DIR points at a non-git directory: ${gitDir}. Set AMIGO_GIT_DIR to an empty folder or an amigo checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via AMIGO_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Amigo to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["amigo update", "Update a source checkout (git)"],
        ["amigo update --channel beta", "Switch to beta channel (git + npm)"],
        ["amigo update --channel dev", "Switch to dev channel (git + npm)"],
        ["amigo update --tag beta", "One-off update to a dist-tag or version"],
        ["amigo update --no-restart", "Update without restarting the service"],
        ["amigo update --json", "Output result as JSON"],
        ["amigo update --yes", "Non-interactive (accept downgrade prompts)"],
        ["amigo update wizard", "Interactive update wizard"],
        ["amigo --update", "Shorthand for amigo update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run amigo update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.amigo.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["amigo update status", "Show channel + version status."],
          ["amigo update status --json", "JSON output."],
          ["amigo update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.amigo.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
