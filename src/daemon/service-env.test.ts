...
describe("buildMinimalServicePath", () => {
  ...
  it("excludes nonexistent directories when filterExisting is true", () => {
    const result = buildMinimalServicePath({
      platform: "linux",
      env: { HOME: "/home/__nonexistent_test_user__" },
      filterExisting: true,
    });
    const parts = splitPath(result, "linux");
    // All user dirs under a fake HOME should be filtered out
    expect(parts.every((p) => !p.includes("__nonexistent_test_user__"))).toBe(true);
    // System dirs that exist on CI should still be present
    expect(parts.some((p) => p === "/usr/bin" || p === "/bin")).toBe(true);
  });

  it("keeps all candidate directories when filterExisting is false", () => {
    const result = buildMinimalServicePath({
      platform: "linux",
      env: { HOME: "/home/__nonexistent_test_user__" },
    });
    const parts = splitPath(result, "linux");
    expect(parts.some((p) => p.includes("__nonexistent_test_user__"))).toBe(true);
  });
});
