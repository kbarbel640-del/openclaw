...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.AMIGO_GATEWAY_PORT).toBe("18789");
    expect(env.AMIGO_GATEWAY_TOKEN).toBe("secret");
    expect(env.AMIGO_SERVICE_MARKER).toBe("amigo");
    expect(env.AMIGO_SERVICE_KIND).toBe("gateway");
    expect(typeof env.AMIGO_SERVICE_VERSION).toBe("string");
    expect(env.AMIGO_SYSTEMD_UNIT).toBe("amigo-gateway.service");
    if (process.platform === "darwin") {
      expect(env.AMIGO_LAUNCHD_LABEL).toBe("ai.amigo.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", AMIGO_PROFILE: "work" },
      port: 18789,
    });
    expect(env.AMIGO_SYSTEMD_UNIT).toBe("amigo-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.AMIGO_LAUNCHD_LABEL).toBe("ai.amigo.work");
    }
  });
});
