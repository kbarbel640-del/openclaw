...
type BuildServicePathOptions = MinimalServicePathOptions & {
  env?: Record<string, string | undefined>;
  /** When true, exclude directories that do not exist on disk. */
...
export function buildMinimalServicePath(options: BuildServicePathOptions = {}): string {
  ...
  let parts = getMinimalServicePathPartsFromEnv({ ...options, env });
  if (options.filterExisting) {
    parts = parts.filter((dir) => existsSync(dir));
  }
  return parts.join(path.posix.delimiter);
}
...
export function buildServiceEnvironment(params: {
  ...
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env, filterExisting: true }),
