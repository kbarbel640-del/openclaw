...
describe("canvas host state dir defaults", () => {
  ...
  it("uses AMIGO_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Amigo Canvas");
    } finally {
      await handler.close();
