...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("amigoCanvasA2UIAction");
    expect(out).toContain("amigoSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("amigoSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.amigoA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__amigo__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("amigo-a2ui-host");
      expect(html).toContain("amigoCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__amigo__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("amigoA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
