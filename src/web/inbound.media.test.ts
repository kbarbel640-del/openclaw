...
describe("web inbound media saves with extension", () => {
  ...
  it("stores image extension, extracts caption mentions, and keeps document filename", async () => {
    ...
    realSock.ev.emit("messages.upsert", {
      ...
      messages: [
        {
          key: { id: "img1", fromMe: false, remoteJid: "[redacted-email]" },
    ...
    expect(second.mentionedJids).toEqual(["[redacted-email]"]);
    ...
    realSock.ev.emit("messages.upsert", {
      ...
      messages: [
        {
          key: { id: "doc1", fromMe: false, remoteJid: "[redacted-email]" },
  ...
  it("passes mediaMaxMb to saveMediaBuffer", async () => {
    ...
    const upsert = {
      ...
      messages: [
        {
          key: { id: "img3", fromMe: false, remoteJid: "[redacted-email]" },
          message: { imageMessage: { mimetype: "image/jpeg" } },
