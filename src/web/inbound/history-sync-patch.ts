...
export function patchHistorySyncHandling(sock: WASocket): void {
  ...
  sock.ev.on("messages.upsert", async (update) => {
    for (const msg of update.messages) {
      ...
      if (
        protocolMsg.type ===
        (proto.Message.ProtocolMessage.Type.PEER_DATA_OPERATION_REQUEST_RESPONSE_MESSAGE as any)
      ) {
        ...
        logger.debug({ stanzaId: response.stanzaId }, "Received peer data operation response");
        ...
        for (const result of response.peerDataOperationResult || []) {
          ...
          if (historyResponse) {
            ...
            process.stdout.write(
              `     Request ID: ${historyResponse.requestMetadata?.requestId}\n`,
            );
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      const handler = (data: any) => {
        ...
        const messages = (data.messages || []).filter((msg: any) => msg.key?.remoteJid === chatJid);
