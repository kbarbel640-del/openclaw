...
describe("web monitor inbox", () => {
  ...
  it("resolves LID JIDs using Baileys LID mapping store", async () => {
    ...
    sock.signalRepository.lidMapping.getPNForLID.mockResolvedValueOnce("999:[redacted-email]");
    const upsert = buildMessageUpsert({
      id: "abc",
      remoteJid: "999@lid",
      text: "ping",
  ...
  it("resolves group participant LID JIDs via Baileys mapping", async () => {
    ...
    const { listener, sock } = await startInboxMonitor(onMessage);
    const getPNForLID = vi.spyOn(sock.signalRepository.lidMapping, "getPNForLID");
    sock.signalRepository.lidMapping.getPNForLID.mockResolvedValueOnce("444:[redacted-email]");
    const upsert = buildMessageUpsert({
      id: "abc",
      remoteJid: "[redacted-email]",
      participant: "444@lid",
  ...
  it("does not block follow-up messages when handler is pending", async () => {
    ...
    const upsert = {
      ...
      messages: [
        {
          key: { id: "abc1", fromMe: false, remoteJid: "[redacted-email]" },
          message: { conversation: "ping" },
          messageTimestamp: 1_700_000_000,
        },
        {
          key: { id: "abc2", fromMe: false, remoteJid: "[redacted-email]" },
