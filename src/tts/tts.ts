...
export function resolveTtsConfig(cfg: OpenClawConfig): ResolvedTtsConfig {
  ...
  return {
    ...
    qwen: {
      enabled: raw.qwen?.enabled ?? false,
      command: raw.qwen?.command?.trim() || DEFAULT_QWEN_COMMAND,
      args: raw.qwen?.args ?? [],
      model: raw.qwen?.model?.trim() || undefined,
      voice: raw.qwen?.voice?.trim() || undefined,
      instruct: raw.qwen?.instruct?.trim() || undefined,
      speed: raw.qwen?.speed,
      language: raw.qwen?.language?.trim() || undefined,
      temperature: raw.qwen?.temperature,
      cfgScale: raw.qwen?.cfgScale,
      ddpmSteps: raw.qwen?.ddpmSteps,
      maxTokens: raw.qwen?.maxTokens,
...
function resolveQwenOutputFormat(
  ...
  return config.outputFormat ?? DEFAULT_QWEN_OUTPUT_FORMAT;
}
...
function resolveQwenCommand(command: string): string {
...
function resolveQwenJsonInput(raw: string): string | null {
...
const TTS_PROVIDER_FALLBACKS = ["openai", "elevenlabs", "edge"] as const;
export const TTS_PROVIDERS = [...TTS_PROVIDER_FALLBACKS, "qwen"] as const;

export function resolveTtsProviderOrder(primary: TtsProvider): TtsProvider[] {
  if (primary === "qwen") {
    return ["qwen", ...TTS_PROVIDER_FALLBACKS];
  }
  return [primary, ...TTS_PROVIDER_FALLBACKS.filter((provider) => provider !== primary)];
}
      ...
      try {
        switch (key) {
          ...
          case "qwenvoice":
            ...
            overrides.qwen = { ...overrides.qwen, voice: rawValue };
            break;
          case "qwen_instruct":
          case "qweninstruct":
          case "qwen_emotion":
          case "qwenemotion":
            ...
            overrides.qwen = { ...overrides.qwen, instruct: rawValue };
            break;
          case "qwen_speed":
          case "qwenspeed":
            if (!policy.allowVoiceSettings) {
            ...
            }
            {
              const value = parseNumberValue(rawValue);
              if (value == null) {
                warnings.push("invalid qwen speed value");
                break;
              }
              overrides.qwen = { ...overrides.qwen, speed: value };
            }
            break;
          case "qwen_language":
          case "qwenlanguage":
            ...
            overrides.qwen = { ...overrides.qwen, language: rawValue };
            break;
          case "qwen_temperature":
          case "qwentemperature":
            if (!policy.allowVoiceSettings) {
              break;
            }
            {
              const value = parseNumberValue(rawValue);
              if (value == null) {
                warnings.push("invalid qwen temperature value");
                break;
              }
              overrides.qwen = { ...overrides.qwen, temperature: value };
            }
            break;
          case "qwen_cfg_scale":
          case "qwencfgscale":
            ...
            }
            {
              const value = parseNumberValue(rawValue);
              if (value == null) {
                warnings.push("invalid qwen cfgScale value");
                break;
              }
              overrides.qwen = { ...overrides.qwen, cfgScale: value };
            }
            break;
          case "qwen_ddpm_steps":
          case "qwenddpmsteps":
            if (!policy.allowVoiceSettings) {
              break;
            }
            {
              ...
              if (!Number.isFinite(value)) {
                warnings.push("invalid qwen ddpmSteps value");
                break;
              }
              overrides.qwen = { ...overrides.qwen, ddpmSteps: value };
            }
            break;
          case "qwen_max_tokens":
          case "qwenmaxtokens":
            if (!policy.allowVoiceSettings) {
              break;
            }
            {
              const value = Number.parseInt(rawValue, 10);
              if (!Number.isFinite(value)) {
                warnings.push("invalid qwen maxTokens value");
                break;
              }
              overrides.qwen = { ...overrides.qwen, maxTokens: value };
            }
            break;
          case "qwen_model":
          case "qwenmodel":
            ...
            overrides.qwen = { ...overrides.qwen, model: rawValue };
            break;
          default:
            break;
        }
...
async function qwenTTS(params: {
  ...
  outputFormat: "ogg" | "mp3" | "wav";
  config: ResolvedTtsConfig["qwen"];
  overrides?: TtsDirectiveOverrides["qwen"];
  timeoutMs: number;
}): Promise<void> {
  const { text, outputPath, outputFormat, config, overrides, timeoutMs } = params;
  const command = resolveQwenCommand(config.command);
  if (!command) {
    throw new Error("Qwen TTS command is not configured");
  }
  const resolvedRefAudio = config.refAudio ? resolveUserPath(config.refAudio) : undefined;
  const jsonInput = resolveQwenJsonInput(text);
  const args: string[] = [];

  const model = overrides?.model ?? config.model;
  const voice = overrides?.voice ?? config.voice;
  const instruct = overrides?.instruct ?? config.instruct;
  const speed = overrides?.speed ?? config.speed;
  const language = overrides?.language ?? config.language;
  const temperature = overrides?.temperature ?? config.temperature;
  const cfgScale = overrides?.cfgScale ?? config.cfgScale;
  const ddpmSteps = overrides?.ddpmSteps ?? config.ddpmSteps;
  ...
  if (cfgScale != null) {
    args.push("--cfg-scale", String(cfgScale));
  }
  if (ddpmSteps != null) {
    args.push("--ddpm-steps", String(ddpmSteps));
  }
  if (maxTokens != null) {
    args.push("--max-tokens", String(maxTokens));
  }
  if (resolvedRefAudio) {
    args.push("--ref-audio", resolvedRefAudio);
  }
  if (config.refText) {
    args.push("--ref-text", config.refText);
  }

  args.push("--format", outputFormat, "--output", outputPath);
  ...
  if (result.code !== 0) {
    ...
    throw new Error(`Qwen TTS failed${stderr ? `: ${stderr}` : stdout ? `: ${stdout}` : ""}`);
  }
  if (!existsSync(outputPath)) {
    throw new Error("Qwen TTS did not produce output audio");
  }
}
...
export async function textToSpeech(params: {
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      if (provider === "qwen") {
        if (!config.qwen.enabled) {
          lastError = "qwen: disabled";
          continue;
        }
        const tempDir = mkdtempSync(path.join(tmpdir(), "tts-"));
        const outputFormat = resolveQwenOutputFormat(channelId, config.qwen);
        const audioPath = path.join(tempDir, `voice-${Date.now()}.${outputFormat}`);
        try {
          await qwenTTS({
            text: params.text,
            outputPath: audioPath,
...
export async function textToSpeechTelephony(params: {
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      if (provider === "qwen") {
        lastError = "qwen: unsupported for telephony";
        continue;
      }
