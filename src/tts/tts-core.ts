...
export async function qwenTTS(params: {
  ...
  try {
    // 1. Initiate TTS request to get audio URL
    const reqBody: {
      model: string;
      input: { text: string };
      parameters: {
    ...
    // Map intuitive language to language_type parameter if not Auto
    if (language && language.toLowerCase() !== "auto") {
      reqBody.parameters.language_type = language;
    }
    // Only pass instruction if specified (mainly for instruct models)
    if (instruction) {
      reqBody.parameters.instruction = instruction;
    }
    ...
    const audioUrl = initData?.output?.audio_url ?? initData?.output?.audio?.url;
    if (!audioUrl) {
      throw new Error(
