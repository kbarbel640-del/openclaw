...
import { scheduleGatewaySigusr1Restart, triggerAmigoRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Amigo in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerAmigoRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Amigo via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
