...
export function buildInboundUserContextPrefix(ctx: TemplateContext): string {
  ...
  const conversationInfo: Record<string, any> = {
    conversation_label: safeTrim(ctx.ConversationLabel),
    group_subject: subject,
    group_channel: channel === subject ? undefined : channel, // Dedupe
  ...
  if (!isDirect) {
    ...
    if (label) {
      ...
      const senderInfo: Record<string, any> = { label };
      // Only add others if they differ from label or provide unique info
      if (name && name !== label) {
        senderInfo.name = name;
      }
      if (username && username !== label && username.toLowerCase() !== label.toLowerCase()) {
        senderInfo.username = username;
      }
      if (tag && tag !== label && tag !== username) {
        senderInfo.tag = tag;
      }
      if (e164) {
  ...
  if (threadBody) {
    blocks.push(
      `Thread starter (untrusted, for context):\n\`\`\`json\n${JSON.stringify({ body: threadBody.slice(0, 500) })}\n\`\`\``,
    );
  }
  ...
  if (ctx.ReplyToBody) {
    blocks.push(
      `Replied message (untrusted, for context):\n\`\`\`json\n${JSON.stringify({
        ...
        body: ctx.ReplyToBody.slice(0, 1000), // Cap length
      })}\n\`\`\``,
    );
  }
