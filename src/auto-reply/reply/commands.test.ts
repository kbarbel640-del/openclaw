...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-commands-"));
...
function buildParams(commandBody: string, cfg: AmigoConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
