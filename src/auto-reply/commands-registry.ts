...
import type { AmigoConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: AmigoConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AmigoConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as AmigoConfig),
...
export function getCommandDetection(_cfg?: AmigoConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AmigoConfig) {
