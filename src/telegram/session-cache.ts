...
import { loadSessionStore, saveSessionStore, withSessionStoreLock } from "../config/sessions/store.js";
    ...
    await withSessionStoreLock(storePath, async () => {
      ...
      for (const [sessionKey, entry] of Object.entries(store)) {
        if (entry.lastTo === String(params.chatId) && entry.lastThreadId != null) {
          log.debug(`Clearing stale thread ID for session ${sessionKey} targeting ${params.chatId}`);
