...
const mockGrantsDir = path.join(os.tmpdir(), `openclaw-grants-test-${Date.now()}`);
...
describe("grants module", () => {
  ...
  describe("writeGrant", () => {
    ...
    test("creates grants directory if it doesn't exist", async () => {
      ...
      const exists = await fs.access(grantPath).then(() => true).catch(() => false);
  ...
  describe("listGrants", () => {
    ...
    test("returns only valid grants", async () => {
      ...
      expect(grants.map(g => g.name).sort()).toEqual(["valid1", "valid2"]);
    ...
    test("automatically cleans up expired grants", async () => {
      ...
      const exists = await fs.access(expiredPath).then(() => true).catch(() => false);
      expect(exists).toBe(false);
    });
  ...
  describe("cleanupExpiredGrants", () => {
    test("returns count of revoked grants", async () => {
      ...
      const expiredTime = Math.floor(Date.now() / 1000) - 60;
      await fs.writeFile(
        path.join(getGrantsDir(), "expired1.grant"),
        String(expiredTime),
        "utf8"
      );
      await fs.writeFile(
        path.join(getGrantsDir(), "expired2.grant"),
        String(expiredTime),
        "utf8"
      );
