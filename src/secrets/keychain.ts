 ...
 */
function execWithStdin(command: string, input: string, options?: { shell?: string }): Promise<{ stdout: string; stderr: string }> {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      else reject(new Error(`Command failed (exit ${code}): ${stderr}`));
    });
...
async function keychainDeleteLinux(name: string): Promise<void> {
  try {
    await execAsync(
      `secret-tool clear service "${SERVICE_NAME}" account "${sanitize(name)}"`,
      { encoding: "utf8" }
    );
  } catch {
    // Ignore errors
...
async function keychainSetWindows(name: string, value: string): Promise<void> {
  ...
  await execAsync(
    `cmdkey /generic:"${targetName}" /user:openclaw /pass:"${escapedValue}"`,
    { encoding: "utf8" }
  );
}
