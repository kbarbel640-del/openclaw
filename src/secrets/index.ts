...
async function getTotpSecret(): Promise<string> {
  ...
  if (!secret) {
    throw new Error(
      "TOTP not configured. Run 'openclaw secrets setup-totp' first.",
    );
  }
  return secret;
}
...
export async function getSecret(name: string): Promise<string> {
  ...
  if (value === null) {
    throw new Error(`Secret '${name}' not found in ${vault.name}`);
  }
  ...
  await auditLog({
    ...
    name,
    timestamp: Date.now(),
    details: { tier, backend: vault.name },
  });
...
export async function setSecret(
  ...
  try {
    ...
    if (!config.secrets.registry) config.secrets.registry = [];
    
    // Remove existing entry if present
    config.secrets.registry = config.secrets.registry.filter((e: any) => e.name !== name);
    ...
    const vault = getVaultBackend();
    console.warn(
      `Note: Secret stored in ${vault.name} but registry not persisted to config: ` + (err as Error).message,
    );
  }
}
