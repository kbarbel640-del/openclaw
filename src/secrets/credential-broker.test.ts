...
vi.mock("./index.js", () => ({
  getSecret: vi.fn(async (name: string) => {
    if (name === "test_token") return "secret-value-123";
    if (name === "api_key") return "api-key-456";
...
describe("CredentialBroker", () => {
  ...
  describe("resolve", () => {
    ...
    test("throws when secret not found", async () => {
      const broker = new CredentialBroker(config);
      await expect(broker.resolve("secret:nonexistent")).rejects.toThrow("Secret not found: nonexistent");
    });
  });
  ...
  describe("inject", () => {
    ...
    test("deep clones params (original unchanged)", async () => {
      ...
      // Original unchanged
      expect(original).toEqual({ command: "echo test", auth: { credentialRef: "secret:test_token" } });
