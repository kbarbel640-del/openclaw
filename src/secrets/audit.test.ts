...
// Mock STATE_DIR to use temp directory for tests
const mockDataDir = join(os.tmpdir(), `openclaw-audit-test-${Date.now()}`);
  ...
  describe("auditLog", () => {
    ...
    test("handles multiple entries correctly", async () => {
      const entries: AuditEntry[] = [
        ...
        { event: "credential_denied", name: "secret3", timestamp: 3000, details: { reason: "expired" } },
      ];
