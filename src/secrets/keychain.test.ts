...
describe("keychain module", () => {
  ...
  describe("keychainSet - macOS", () => {
    ...
    test("constructs correct macOS add command", async () => {
      ...
      const addCall = mockedExec.mock.calls.find(call => 
        (call[0] as string).includes("add-generic-password")
      );
      
      expect(addCall).toBeDefined();
      const command = addCall![0] as string;
    ...
    test("passes value via stdin on macOS", async () => {
      mockedExec.mockImplementation((cmd, options, callback) => {
        if (typeof callback === "function") {
          callback(null, { stdout: "", stderr: "" } as any, "");
        }
      ...
      const options = addCall![1] as any;
      
      expect(options.input).toBe("secret123");
    });
  });
  ...
  describe("keychainSet - Linux", () => {
    ...
    test("stores secret using secret-tool", async () => {
      mockedExec.mockImplementation((cmd, options, callback) => {
        if (typeof callback === "function") {
          callback(null, { stdout: "", stderr: "" } as any, "");
        }
      ...
      expect(options.input).toBe("linux-value");
    });
  });
  ...
  describe("keychainGet - Windows", () => {
    ...
    test("retrieves secret using PowerShell CredRead", async () => {
      ...
      expect(command).toContain("CredRead");
  ...
  describe("keychainSet - Windows", () => {
    ...
    test("passes value via stdin on Windows", async () => {
      mockedExec.mockImplementation((cmd, options, callback) => {
        if (typeof callback === "function") {
          callback(null, { stdout: "", stderr: "" } as any, "");
        }
      ...
      const setCall = mockedExec.mock.calls.find(call => 
        (call[0] as string).includes("cmdkey /generic")
      );
      
      expect(setCall).toBeDefined();
      const options = setCall![1] as any;
      expect(options.input).toBe("secret456");
    });
  });
  ...
  describe("name sanitization", () => {
    ...
    test("handles empty string after sanitization", async () => {
      ...
      const call = mockedExec.mock.calls[0];
      const command = call[0] as string;
      // All dangerous chars stripped, should leave empty or minimal chars
