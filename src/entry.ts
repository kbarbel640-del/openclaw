...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.AMIGO_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.AMIGO_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.AMIGO_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[amigo] ${parsed.error}`);
    process.exit(2);
  }
