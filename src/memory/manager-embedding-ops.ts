...
export abstract class MemoryManagerEmbeddingOps extends MemoryManagerSyncOps {
   ...
   */
  private readonly perDbLocks = new Map<DatabaseSync, Promise<void>>();
  ...
  protected async withDbContext<T>(params: {
    ...
    try {
      ...
      releaseGlobal();
      release();
      // Clean up resolved per-DB locks to prevent memory leaks
      const currentLock = this.perDbLocks.get(targetDb);
      if (currentLock) {
        void currentLock.then(() => {
          if (this.perDbLocks.get(targetDb) === currentLock) {
