...
export class BrainMcpClient {
  ...
  async quickSearch(params: {
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      log.warn(`quick_search failed after ${duration}ms: ${error}`);
      throw error;
    }
  }
  ...
  async unifiedSearch(params: {
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      log.warn(`unified_search failed after ${duration}ms: ${error}`);
      throw error;
    }
  }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      log.debug(`healthCheck failed: ${error}`);
      return false;
    }
  }
  ...
  private async callTool(selector: string, args: string[]): Promise<string> {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes("ETIMEDOUT")) {
        throw new Error(`Brain MCP request timed out after ${this.timeoutMs}ms`);
      }
      throw error;
    }
  }
