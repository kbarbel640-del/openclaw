...
describe("normalizeE164 & toWhatsappJid", () => {
  it("strips formatting and prefixes", () => {
    ...
    expect(toWhatsappJid("whatsapp:+555 123 4567")).toBe("[redacted-email]");
  });

  it("preserves existing JIDs", () => {
    expect(toWhatsappJid("[redacted-email]")).toBe("[redacted-email]");
    expect(toWhatsappJid("whatsapp:[redacted-email]")).toBe("[redacted-email]");
    expect(toWhatsappJid("[redacted-email]")).toBe("[redacted-email]");
  });
});
...
describe("jidToE164", () => {
  ...
  it("maps @hosted.lid from authDir mapping files", () => {
    ...
    expect(jidToE164("[redacted-email]", { authDir })).toBe("+4440001");
    fs.rmSync(authDir, { recursive: true, force: true });
  });
...
describe("resolveJidToE164", () => {
  it("resolves @lid via lidLookup when mapping file is missing", async () => {
    const lidLookup = {
      getPNForLID: vi.fn().mockResolvedValue("777:[redacted-email]"),
    };
  ...
  it("skips lidLookup for non-lid JIDs", async () => {
    const lidLookup = {
      getPNForLID: vi.fn().mockResolvedValue("888:[redacted-email]"),
    };
    await expect(resolveJidToE164("[redacted-email]", { lidLookup })).resolves.toBe("+888");
