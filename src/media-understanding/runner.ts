...
async function runAttachmentEntries(params: {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      console.warn(`[media-runner] ${capability} understanding failed for entry ${entryType}: ${String(err)}`);
      if (shouldLogVerbose()) {
...
export async function runCapability(params: {
  ...
  if (resolvedEntries.length === 0) {
    console.warn(`[media-runner] ${capability} skipped: no provider entries resolved (no API key or no config)`);
    return {
      outputs: [],
      decision: {
        capability,
