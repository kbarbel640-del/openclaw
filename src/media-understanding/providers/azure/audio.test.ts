...
describe("transcribeAzureAudio", () => {
  it("uses api-key auth for Azure OpenAI cognitiveservices endpoints", async () => {
    ...
    expect(seenUrl).toBe(
      "https://oc-01-resource.cognitiveservices.azure.com/openai/deployments/whisper/audio/transcriptions?api-version=2024-12-01-preview",
    );
  ...
  it("builds deployment transcription URL from bare endpoint", async () => {
    ...
    expect(getRequest().url).toBe(
      "https://oc-01-resource.services.ai.azure.com/openai/deployments/gpt-4o-mini-transcribe/audio/transcriptions?api-version=2024-12-01-preview",
    );
  });
});
