...
function setSlackDmPolicy(cfg: AmigoConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Amigo";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Amigo`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: AmigoConfig, allowFrom: string[]): AmigoConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
