...
homepage: https://docs.amigo.ai/hooks#command-logger
metadata:
  {
    "amigo":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.amigo/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.amigo/logs/commands.log ~/.amigo/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/amigo`:
   ```
   /home/username/.amigo/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
