...
This directory contains hooks that ship with Amigo. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.amigo.ai/hooks/soul-evil
...
Hooks can be configured in `~/.amigo/amigo.json`:
...
2. Restart gateway: `pkill -9 -f 'amigo.*gateway' && pnpm amigo gateway`
3. Enable the hook: `amigo hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.amigo.ai/hooks
