...
`;
    ...
    expect(parsed.amigo.emoji).toBe("💾");
    expect(parsed.amigo.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.amigo.emoji).toBe("📝");
    expect(parsed.amigo.events).toEqual(["command"]);
    expect(parsed.amigo.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.amigo.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"amigo": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"amigo": {"events": ["test"]}}');
  });
...
describe("resolveAmigoMetadata", () => {
  it("extracts amigo metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveAmigoMetadata(frontmatter);
  ...
  it("returns undefined when amigo key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        amigo: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Amigo" },
            { id: "npm", kind: "npm", package: "@amigo/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@amigo/hook");
  });
...
`;
    ...
    const amigo = resolveAmigoMetadata(frontmatter);
    expect(amigo).toBeDefined();
    expect(amigo?.emoji).toBe("💾");
    expect(amigo?.events).toEqual(["command:new"]);
    expect(amigo?.requires?.config).toEqual(["workspace.dir"]);
    expect(amigo?.install?.[0].kind).toBe("bundled");
  });
