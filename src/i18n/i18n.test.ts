...
describe("i18n System", () => {
  ...
  describe("Basic functionality", () => {
    ...
    it("should substitute {placeholder} parameters", () => {
      expect(t("commands.dock.description", { dock: "telegram" })).toBe(
        "Switch to telegram for replies.",
      );

      setLocale("uk");
      expect(t("commands.dock.description", { dock: "telegram" })).toBe(
        "Перемкнутися на telegram для відповідей.",
      );
    });
    ...
    it("should have no empty string translations", () => {
      for (const locale of allLocaleFiles) {
        ...
        for (const key of keys) {
          ...
          expect(typeof value, `Non-string value found for key "${key}" in ${locale}.json`).toBe(
            "string",
          );
        }
      }
  ...
  describe("Locale-specific functionality tests", () => {
    it("should return non-empty string for 'commands.help.description' in every locale", () => {
      ...
      for (const locale of locales) {
        ...
        expect(description.length, `Too short description for locale "${locale}"`).toBeGreaterThan(
          0,
        );
        expect(description, `Description equals key for locale "${locale}"`).not.toBe(
    ...
    it("should handle nested key access correctly in all locales", () => {
      ...
      for (const locale of locales) {
        ...
        expect(skillName, `Nested key returned key itself for locale "${locale}"`).not.toBe(
          "commands.skill.args.name",
        );
        ...
        expect(helpTitle, `Help title returned key itself for locale "${locale}"`).not.toBe(
          "help.title",
        );
      }
    });
    ...
    it("should handle empty parameters object gracefully", () => {
      ...
      for (const locale of locales) {
        ...
        expect(() => t("commands.help.description", null as unknown)).not.toThrow();
      }
    });
  ...
  describe("getLocaleDisplayNames()", () => {
    ...
    it("should not contain locales that are not available", () => {
      ...
      for (const key of Object.keys(names)) {
        expect(
          locales.has(key as Locale),
          `Extra locale "${key}" in display names but not in availableLocales()`,
        ).toBe(true);
      }
    });
    ...
    it("should have exactly the same keys as availableLocales()", () => {
      const names = getLocaleDisplayNames();
      expect(Object.keys(names).toSorted()).toEqual(availableLocales().toSorted());
    });
  });

  describe("Error handling and edge cases", () => {
    it("should handle deeply nested missing keys gracefully", () => {
      expect(t("very.deeply.nested.key.that.does.not.exist.anywhere")).toBe(
        "very.deeply.nested.key.that.does.not.exist.anywhere",
      );
    });
    ...
    it("should handle undefined and null values in parameters", () => {
      ...
      expect(() => t("system.model_changed", { model: undefined as unknown })).not.toThrow();

      // Test with null parameter value
      expect(() => t("system.model_changed", { model: null as unknown })).not.toThrow();
    });
  });
});
