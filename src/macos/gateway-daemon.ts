...
declare const __AMIGO_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __AMIGO_VERSION__ === "string" && __AMIGO_VERSION__) ||
  process.env.AMIGO_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `amigo --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.AMIGO_GATEWAY_PORT ??
    process.env.AMIGO_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.AMIGO_GATEWAY_BIND ??
    process.env.AMIGO_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
