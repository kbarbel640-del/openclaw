...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAmigoAgentDir: vi.fn().mockReturnValue("/tmp/amigo-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/amigo-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/amigo-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAmigoAgentDir: mocks.resolveAmigoAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAmigoAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/amigo-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/amigo-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/amigo-agent-custom");
