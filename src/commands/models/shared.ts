...
export async function updateConfig(
  mutator: (cfg: AmigoConfig) => AmigoConfig,
): Promise<AmigoConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: AmigoConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AmigoConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("amigo agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
