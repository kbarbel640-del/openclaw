...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAmigoAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("amigo models auth setup-token")}\` or \`${formatCliCommand("amigo configure")}\`.`
          : `Run \`${formatCliCommand("amigo configure")}\` or set an API key env var.`;
