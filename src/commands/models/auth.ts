...
import { readConfigFileSnapshot, type AmigoConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: AmigoConfig, model: string): AmigoConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("amigo plugins install")}\`.`,
    );
  }
