...
const SEARCH_TOOL = "https://docs.amigo.ai/mcp.SearchAmigo";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.amigo.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('amigo docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.amigo.ai/");
      runtime.log(`Search: ${formatCliCommand('amigo docs "your query"')}`);
    }
    return;
  }
