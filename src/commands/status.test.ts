...
beforeAll(() => {
  previousProfile = process.env.AMIGO_PROFILE;
  process.env.AMIGO_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.AMIGO_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/amigo-root.js", () => ({
  resolveAmigoPackageRoot: vi.fn().mockResolvedValue("/tmp/amigo"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/amigo/pnpm-lock.yaml",
      markerPath: "/tmp/amigo/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Amigo status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("amigo status --all") ||
          l.includes("amigo --profile isolated status --all") ||
          l.includes("amigo status --all") ||
          l.includes("amigo --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.AMIGO_GATEWAY_TOKEN = "abcd1234";
    try {
