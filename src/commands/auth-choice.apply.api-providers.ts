...
import {
  ...
  applyPollinationsProviderConfig,
...
export async function applyAuthChoiceApiProviders(
  ...
  if (
    ...
    } else if (params.opts.tokenProvider === "pollinations") {
      authChoice = "pollinations";
    }
  }
  ...
  if (authChoice === "pollinations") {
    ...
    if (!hasCredential && params.opts?.token && params.opts?.tokenProvider === "pollinations") {
      await setPollinationsApiKey(normalizeApiKeyInput(params.opts.token), params.agentDir);
      hasCredential = true;
    }
    ...
    if (!hasCredential) {
      await params.prompter.note(
        [
          "Pollinations.ai provides AI API access.",
    ...
    const envKey = resolveEnvApiKey("pollinations");
    if (envKey) {
      const useExisting = await params.prompter.confirm({
        message: `Use existing POLLINATIONS_API_KEY (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
        initialValue: true,
      });
      if (useExisting) {
        await setPollinationsApiKey(envKey.apiKey, params.agentDir);
        hasCredential = true;
      }
    }
    if (!hasCredential) {
      ...
      await setPollinationsApiKey(normalizeApiKeyInput(String(key)), params.agentDir);
    }
    nextConfig = applyAuthProfileConfig(nextConfig, {
    ...
    });
    {
      const applied = await applyDefaultModelChoice({
        ...
        applyDefaultConfig: applyPollinationsConfig,
        applyProviderConfig: applyPollinationsProviderConfig,
        noteDefault: POLLINATIONS_DEFAULT_MODEL_REF,
        noteAgentModel,
        prompter: params.prompter,
      });
      nextConfig = applied.config;
