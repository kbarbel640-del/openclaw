...
const SIMPLE_API_KEY_PROVIDER_FLOWS: Partial<Record<AuthChoice, SimpleApiKeyProviderFlow>> = {
  ...
  "clawpane-api-key": {
    ...
    expectedProviders: ["clawpane"],
    envLabel: "CLAWPANE_API_KEY",
    promptMessage: "Enter Clawpane API key",
    setCredential: setClawpaneApiKey,
    defaultModel: CLAWPANE_DEFAULT_MODEL_REF,
    applyDefaultConfig: applyClawpaneConfig,
    applyProviderConfig: applyClawpaneProviderConfig,
    noteDefault: CLAWPANE_DEFAULT_MODEL_REF,
    noteMessage: [
      "Clawpane is a smart LLM router that automatically selects the best model for each request.",
      "Get your API key at: https://clawpane.co/dashboard",
      "Default model: clawpane/auto (balanced). Also available: fast, economy, quality.",
    ].join("\n"),
    noteTitle: "Clawpane",
  },
  "kilocode-api-key": {
