...
const requireAgentDir = () => {
  const agentDir = process.env.AMIGO_AGENT_DIR;
  if (!agentDir) {
    throw new Error("AMIGO_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.AMIGO_STATE_DIR;
  const previousAgentDir = process.env.AMIGO_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.AMIGO_AGENT_DIR;
    } else {
      process.env.AMIGO_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.AMIGO_OAUTH_DIR;
  });

  it("writes auth-profiles.json under AMIGO_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-oauth-"));
    process.env.AMIGO_STATE_DIR = tempStateDir;
    process.env.AMIGO_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AMIGO_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to AMIGO_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-minimax-"));
    process.env.AMIGO_STATE_DIR = tempStateDir;
    process.env.AMIGO_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AMIGO_AGENT_DIR;
