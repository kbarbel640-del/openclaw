...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.AMIGO_GATEWAY_PASSWORD,
    disableConfigCache: process.env.AMIGO_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("AMIGO_STATE_DIR", prev.stateDir);
  restoreEnvVar("AMIGO_CONFIG_PATH", prev.configPath);
  restoreEnvVar("AMIGO_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("AMIGO_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("AMIGO_SKIP_CRON", prev.skipCron);
  restoreEnvVar("AMIGO_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("AMIGO_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("AMIGO_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("AMIGO_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.AMIGO_DISABLE_CONFIG_CACHE = "1";
  ...
  const configPath = path.join(tempHome, "amigo.json");
  process.env.HOME = tempHome;
  process.env.AMIGO_STATE_DIR = tempHome;
  process.env.AMIGO_CONFIG_PATH = configPath;
  vi.resetModules();
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("amigo-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("amigo-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("amigo-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("amigo-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("amigo-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("amigo-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
