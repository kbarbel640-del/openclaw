...
describe("doctor config flow", () => {
  ...
  it("converts numeric discord ids to strings on repair", async () => {
    await withTempHome(async (home) => {
      ...
      expect(workAccount.dm).toBeDefined();
      expect(workAccount.execApprovals).toBeDefined();
      expect(workAccount.guilds).toBeDefined();
      ...
      expect(workAccount.guilds?.["200"]?.users).toEqual(["999"]);
      expect(workAccount.guilds?.["200"]?.roles).toEqual(["1010"]);
      expect(workAccount.guilds?.["200"]?.channels?.help?.users).toEqual(["1111"]);
      expect(workAccount.guilds?.["200"]?.channels?.help?.roles).toEqual(["1212"]);
    });
  });
