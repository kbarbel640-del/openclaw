...
beforeEach(() => {
  ...
  resolveAmigoPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.AMIGO_STATE_DIR;
  originalUpdateInProgress = process.env.AMIGO_UPDATE_IN_PROGRESS;
  process.env.AMIGO_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "amigo-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.AMIGO_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.AMIGO_UPDATE_IN_PROGRESS;
  } else {
    process.env.AMIGO_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveAmigoPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadAmigoPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/amigo-root.js", () => ({
  resolveAmigoPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "amigo-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.AMIGO_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when AMIGO_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/amigo.json",
      exists: true,
      raw: "{}",
    ...
    process.env.AMIGO_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
