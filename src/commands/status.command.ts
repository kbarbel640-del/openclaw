...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Amigo status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("amigo security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("amigo security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.amigo.ai/faq");
  runtime.log("Troubleshooting: https://docs.amigo.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("amigo status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("amigo logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("amigo status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("amigo gateway probe")}`);
  }
}
