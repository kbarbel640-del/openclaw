...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.AMIGO_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "amigo");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.AMIGO_STATE_DIR = stateDir;
    process.env.AMIGO_CONFIG_PATH = path.join(stateDir, "amigo.json");
