...
export function listAgentEntries(cfg: AmigoConfig): AgentEntry[] {
...
function resolveAgentName(cfg: AmigoConfig, agentId: string) {
...
function resolveAgentModel(cfg: AmigoConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AmigoConfig): AgentSummary[] {
