...
describe("resolveRequestedLoginProviderOrThrow", () => {
  ...
  it("lists built-in choices when provided", () => {
    const providers = [makeProvider({ id: "google-gemini-cli" })];

    expect(() =>
      resolveRequestedLoginProviderOrThrow(providers, "unknown-provider", [
        "openai-codex",
        "token",
      ]),
    ).toThrowError(
      'Unknown provider "unknown-provider". Available providers: google-gemini-cli, openai-codex, token. Verify plugins via `openclaw plugins list --json`.',
    );
  });
});
