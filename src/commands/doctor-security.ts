...
import type { AmigoConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: AmigoConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("amigo security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("amigo configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("amigo config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("amigo doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("amigo config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
