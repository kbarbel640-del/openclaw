...
import { useNewExecutionLayer, createDefaultExecutionKernel } from "../execution/index.js";
import {
...
export async function agentCommand(
  ...
  // Feature flag: use new ExecutionKernel when enabled for CLI entry point
  if (useNewExecutionLayer(cfg, "cli")) {
    return runAgentWithKernel(opts, body, cfg, runtime, deps);
  }
...
// ---------------------------------------------------------------------------
// New Execution Layer Path (Phase 6)
// ---------------------------------------------------------------------------

/**
 * Run the agent command using the new ExecutionKernel.
 *
 * This replaces the ~190 lines of runtime selection, session updates, and
 * event emission in the old path with a single kernel.execute() call.
 * The kernel handles runtime resolution, turn execution, state persistence,
 * and event routing internally.
 *
 * Feature flag: execution.useNewLayer.cli
 */
async function runAgentWithKernel(
  opts: AgentCommandOpts,
  body: string,
  cfg: OpenClawConfig,
  runtime: RuntimeEnv,
  ...
  // --- Agent ID validation (shared with old path) ---
  const agentIdOverrideRaw = opts.agentId?.trim();
  const agentIdOverride = agentIdOverrideRaw ? normalizeAgentId(agentIdOverrideRaw) : undefined;
  if (agentIdOverride) {
    const knownAgents = listAgentIds(cfg);
