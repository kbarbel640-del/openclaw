...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "AMIGO_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("AMIGO_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv AMIGO_GATEWAY_TOKEN");
    expect(message).not.toContain("AMIGO_GATEWAY_PASSWORD");
  });
