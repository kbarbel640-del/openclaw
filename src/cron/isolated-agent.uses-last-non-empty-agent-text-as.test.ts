...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "amigo-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".amigo", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<AmigoConfig> = {},
): AmigoConfig {
  const base: AmigoConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "amigo"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".amigo", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
