...
describe("Cron issue regressions", () => {
  ...
  it("#17554: run() clears stale runningAtMs and executes the job", async () => {
    const store = await makeStorePath();
    const now = Date.parse("2026-02-06T10:05:00.000Z");
    // STUCK_RUN_MS = 2 * 60 * 60 * 1000 = 7_200_000
    ...
    // Do not call start() — it clears all runningAtMs markers on startup,
    // which would hide the bug.  Call run() directly to verify that the
    // stale marker is cleared by recomputeNextRunsForMaintenance.
