...
export function triggerAmigoRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.AMIGO_SYSTEMD_UNIT,
        process.env.AMIGO_PROFILE,
      );
  ...
  const label =
    process.env.AMIGO_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.AMIGO_PROFILE);
