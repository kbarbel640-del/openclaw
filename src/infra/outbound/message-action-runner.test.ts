...
describe("runMessageAction context isolation", () => {
  ...
  it("allows WhatsApp send when target matches current chat", async () => {
    const result = await runMessageAction({
      ...
      params: {
        channel: "whatsapp",
        target: "group:[redacted-email]",
        message: "hi",
      },
      toolContext: { currentChannelId: "[redacted-email]" },
      dryRun: true,
    });
  ...
  it("blocks WhatsApp send when target differs from current chat", async () => {
    const result = await runMessageAction({
      ...
      params: {
        channel: "whatsapp",
        target: "[redacted-email]",
        message: "hi",
      },
      toolContext: { currentChannelId: "[redacted-email]", currentChannelProvider: "whatsapp" },
      dryRun: true,
    });
