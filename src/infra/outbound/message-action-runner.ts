...
function resolveTelegramAutoThreadId(params: {
  ...
  // Only block auto-injection when we are certain a chat is a DM.
  // Unknown forms (e.g. @usernames/links) are typically groups/channels and
  // should remain eligible for topic routing.
  const targetChatType = resolveTelegramTargetChatType(params.to);
  const sourceChatType = resolveTelegramTargetChatType(context.currentChannelId);
  if (targetChatType === "direct" || sourceChatType === "direct") {
    return undefined;
  }
  return context.currentThreadTs;
}
