...
describe("withExtractedArchiveRoot", () => {
  ...
  const tempDir = path.join(tmpRoot, "openclaw-install-flow");
  const archivePath = path.join(tmpRoot, "plugin.tgz");
  const extractDir = path.join(tempDir, "extract");
  const packedRootDir = path.join(extractDir, "package");
  ...
  it("extracts archive and passes root directory to callback", async () => {
    const withTempDirSpy = vi
      ...
      .mockImplementation(async (_prefix, fn) => await fn(tempDir));
    ...
    const resolveRootSpy = vi
      ...
      .mockResolvedValue(packedRootDir);
