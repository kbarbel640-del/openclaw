...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.AMIGO_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*amigo-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "amigo";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "amigo");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "amigo-gw");
  ...
  records.push(`_amigo-gw._tcp IN PTR ${instanceLabel}._amigo-gw._tcp`);
  records.push(`${instanceLabel}._amigo-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._amigo-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; amigo-content-hash: ${contentHash}\n${contentBody}`;
}
