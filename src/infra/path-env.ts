...
function candidateBinDirs(opts: EnsureAmigoPathOpts): string[] {
  ...
  // Bundled macOS app: `amigo` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "amigo");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/amigo` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "amigo"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `amigo` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAmigoCliOnPath(opts: EnsureAmigoPathOpts = {}) {
  if (isTruthyEnvValue(process.env.AMIGO_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.AMIGO_PATH_BOOTSTRAPPED = "1";
