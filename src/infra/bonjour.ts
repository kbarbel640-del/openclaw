...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.AMIGO_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Amigo";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Amigo\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.AMIGO_MDNS_HOSTNAME?.trim() ||
    process.env.AMIGO_MDNS_HOSTNAME?.trim() ||
    "amigo";
  const hostname =
    hostnameRaw
