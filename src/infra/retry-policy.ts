  ...
  evaluate(error: unknown, hasChangedInput: boolean): RetryOutcome {
    ...
    if (escalationCheck.shouldEscalate) {
      ...
      return {
        ...
        action: escalationCheck.action ?? EscalationAction.ASK_USER,
        failedResult: failureRecord,
      };
    }
...
// ============================================================================
// Legacy Platform Retry Runners (Discord / Telegram)
// ============================================================================

/** A function that wraps an async operation with retry logic. */
...
export function createTelegramRetryRunner(params: {
  ...
  const shouldRetry = params.shouldRetry
    ? (err: unknown) =>
        (params.shouldRetry?.(err) ?? false) || TELEGRAM_RETRY_RE.test(formatErrorMessage(err))
