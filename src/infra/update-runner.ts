...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "amigo.mjs");
    ...
    const doctorStep = await runStep(
      step("amigo doctor", doctorArgv, gitRoot, { AMIGO_UPDATE_IN_PROGRESS: "1" }),
    );
