...
import { discoverAmigoPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AmigoPluginDefinition;
  register?: AmigoPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AmigoPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AmigoPluginDefinition;
...
export function loadAmigoPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAmigoPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "amigo/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AmigoPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as AmigoPluginModule;
    } catch (err) {
