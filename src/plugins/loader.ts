...
import { discoverClawdbrainPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ClawdbrainPluginDefinition;
  register?: ClawdbrainPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ClawdbrainPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ClawdbrainPluginDefinition;
...
export function loadClawdbrainPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverClawdbrainPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            "clawdbrain/plugin-sdk": pluginSdkAlias,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: ClawdbrainPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as ClawdbrainPluginModule;
    } catch (err) {
