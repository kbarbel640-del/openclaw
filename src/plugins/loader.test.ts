...
import { loadAmigoPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.AMIGO_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `amigo-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "amigo.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.AMIGO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadAmigoPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.AMIGO_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadAmigoPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadAmigoPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
