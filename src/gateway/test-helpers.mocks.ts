...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `clawdbrain-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.CLAWDBRAIN_CONFIG_PATH = path.join(root, "clawdbrain.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "clawdbrain.json");
