...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.AMIGO_SKIP_CHANNELS;
    prevSkipGmail = process.env.AMIGO_SKIP_GMAIL_WATCHER;
    process.env.AMIGO_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.AMIGO_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.AMIGO_SKIP_GMAIL_WATCHER;
    } else {
      process.env.AMIGO_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
