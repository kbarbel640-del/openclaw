...
import type { AmigoConfig, ModelProviderConfig } from "../config/types.js";
import { resolveAmigoAgentDir } from "../agents/agent-paths.js";
...
import { ensureAmigoModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.AMIGO_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.AMIGO_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.AMIGO_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<AmigoConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.AMIGO_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.AMIGO_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveAmigoAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-live-state-"));
  process.env.AMIGO_STATE_DIR = tempStateDir;
  ...
  process.env.AMIGO_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.amigo-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveAmigoAgentDir();
  const sanitizedCfg: AmigoConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Amigo live tool probe (local, safe): " +
    ...
    process.env.AMIGO_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.AMIGO_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureAmigoModelsJson(cfg);
      ...
      const rawModels = process.env.AMIGO_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.amigo-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
