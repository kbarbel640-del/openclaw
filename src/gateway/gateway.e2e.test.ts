...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.AMIGO_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "amigo");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.amigo-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".amigo");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "amigo.json");
      ...
      try {
        ...
        process.env.AMIGO_CONFIG_PATH = prev.configPath;
        process.env.AMIGO_GATEWAY_TOKEN = prev.token;
        process.env.AMIGO_SKIP_CHANNELS = prev.skipChannels;
        process.env.AMIGO_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.AMIGO_SKIP_CRON = prev.skipCron;
        process.env.AMIGO_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.AMIGO_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.AMIGO_STATE_DIR = prev.stateDir;
      process.env.AMIGO_CONFIG_PATH = prev.configPath;
      process.env.AMIGO_GATEWAY_TOKEN = prev.token;
      process.env.AMIGO_SKIP_CHANNELS = prev.skipChannels;
      process.env.AMIGO_SKIP_GMAIL_WATCHER = prev.skipGmail;
