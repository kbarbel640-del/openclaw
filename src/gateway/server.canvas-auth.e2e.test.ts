...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.AMIGO_CONFIG_PATH;
  const prevDisableCache = process.env.AMIGO_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "amigo-canvas-auth-test-"));
  const configPath = path.join(dir, "amigo.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.AMIGO_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.AMIGO_DISABLE_CONFIG_CACHE;
    } else {
      process.env.AMIGO_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
