...
const execHostEnforced = process.env.AMIGO_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.AMIGO_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureAmigoCliOnPath({ pathEnv: process.env.PATH ?? "" });
