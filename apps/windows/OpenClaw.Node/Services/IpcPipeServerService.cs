...
namespace OpenClaw.Node.Services
{
    public sealed class IpcPipeServerService : IDisposable
    {
        ...
        private async Task<IpcResponse> DispatchAsync(IpcRequest req, CancellationToken ct)
        {
            ...
            if (string.Equals(method, "ipc.dev.update", StringComparison.OrdinalIgnoreCase))
            {
                var p = req.Params ?? default;
                ...
                {
                    if (p.TryGetProperty("repoPath", out var rp) && rp.ValueKind == JsonValueKind.String)
                    {
                        repoPath = (rp.GetString() ?? string.Empty).Trim();
                    }
                    if (p.TryGetProperty("branch", out var br) && br.ValueKind == JsonValueKind.String)
                    {
                        branch = (br.GetString() ?? branch).Trim();
                    }
                    if (p.TryGetProperty("build", out var b) && (b.ValueKind == JsonValueKind.True || b.ValueKind == JsonValueKind.False))
                    {
                ...
                var probe = await Run("git-probe", "git", "rev-parse", "--is-inside-work-tree");
                if (probe.ExitCode != 0)
                {
                    return new IpcResponse { Id = req.Id ?? string.Empty, Ok = false, Payload = new { ok = false, repoPath, branch, steps }, Error = new IpcError { Code = "UNAVAILABLE", Message = "git repo probe failed" } };
                }

                var fetch = await Run("git-fetch", "git", "fetch", "origin", branch);
                if (fetch.ExitCode != 0)
                {
                    return new IpcResponse { Id = req.Id ?? string.Empty, Ok = false, Payload = new { ok = false, repoPath, branch, steps }, Error = new IpcError { Code = "UNAVAILABLE", Message = "git fetch failed" } };
                }

                var checkout = await Run("git-checkout", "git", "checkout", branch);
                if (checkout.ExitCode != 0)
                {
                    return new IpcResponse { Id = req.Id ?? string.Empty, Ok = false, Payload = new { ok = false, repoPath, branch, steps }, Error = new IpcError { Code = "UNAVAILABLE", Message = "git checkout failed" } };
                }

                var pull = await Run("git-pull", "git", "pull", "--ff-only", "origin", branch);
                if (pull.ExitCode != 0)
                {
                    return new IpcResponse { Id = req.Id ?? string.Empty, Ok = false, Payload = new { ok = false, repoPath, branch, steps }, Error = new IpcError { Code = "UNAVAILABLE", Message = "git pull failed" } };
                }

                if (doBuild)
                {
                    var build = await Run("dotnet-build", "dotnet", "build", "apps/windows/OpenClaw.Node/OpenClaw.Node.csproj", "-p:Platform=x64");
                    if (build.ExitCode != 0)
                    {
                        return new IpcResponse { Id = req.Id ?? string.Empty, Ok = false, Payload = new { ok = false, repoPath, branch, steps }, Error = new IpcError { Code = "UNAVAILABLE", Message = "dotnet build failed" } };
                    }
                }
                ...
                return new IpcResponse
                {
                    Id = req.Id ?? string.Empty,
                    Ok = true,
                    Payload = new { ok = true, repoPath, branch, built = doBuild, steps }
                };
            }

            if (string.Equals(method, "ipc.dev.restart", StringComparison.OrdinalIgnoreCase))
            {
                var p = req.Params ?? default;
                var delayMs = 1500;
                var dryRun = false;
                ...
                if (string.IsNullOrWhiteSpace(processPath))
                {
                    return new IpcResponse
                    {
                        Id = req.Id ?? string.Empty,
                        Ok = false,
                        Error = new IpcError { Code = "UNAVAILABLE", Message = "unable to resolve process path" }
                    };
                }
                ...
                if (proc == null)
                {
                    return new IpcResponse
                    {
                        Id = req.Id ?? string.Empty,
                        Ok = false,
                        Error = new IpcError { Code = "UNAVAILABLE", Message = "failed to schedule restart" }
                    };
                }
                ...
                return new IpcResponse
                {
                    Id = req.Id ?? string.Empty,
                    Ok = true,
                    Payload = new { ok = true, scheduled = true, delayMs, pid }
                };
            }
