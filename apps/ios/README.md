...
This is an **alpha** iOS app that connects to an Amigo Gateway as a `role: node`.
...
3. On iOS: Amigo → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project Amigo.xcodeproj -scheme Amigo -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/AmigoKit` contains the shared transport/types used by the iOS app.
