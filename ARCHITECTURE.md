...
Maps sender identities to tiers. Leverages OpenClaw's existing `session.identityLinks` for cross-channel identity resolution and adds tier metadata on top.
...
OpenClaw resolves session via identityLinks
  → Canonical slug (e.g., "ana") or raw sender ID
    │
    ▼
Saint plugin (before_agent_prepare hook, §4.4) resolves tier:
  → Lookup slug in contacts.json → tier
  → Unknown sender → tier: "external"
    │
    ▼
Plugin returns model/tool overrides (before_agent_prepare result):
  → Return tool policy (allow/deny) from tier definition
  → Set model override from tier (e.g., haiku for external, sonnet for owner)
...
Plugin injects prompt context (before_agent_start hook, fires later):
  → Build tier-specific system prompt (filtered boot files via agent:bootstrap)
  → Inject tier context via prependContext ("You are talking to Ana (tier: owner)...")
  → Set exec blocklist for before_tool_call enforcement
  → Set memory pathFilter for tier's memory_scope
  → Set budget cap
```

### 2.4 System Prompt Assembly

Each tier gets a different system prompt. The plugin uses two hooks:
...
1. **`agent:bootstrap` (internal hook)** — Filters the **fixed** boot file set. The bootstrap loader (`workspace.ts:245-274`) seeds exactly 7-9 files: AGENTS.md, SOUL.md, TOOLS.md, IDENTITY.md, USER.md, HEARTBEAT.md, BOOTSTRAP.md, and optional MEMORY.md. `WorkspaceBootstrapFileName` is a TypeScript union type — only these filenames are valid. The hook can **filter down** (remove files) but **cannot add** files like COMPANY.md.
   ...
   **Note on skills:** Skills are loaded separately via `resolveSkillsPromptForRun()`, not through the bootstrap hook — `agent:bootstrap` cannot filter them. This is not a security gap: tool policy (§3.2) hides tools the tier can't use, so skill knowledge about hidden tools is inert (the agent can't act on it). The `skills` field in tier definitions (§2.2) documents intended skill exposure, enforced indirectly through tool visibility.

2. **`before_agent_start` (plugin hook)** — Injects tier-specific context via `prependContext`:
   - "You are talking to Ana (owner). Full access."
   - "You are talking to an unknown contact. Do not reveal internal details."
   - **COMPANY.md content** (for owner/manager tiers) — COMPANY.md is NOT in the fixed bootstrap file set, so it's injected here by reading from the workspace at hook time
...
{"ts":"2026-02-10T14:00:00Z","user":"ana","tier":"owner","tool":"web_search","params":{"query":"client deadline"},"durationMs":340,"estimatedCostUsd":0.004}
{"ts":"2026-02-10T14:00:01Z","user":"ana","tier":"owner","tool":"exec","params":{"command":"gog gmail send ..."},"durationMs":1200,"estimatedCostUsd":0.01}
```
...
`workspaceAccess` is set to `"ro"` in static config for all tiers (default is `"none"`, so this must be set explicitly). This is simpler and more secure than trying to set mount modes dynamically per-session.
...
- **Shadow write/edit tools** operate on the **host filesystem** (the real workspace), enforcing tier-based path restrictions (§5.2). Writes via shadow tools are the only way to modify the real workspace.
...
**Limitation:** The existing per-sender tool filtering (`toolsBySender`) only works in group chats — it requires a `groupId` and returns nothing for DMs. Since Saint's primary use case is DM sessions, we can't use the built-in per-sender policy for tier-based tool filtering.

**What Saint uses:** A new `before_agent_prepare` hook (§4.4 core patch) fires early in `run.ts` — before model resolution and tool creation. The plugin resolves the sender's tier and returns a tool allow/deny policy. OpenClaw applies this filter in `attempt.ts` before `createAgentSession()`, so denied tools never enter the session. External users get `tools.allow: ["web_search", "web_fetch"]` — everything else is invisible. No shadow tools needed for visibility.

**What shadow tools are still needed for:** Adding restrictions _within_ visible tools — path scoping on read/write/edit, URL restrictions on web_fetch/browser, exec blocklist enforcement, and CLI Porter routing (see §5).
...
api.registerTool((ctx, original) => createShadowRead(original!), { name: "read", override: true });
...
The saint plugin injects the pathFilter for every search based on the sender's tier and memory_scope.
...
run.ts (BEFORE resolveModel):
  ...
  │   → Plugin resolves tier from contacts.json
  │   → Plugin returns { model, provider, tools } for this tier
  │
...
**B. Sender identity in hook contexts.** Currently, `before_agent_start`, `before_tool_call`, and the new `before_agent_prepare` hook contexts only include `sessionKey` and `agentId`. The plugin needs sender identity for tier resolution but would have to parse the session key (fragile, format varies by dmScope). Sender data exists at the `run.ts` level (`RunEmbeddedPiAgentParams` has `senderId`, `senderName`, `senderUsername`, `senderE164`) but is **dropped at the run→attempt handoff** — `run.ts:399-454` only passes `senderIsOwner` to `runEmbeddedAttempt`, not the other sender fields, despite `EmbeddedRunAttemptParams` defining them. Separately, hook contexts (`PluginHookAgentContext`, `PluginHookToolContext`, and the `HookContext` in `pi-tools.before-tool-call.ts`) lack sender fields entirely.
...
This lets the plugin do `contacts.findBySlug(ctx.peerId)` instead of parsing session keys. The plugin resolves the tier once in `before_agent_prepare` (for model/tools), caches it, and reuses it in `before_agent_start` (for prompt injection) and `before_tool_call` (for exec blocklist, budget caps).
...
This lets the plugin assign claude-haiku-4-5 to external users and claude-sonnet-4-5 to owners from the same bot config, and control tool visibility per tier — all applied before the session is created.
...
EXISTING FEATURES USED (zero core changes):
  ...
  ├── Tool policy                → per-tier allow/deny via hook return value
  ...
  ├── before_agent_start hook    → per-tier prompt injection (prependContext)
  └── agent:bootstrap hook       → per-tier boot file filtering (SOUL.md, COMPANY.md etc)
...
PLUGIN (saint-orchestrator, ~1000-1500 lines, ALL NEW FILES):
  ...
  ├── Tier resolver               → contacts.json → tier lookup
  ├── Prompt builder             → before_agent_start hook
  ├── Boot file filter           → agent:bootstrap hook
  ├── Tool policy generator      → tier → allow/deny config
  ├── Budget enforcement         → before_tool_call + after_tool_call
  ├── Usage metering             → after_tool_call hook
  ├── Cron tier injection         → before_tool_call hook
  └── Sub-agent tier injection   → before_tool_call hook
```
...
- **Pre-check + call-through** (read, exec, web_fetch, browser) — validate params against tier permissions, then delegate to `original.execute()`. The original's built-in guards (`assertSandboxPath()`, `fetchWithSsrfGuard()`, etc.) are preserved — the shadow adds restrictions on top, not instead of.
- **Post-filter + call-through** (sessions_list, sessions_history) — call `original.execute()`, then filter results by tier.
...
Only needed where the tool policy system (binary allow/deny) is insufficient — i.e., where a tier CAN use a tool but with restrictions.

### 5.1 Shadow Exec (Porter Routing + Blocklist)

Wraps the built-in exec (pre-check pattern) to:

1. Check command against tier's `exec_blocklist` (deterministic regex match)
...
The built-in file tools (read, write, edit, apply_patch) run on the **host process** (not the sandbox), so they have full host filesystem access. Shadow versions add tier-based path restrictions.

**Two different patterns for file tools:**

- **Shadow read** uses the **pre-check + call-through** pattern. The built-in read exists in `"ro"` mode. The shadow wraps it: validate path against tier's allowed read paths, then call `original.execute()`. The original's `assertSandboxPath()` guard is preserved — the shadow adds tier restrictions on top.
- **Shadow write/edit/apply_patch** use the **full replacement** pattern. The built-ins are removed in `"ro"` sandbox mode (§3.1). The factory receives `original: null` and provides the full implementation: tier-based path validation + direct `fs` operations on the host workspace.

**Note on sandbox mount behavior (see §3.1):** With `workspaceAccess: "ro"`, the sandbox has two mounts: a writable **copy** at `/workspace` (bootstrap files + skills only) and the **real workspace** at `/agent` (read-only). Shadow write/edit tools operate on the **host** (real workspace) and enforce tier-based permissions. The exec tool runs in the sandbox and can:
...
**Layer 2: Tier-based path restrictions:**

```yaml
file_access:
  # Tier 1: Platform-managed (deny-write for ALL tiers, only Saint edits)
  platform_protected: [SOUL.md, IDENTITY.md, AGENTS.md, config/tiers.yaml, config/contacts.json]

  # Tier 2 & 3: Per-tier access
  owner:
    read: ["*"]
    write: ["*"]
    deny_write: [SOUL.md, IDENTITY.md, AGENTS.md, config/tiers.yaml, config/contacts.json]
  ...
  employee:
    read:
      ["memory/shared/*", "memory/users/<self>/*", "skills/*", "data/*", "SOUL.md", "IDENTITY.md"]
...
- **Platform files** (SOUL.md, IDENTITY.md, AGENTS.md, config/) — no tier can write. Only Saint edits during provisioning.
- **Owner-writable files** (COMPANY.md, TOOLS.md) — bot can update at owner's request ("update our office hours in company notes").
- **Self-writable memory** (`memory/users/<self>/*`) — any internal tier can ask the bot to remember their preferences. The `<self>` placeholder resolves to the current user's slug at runtime.
...
```
Blocked: file://, data://, localhost, admin.saint.work,
         127.0.0.1, 0.0.0.0, 10.*, 172.16-31.*, 192.168.*, 169.254.*,
         IPv6 loopback (::1), IPv4-mapped IPv6 (::ffff:x.x.x.x),
...
- **Internal tiers (employee/manager):** `before_tool_call` validates the `path` param against the tier's allowed memory paths. If outside scope, returns `{ block: true }` with an error message. If allowed, the call proceeds to the original `memory_get` unmodified. This is just another case in the same handler that enforces exec blocklist and budget caps — no new hook, no new patch.
- **External tier:** `memory_get` is hidden entirely via tool policy (`before_agent_prepare` deny list). It never enters the session.
- **Owner:** unrestricted.
...
Uses the **post-filter + call-through** pattern. Calls `original.execute()`, then filters results by tier:

| Tier     | Scope                                          |
| -------- | ---------------------------------------------- |
| owner    | All sessions (`sessions_scope: "all"`)         |
| manager  | All sessions (`sessions_scope: "all"`)         |
| employee | Only their own (`sessions_scope: "own"`)       |
| external | Tool hidden via policy                         |

Filtering uses `sessions_scope` from the tier config. When `"all"`, all sessions are returned. When `"own"`, sessions are filtered by slug matching on `:` delimiters in session keys (prevents partial slug matches).
...
One small Node.js proxy shim (`cliport-proxy.js`). Uses symlinks + `argv[0]` detection:

```
/usr/local/bin/cliport-proxy  → the actual script shim
...
│   └── contacts.json
├── memory/
│   ├── shared/         business knowledge (all internal tiers)
...
- Inbound: Gmail push notifications (or polling fallback) → session per sender → tier resolution
...
Layer 1: Tool policy (OpenClaw built-in + §4.4 patch)
  → Tools not in tier's allow list are invisible to the agent
...
Layer 6: Session access scoping (shadow tools)
  → sessions_list/history filtered by tier

Layer 7: Boot file filtering (plugin, agent:bootstrap hook)
  → Per-tier workspace file injection

Layer 8: Budget caps (plugin, before/after_tool_call hooks)
  → Per-tier max_budget_usd

Layer 9: Three-tier write protection (shadow write tool)
  → Platform files (SOUL.md, IDENTITY.md, AGENTS.md, config/) — no tier can write, only Saint
...
**Can do:** Use curated tools (per tier), read/write within workspace (per tier), run sandboxed exec, use memory (scoped to tier).
...
| ---------------------- | ------------- | ---------------------------- | -------------------- | ---------- | ---------------------------------------------------------------------------------- |
| `before_agent_prepare` | **no** (§4.4) | model, provider, tool policy | no                   | sequential | Per-tier model + tool visibility (fires early, before session creation)            |
| `before_agent_start`   | **yes**       | prompt, context              | no                   | sequential | Per-tier prompt injection (prependContext)                                         |
| `before_tool_call`     | **yes**       | params                       | yes (`block: true`)  | sequential | Exec blocklist, memory_get path scoping, budget caps, cron/subagent tier injection |
...
| `session_start`        | **no**        | no                           | no                   | parallel   | Not needed — tier resolution via `before_agent_start`                              |
...
| --------------------- | -------------------------------------- | ------- | --------------------------------------------------------- |
| `message_received`    | `dispatch-from-config.ts`              | no      | (not planned — tier resolution uses `before_agent_start`) |
...
| ----------------- | -------------------------- | ---------------------------- |
| `agent:bootstrap` | yes (bootstrapFiles array) | Per-tier boot file filtering |

**Concurrent safety:** Both hook types receive `sessionKey` and `agentId` per-invocation. The plugin must be stateless (session key in → tier out) to handle simultaneous sessions safely.
...
| cliport-proxy shim                                    | New script       | ~150-250 lines (Node.js) | **Zero**                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- [ ] Build saint-orchestrator plugin (tiers, contacts, shadow tools, prompt builder, metering)
- [ ] Build CLI Porter daemon + cliport-proxy shim
...
**I2: Porter streaming + signals** — Length-prefixed frame protocol for stdout/stderr streaming, plus SIGTERM/SIGKILL on socket close. Design settled in §6.8 — main implementation effort is the frame protocol in both Porter daemon and proxy shim.
...
**N1: Sandbox exec read bypass** — With `workspaceAccess: "ro"`, exec can read the full real workspace via the `/agent` mount. This bypasses shadow read-tool path restrictions (e.g. a restricted tier could `cat /agent/config.yaml` even if the shadow read tool blocks that path). Mitigations: (1) exec blocklist regex can deny reads of sensitive paths, (2) the sandbox has no credentials so reading config files yields no secrets, (3) CLI Porter ensures credential-bearing operations are proxy-only. Residual risk is low but should be monitored if tiers have genuinely confidential workspace files.
