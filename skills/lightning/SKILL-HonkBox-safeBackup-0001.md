...
name: lightning
description: >-
  Operate the klabo.world Lightning stack on honkbox (LND + LNbits): status,
  logs, backups, recovery, and integration touchpoints.
invocation: user
arguments: "[status|logs|backup|recover|deploy|help] [focus]"
---

# NOTE: Skill files must start with YAML frontmatter (above) so Codex can load them.
