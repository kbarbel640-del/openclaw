...
| Scroll down     | `axe gesture scroll-down --screen-width 430 --screen-height 932 --udid $UDID` |
...
When logging into Engage or Teams apps in the simulator, use this test account:

```
Email: joel@yamios.onmicrosoft.com
Password: Bug225500
```
...
axe type "joel@yamios.onmicrosoft.com" --udid $UDID
axe tap --id "login_button" --post-delay 2 --udid $UDID
# Wait for password field, then enter password
axe type "Bug225500" --udid $UDID
axe tap --label "Sign in" --post-delay 3 --udid $UDID
```

### App Authentication

**For YammerSDKDemoApp and Engage, use `/injecting-simulator-tokens` for automated token injection instead of manual login.**

| App              | Command                         |
| ---------------- | ------------------------------- |
| YammerSDKDemoApp | `mise run inject-token:restart` |
| Engage           | `mise run inject-token:engage`  |

See `/injecting-simulator-tokens` for full documentation, prerequisites, and troubleshooting.
...
### 3. Gestures (Scroll, Swipe, Back)

```bash
SCREEN_WIDTH=430   # Adjust for device
SCREEN_HEIGHT=932

# Scroll down (to see more content below)
axe gesture scroll-down --screen-width $SCREEN_WIDTH --screen-height $SCREEN_HEIGHT --udid $UDID

# Scroll up (to see content above)
axe gesture scroll-up --screen-width $SCREEN_WIDTH --screen-height $SCREEN_HEIGHT --udid $UDID

# Horizontal scroll
axe gesture scroll-left --screen-width $SCREEN_WIDTH --screen-height $SCREEN_HEIGHT --udid $UDID
axe gesture scroll-right --screen-width $SCREEN_WIDTH --screen-height $SCREEN_HEIGHT --udid $UDID

# Edge swipes (for system gestures)
...
scroll_to_element() {
  ...
  local SCREEN_WIDTH="${4:-430}"
  local SCREEN_HEIGHT="${5:-932}"
