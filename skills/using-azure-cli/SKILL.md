...
description: Use whenever the user provides a link to Azure DevOps (visualstudio.com or dev.azure.com) — PRs, work items, wikis, repos, pipelines, builds, or any ADO URL. Also use for running az commands, ADO scripting, querying VMs/storage/AKS, DevOps work items, or automation. Covers common gotchas (pr show flags, pr images via REST, WIQL queries).
invocation: user
---
...
| Create PR           | `az repos pr create --title "..." --source-branch <branch> --draft true` |
...
# List PRs (REQUIRES -p for project, unlike pr show)
az repos pr list --status active -p engineering --org https://yammer.visualstudio.com -o table

# Find PRs by branch (use --source-branch, NOT --creator which needs full email)
az repos pr list --source-branch user/klabo/my-branch -p engineering --org https://yammer.visualstudio.com -o table
...
**IMPORTANT:** Always create PRs as drafts (`--draft true`). For embedding images in PR descriptions, see [references/common-gotchas.md](references/common-gotchas.md#pr-images---critical).

**IMPORTANT:** `az repos pr show` only accepts `--id` and `--org`. Do NOT pass `-p`, `--project`, or `--repository`.

**IMPORTANT:** `az repos pr list` REQUIRES `-p <project>` (unlike `pr show`). Always pass `-p engineering` for yammer org. Use `--source-branch` to find PRs by branch name — do NOT use `--creator` with a username fragment (it needs a full email/identity and will error otherwise).
...
- `/fixing-themis-gates` - Themis policy gate failures (work item linking for Teams PRs)
