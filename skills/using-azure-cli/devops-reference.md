...
```bash
# Create PR with all options (ALWAYS use --draft true)
az repos pr create \
...
az repos pr update --id 123 --description "New description"  # WARNING: escapes '!' — DON'T use for descriptions with images
...
See [common-gotchas.md](references/common-gotchas.md#pr-images---critical) for the authoritative PR image upload instructions.
