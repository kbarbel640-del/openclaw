...
└───────────────────────┬──────────────────────────────┘
                        ...
                        ▼
┌──────────────────────────────────────────────────────┐
│           finances.xlsx (OneDrive/Finance)            │
│  ├── fact_Transactions                                │
│  ├── dim_Accounts                                     │
│  ├── dim_Categories                                   │
│  └── import_Log                                       │
...
- Downloads CSV to `~/Shared/quicken-imports/YYYY-MM-DD/fidelity/`
...
python3 ~/bin/teller-to-excel.py --fidelity ~/Shared/quicken-imports/$(date +%Y-%m-%d)/fidelity/*.csv
```

State file: `~/Shared/quicken-imports/fidelity-state.json`
...
4. Save to `~/OneDrive/Financial/Inbox/` (inbox)
5. Move CSVs into `~/Shared/quicken-imports/YYYY-MM-DD/river/` for import

### Financial Document Inbox (Manual Docs)

Drop any downloaded PDFs/CSVs (statements, tax forms, manual exports) into:

`~/OneDrive/Financial/Inbox/`
...
```bash
~/Shared/skills/financial-sync/scripts/daily_sync.sh
```

## Excel Workbook Structure

Location: `~/OneDrive/Finance/finances.xlsx`
...
# Check state files
cat ~/Shared/quicken-imports/fidelity-state.json
```
...
```bash
# Edit: crontab -e
0 6 * * * cd ~/projects/teller-sync && source .envrc && python teller_sync.py && ~/bin/teller-to-excel.py >> ~/Shared/quicken-imports/sync.log 2>&1
```
...
- Status dashboard: `~/Shared/skills/financial-sync/scripts/status.py`
- Daily sync: `~/Shared/skills/financial-sync/scripts/daily_sync.sh`
