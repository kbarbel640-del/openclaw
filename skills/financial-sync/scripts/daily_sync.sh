...
QUICKEN_DIR="$HOME/OneDrive/quicken-imports"
FINANCE_INBOX_PRIMARY="$HOME/OneDrive/Finance/Inbox"
FINANCE_INBOX_SECONDARY="$HOME/OneDrive/Financial/Inbox"
FINANCIAL_INBOX="$FINANCE_INBOX_PRIMARY"
TODAY=$(date +%Y-%m-%d)
...
if [[ "$TELLER_ONLY" == "false" ]]; then
    ...
    # Financial Inbox (manual docs)
    # Prefer Finance/Inbox but fall back to Financial/Inbox for legacy path
    if [[ -d "$FINANCE_INBOX_PRIMARY" ]]; then
        FINANCIAL_INBOX="$FINANCE_INBOX_PRIMARY"
    elif [[ -d "$FINANCE_INBOX_SECONDARY" ]]; then
        FINANCIAL_INBOX="$FINANCE_INBOX_SECONDARY"
    fi
    if [[ -d "$FINANCIAL_INBOX" ]]; then
        INBOX_COUNT=$(find "$FINANCIAL_INBOX" -maxdepth 1 -type f \( -name "*.csv" -o -name "*.pdf" -o -name "*.CSV" -o -name "*.PDF" \) 2>/dev/null | wc -l)
        if [[ $INBOX_COUNT -gt 0 ]]; then
            warn "Financial Inbox: $INBOX_COUNT file(s) waiting"
            echo "INBOX: $INBOX_COUNT file(s)" >> "$LOG_FILE"
        else
            success "Financial Inbox: empty"
            echo "INBOX: empty" >> "$LOG_FILE"
        fi

        # If both inbox paths exist, surface files in the secondary location too
        if [[ -d "$FINANCE_INBOX_PRIMARY" && -d "$FINANCE_INBOX_SECONDARY" && "$FINANCIAL_INBOX" == "$FINANCE_INBOX_PRIMARY" ]]; then
            SECONDARY_COUNT=$(find "$FINANCE_INBOX_SECONDARY" -maxdepth 1 -type f \( -name "*.csv" -o -name "*.pdf" -o -name "*.CSV" -o -name "*.PDF" \) 2>/dev/null | wc -l)
            if [[ $SECONDARY_COUNT -gt 0 ]]; then
                warn "Legacy Financial Inbox has $SECONDARY_COUNT file(s) waiting"
                echo "INBOX_LEGACY: $SECONDARY_COUNT file(s)" >> "$LOG_FILE"
            fi
        fi
    else
        warn "Financial Inbox not found: $FINANCIAL_INBOX"
        echo "INBOX: missing dir" >> "$LOG_FILE"
    fi
fi
