...
5. **Capture memory**: After meaningful interactions, add one small detail (story, joke, like/dislike, routine) to `~/Shared/notes/family/`.
...
- Write entries to `~/Shared/notes/family/` (per-person folders) or `FAMILY.md` if it is a shared summary.
- After meaningful interactions, add one concrete detail so the family feels remembered.

## Source Of Truth

- Store family facts, stories, and preferences in `FAMILY.md` or `~/Shared/notes/family/`.
