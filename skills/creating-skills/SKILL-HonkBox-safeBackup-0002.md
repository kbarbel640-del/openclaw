...
```bash
for skill in ~/.claude/skills/*/learnings.md; do
...
```bash
cat >> ~/.claude/skills/creating-skills/improvements.log << EOF
...
View recent improvements: `tail -30 ~/.claude/skills/creating-skills/improvements.log`
...
**IMPORTANT:** After any skill creation, improvement, or audit changes, sync skills to Codex:

```bash
~/.claude/sync-skills.sh
```

Or invoke `/syncing-skills` to sync to local Codex and honkbox.

## Related Skills

- `/syncing-skills` - Sync skills to Codex (local and honkbox)
