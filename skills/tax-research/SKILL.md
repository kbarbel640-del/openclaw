...
1. **All available documents reviewed** - Every document in `~/.openclaw/files/finance/documents/` has been read and analyzed
...
cat ~/.openclaw/files/finance/documents/document-index.json | jq '[.documents[] | select(.status != "reviewed")] | length'

# Check documents needed
cat ~/.openclaw/notes/finance/documents-needed.md
```
...
# Or manually check
find ~/.openclaw/files/finance/documents/ -type f \( -name "*.pdf" -o -name "*.csv" \) | head -50
cat ~/.openclaw/files/finance/documents/document-index.json | jq '.documents[] | select(.status == "new")'
```
...
head -20 ~/.openclaw/notes/finance/Tax\ Savings\ Tracker.md

# Review recent research logs
ls -lt ~/.openclaw/notes/finance/logs/ | head -5
```
...
Append to `~/.openclaw/notes/finance/logs/runs.md`:
...
Update `~/.openclaw/notes/finance/documents-needed.md` with:
...
After each run, add to `~/.openclaw/notes/finance/logs/skill-improvements.md`:
...
Write findings to: `~/.openclaw/notes/finance/research/Research - [Topic] - YYYY-MM-DD.md`
...
| File | Purpose |
|------|---------|
| `~/.openclaw/notes/finance/Tax Savings Tracker.md` | Potential vs Verified savings |
| `~/.openclaw/notes/finance/Tax File Index.md` | Document inventory |
| `~/.openclaw/notes/finance/2024 Self-Filing Checklist.md` | Filing guide |
| `~/.openclaw/notes/finance/documents-needed.md` | Documents to request from user |
| `~/.openclaw/notes/finance/research/` | Research documents |
| `~/.openclaw/notes/finance/logs/` | Run logs |

**Tax Documents:**
| File | Purpose |
|------|---------|
| `~/.openclaw/files/finance/documents/` | Tax documents (PDFs, returns, W2s, 1099s) |
| `~/.openclaw/files/finance/documents/document-index.json` | Structured document index with status |
| `~/.openclaw/files/finance/documents/Tax Returns/` | Filed returns and IRS transcripts |
| `~/.openclaw/files/finance/documents/Crypto/` | Bitcoin/crypto transaction records |
...
- Read specific docs from ~/.openclaw/files/finance/documents/
