...
if [ ! -d "$PRIMARY_DIR" ] && [ -d "$LEGACY_DIR" ]; then
    DOCS_DIR="$LEGACY_DIR"
    INDEX_FILE="$DOCS_DIR/document-index.json"
    echo "Primary dir missing; falling back to $DOCS_DIR"
fi
...
# Build scan directory list (recent tax years + crypto + returns)
SCAN_DIRS=()
for year in 2022 2023 2024 2025 2026; do
  if [ -d "$DOCS_DIR/$year" ]; then
    SCAN_DIRS+=("$DOCS_DIR/$year")
  fi
 done

if [ -d "$DOCS_DIR/Crypto" ]; then
  SCAN_DIRS+=("$DOCS_DIR/Crypto")
fi

if [ -d "$DOCS_DIR/Tax Returns" ]; then
  SCAN_DIRS+=("$DOCS_DIR/Tax Returns")
fi

if [ ${#SCAN_DIRS[@]} -eq 0 ]; then
  echo "No scan directories found in $DOCS_DIR"
  exit 0
fi
...
# Find all tax documents (PDF/CSV only)
FOUND_FILES=$(find "${SCAN_DIRS[@]}" -type f \( -name "*.pdf" -o -name "*.csv" -o -name "*.PDF" -o -name "*.CSV" \) 2>/dev/null | wc -l)
...
find "${SCAN_DIRS[@]}" -type f \( -name "*.pdf" -o -name "*.csv" -o -name "*.PDF" -o -name "*.CSV" \) 2>/dev/null | while read -r filepath; do
