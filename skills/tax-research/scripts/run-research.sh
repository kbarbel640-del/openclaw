...
NOTES_DIR="$HOME/OneDrive/notes/personal/taxes"
...
SCAN_SCRIPT="$SCRIPT_DIR/scan-documents.sh"
...
EOF_SUMMARY
  ...
  run)
...

    {
      ...
      echo "== Document Scan =="
      if [ "$SCAN_SKIP" = "1" ]; then
        echo "Skipping document scan (TAX_SCAN_SKIP=1)"
      else
        if [ -f "$SCAN_SCRIPT" ]; then
          # Some Shared/ mounts are noexec; run via bash to avoid permission issues.
          bash "$SCAN_SCRIPT" || echo "Document scan failed"
        else
