...
def render_front_page(css, ctx):
    ...
    if lead_body:
        ...
        # Capitalize the rest of the first word after drop cap to avoid pronoun confusion
        # e.g., "She" becomes drop-cap "S" + "HE" not "S" + "he"
        first_word_end = first_rest.find(' ')
        if first_word_end > 0:
            first_rest = first_rest[:first_word_end].upper() + first_rest[first_word_end:]
        else:
            first_rest = first_rest.upper()
        lead_body_html = (
