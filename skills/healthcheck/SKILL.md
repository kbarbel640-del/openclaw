---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Amigo deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Amigo cron scheduling for periodic checks, or version status checks on a machine running Amigo (laptop, workstation, Pi, VPS).
---

# Amigo Host Hardening

## Overview

Assess and harden the host running Amigo, then align it to a user-defined risk tolerance without breaking access. Use Amigo security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Amigo changes the host firewall, SSH, or OS updates; it does not.
...
5. Amigo gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Amigo can access sensitive data.
...
### 2) Run Amigo security audits (read-only)

As part of the default read-only checks, run `amigo security audit --deep`. Only offer alternatives if the user requests them:

1. `amigo security audit` (faster, non-probing)
2. `amigo security audit --json` (structured output)

Offer to apply Amigo safe defaults (numbered):

1. `amigo security audit --fix`

Be explicit that `--fix` only tightens Amigo defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Amigo version/update status (read-only)

As part of the default read-only checks, run `amigo update status`.
...
- Credential hygiene notes (location of Amigo creds, prefer disk encryption)
...
After Amigo install or first hardening pass, run at least one baseline audit and version check:

- `amigo security audit`
- `amigo security audit --deep`
- `amigo update status`

Ongoing monitoring is recommended. Use the Amigo cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `amigo cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `amigo update status`
...
Before creating, `amigo cron list` and match on exact `name`. If found, `amigo cron edit <id> ...`.
If not found, `amigo cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `amigo update status` (preferred for source checkouts and channels)
2. `npm view amigo version` (published npm version)

## Amigo command accuracy

Use only supported commands and flags:

- `amigo security audit [--deep] [--fix] [--json]`
- `amigo status` / `amigo status --deep`
- `amigo health --json`
- `amigo update status`
- `amigo cron add|list|runs|run`

Do not invent CLI flags or imply Amigo enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Amigo compaction:
