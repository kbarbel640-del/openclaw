...
def test_classify_pointer_variants():
    assert classify_pointer("[Source: https://example.com]") == "external"
    assert classify_pointer("[Source: User Prompt]") == "internal"
    assert classify_pointer("[Context: thread note]") == "internal"
    assert classify_pointer("[InternalDoc: spec.md]") == "internal"
    assert classify_pointer("[Source: Task A1]") == "dependency"
    assert classify_pointer("no pointer") == "none"


def test_count_sources_separate_internal_external():
    output = (
        "[Source: https://example.com]\\n"
        "[Context: user said revenue]\\n"
        "[Source: Task T-1]\\n"
    )
    counts = count_sources(output)
    assert counts["external_sources_count"] == 1
    assert counts["internal_sources_count"] == 1
    assert counts["dependency_sources_count"] == 1
