---
name: crypto-wallet
description: Use when you need to manage a multi-chain crypto wallet: check balances, send tokens, view transaction history across Ethereum, Solana, Bitcoin, and other chains.
metadata:
  openclaw:
    emoji: 💰
    requires:
      bins: ["curl", "jq"]
---

# Crypto Wallet 💰

Manage cryptocurrency wallets across multiple blockchains.

## Supported Chains

- Ethereum (ETH, ERC-20)
- Solana (SOL, SPL tokens)
- Bitcoin (BTC)
- Polygon (MATIC)
- Arbitrum
- Base

## Features

- Check wallet balances
- View transaction history
- Send tokens (with confirmation)
- Token price lookup
- Portfolio overview

## Usage Examples

```
"What's my ETH balance?"
"Show my crypto portfolio"
"Send 0.1 ETH to 0x..."
```

## Safety Rules

1. **ALWAYS** confirm before sending transactions
2. **NEVER** expose private keys
3. **ALWAYS** verify recipient addresses