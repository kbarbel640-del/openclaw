...
import { AmigoApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = AmigoApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("amigo-app") as AmigoApp;
...
beforeEach(() => {
  AmigoApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__AMIGO_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  AmigoApp.prototype.connect = originalConnect;
  window.__AMIGO_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/amigo/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/amigo");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/amigo/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__AMIGO_CONTROL_UI_BASE_PATH__ = "/amigo";
    const app = mountApp("/amigo/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/amigo");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/amigo/sessions");
  });
