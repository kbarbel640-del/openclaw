...
function renderEntry(entry: PresenceEntry) {
  const lastInput =
    entry.lastInputSeconds != null
      ? formatRelativeTimestamp(Date.now() - entry.lastInputSeconds * 1000)
      : t("common.na");
  const mode = entry.mode ?? t("states.unknown");
  const roles = Array.isArray(entry.roles) ? entry.roles.filter(Boolean) : [];
  const scopes = Array.isArray(entry.scopes) ? entry.scopes.filter(Boolean) : [];
  const scopesLabel =
    scopes.length > 0
      ? scopes.length > 3
        ? t("instances.scopesCount", { count: String(scopes.length) })
  ...
  return html`
    <div class="list-item">
      ...
      <div class="list-meta">
        ...
        <div class="muted">${t("instances.reason", { value: entry.reason ?? t("states.none") })}</div>
      </div>
    </div>
  `;
}
