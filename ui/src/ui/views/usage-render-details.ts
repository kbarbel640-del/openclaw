...
function renderSessionSummary(
  ...
  if (!usage) {
    return html`
      <div class="muted">${t("usageUi.details.noUsageData")}</div>
    `;
  }

  const formatTs = (ts?: number): string =>
    ts ? new Date(ts).toLocaleString(getUiLocale()) : t("common.na");
  ...
  const modelItems =
    usage.modelUsage?.slice(0, 6).map((entry) => ({
      label: entry.model ?? t("states.unknown"),
  ...
  return html`
    ...
    <div class="session-summary-grid">
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usageUi.insights.cards.messages.title")}</div>
        <div class="session-summary-value">${usage.messageCounts?.total ?? 0}</div>
        <div class="session-summary-meta">
          ${t("usageUi.insights.cards.messages.sub", {
            user: String(usage.messageCounts?.user ?? 0),
            assistant: String(usage.messageCounts?.assistant ?? 0),
          })}
        </div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usageUi.insights.cards.toolCalls.title")}</div>
        <div class="session-summary-value">${toolCallCount}</div>
        <div class="session-summary-meta">
          ${t("usageUi.insights.cards.toolCalls.sub", { count: String(uniqueToolCount) })}
        </div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usageUi.insights.cards.errors.title")}</div>
        <div class="session-summary-value">${usage.messageCounts?.errors ?? 0}</div>
        <div class="session-summary-meta">
          ${t("usageUi.insights.cards.errors.sub", {
            count: String(usage.messageCounts?.toolResults ?? 0),
          })}
        </div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usageUi.details.duration")}</div>
        <div class="session-summary-value">
          ${formatDurationCompact(usage.durationMs, { spaced: true }) ?? t("common.na")}
        </div>
    ...
    <div class="usage-insights-grid" style="margin-top: 12px;">
      ${renderInsightList(
        t("usageUi.insights.lists.topTools"),
        toolItems,
        t("usageUi.insights.empty.noToolCalls"),
      )}
      ${renderInsightList(
        t("usageUi.details.modelMix"),
        modelItems,
        t("usageUi.insights.empty.noModelData"),
      )}
    </div>
  `;
}
...
function renderSessionDetailPanel(
  ...
  const cursorIndicator = filteredUsage ? ` (${t("usageUi.details.filtered")})` : "";
    ...
    <div class="card session-detail-panel">
      <div class="session-detail-header">
        ...
        <div class="session-detail-stats">
          ${
            ...
            <span>
              ...
              ${t("usageUi.units.tokens")}${cursorIndicator}
            </span>
        ...
        <button
          ...
          @click=${onClose}
          title=${t("usageUi.details.closeSessionDetails")}
        >
          ×
        </button>
      </div>
...
function renderTimeSeriesCompact(
  ...
  if (loading) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("states.loading")}</div>
      </div>
    `;
  }
    ...
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">
          ${t("usageUi.details.noTimelineData")}
        </div>
      </div>
    `;
  }
  ...
  if (points.length < 2) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">
          ${t("usageUi.details.noDataInRange")}
        </div>
      </div>
    `;
  }
    ...
    <div class="session-timeseries-compact">
      <div class="timeseries-header-row">
        <div class="card-title" style="font-size: 12px; color: var(--text);">
          ${t("usageUi.details.usageOverTime")}
        </div>
        <div class="timeseries-controls">
          ...
          <div class="chart-toggle small">
            ...
            >
              ${t("usageUi.details.perTurn")}
            </button>
            <button
              class="toggle-btn ${isCumulative ? "active" : ""}"
              @click=${() => onModeChange("cumulative")}
            >
              ${t("usageUi.details.cumulative")}
            </button>
          </div>
          ${
            !isCumulative
      ...
      <div class="timeseries-chart-wrapper" style="position: relative; cursor: crosshair;">
        ...
        >
          ...
          ${
            points.length > 0
              ? svg`
            <text x="${padding.left}" y="${padding.top + chartHeight + 10}" text-anchor="start" class="ts-axis-label">${new Date(points[0].timestamp).toLocaleTimeString(getUiLocale(), { hour: "2-digit", minute: "2-digit" })}</text>
            <text x="${width - padding.right}" y="${padding.top + chartHeight + 10}" text-anchor="end" class="ts-axis-label">${new Date(points[points.length - 1].timestamp).toLocaleTimeString(getUiLocale(), { hour: "2-digit", minute: "2-digit" })}</text>
          `
              : nothing
          }
          <!-- Bars -->
          ${points.map((p, i) => {
            ...
            const tooltipLines = [
              date.toLocaleDateString(getUiLocale(), {
                month: "short",
                day: "numeric",
                hour: "2-digit",
                minute: "2-digit",
              }),
              t("usageUi.labels.tokensValue", { value: formatTokens(val) }),
            ];
            if (breakdownByType) {
              tooltipLines.push(t("usageUi.details.outValue", { value: formatTokens(p.output) }));
              tooltipLines.push(t("usageUi.details.inValue", { value: formatTokens(p.input) }));
              tooltipLines.push(
                t("usageUi.details.cacheWriteShortValue", { value: formatTokens(p.cacheWrite) }),
              );
              tooltipLines.push(
                t("usageUi.details.cacheReadShortValue", { value: formatTokens(p.cacheRead) }),
              );
            }
        ...
        ${
          hasSelection
            ? html`
              ...
              · 
              ${new Date(rangeStartTs).toLocaleTimeString(getUiLocale(), { hour: "2-digit", minute: "2-digit" })}–${new Date(rangeEndTs).toLocaleTimeString(getUiLocale(), { hour: "2-digit", minute: "2-digit" })}
              ·
            ...
            `
            : html`${t("usageUi.details.messagesCountShort", { count: String(points.length) })} · ${formatTokens(cumTokens)} · ${formatCost(cumCost)}`
        }
      </div>
      ${
        breakdownByType
          ? html`
              <div style="margin-top: 8px;">
                <div class="card-title" style="font-size: 12px; margin-bottom: 6px; color: var(--text);">
                  ${t("usageUi.breakdown.tokensByType")}
                </div>
                ...
                <div class="cost-breakdown-legend">
                  <div class="legend-item" title=${t("usageUi.details.titles.assistantOutputTokens")}>
                    <span class="legend-dot output"></span>${t("usageUi.breakdown.outputValue", { value: formatTokens(filteredOutput) })}
                  </div>
                  <div class="legend-item" title=${t("usageUi.details.titles.userToolInputTokens")}>
                    <span class="legend-dot input"></span>${t("usageUi.breakdown.inputValue", { value: formatTokens(filteredInput) })}
                  </div>
                  <div class="legend-item" title=${t("usageUi.details.titles.cacheWriteTokens")}>
                    <span class="legend-dot cache-write"></span>${t("usageUi.breakdown.cacheWriteValue", { value: formatTokens(filteredCacheWrite) })}
                  </div>
                  <div class="legend-item" title=${t("usageUi.details.titles.cacheReadTokens")}>
                    <span class="legend-dot cache-read"></span>${t("usageUi.breakdown.cacheReadValue", { value: formatTokens(filteredCacheRead) })}
                  </div>
                </div>
                <div class="cost-breakdown-total">
                  ${t("usageUi.breakdown.totalValue", { value: formatTokens(totalTypeTokens) })}
                </div>
              </div>
            `
          : nothing
...
function renderContextPanel(
  ...
  if (!contextWeight) {
    return html`
      <div class="context-details-panel">
        <div class="muted" style="padding: 20px; text-align: center">
          ${t("usageUi.details.noContextData")}
        </div>
      </div>
    `;
  }
  ...
  if (usage && usage.totalTokens > 0) {
    ...
    if (inputTokens > 0) {
      contextPct = t("usageUi.details.contextPctOfInput", {
  ...
  return html`
    <div class="context-details-panel">
      <div class="context-breakdown-header">
        <div class="card-title" style="font-size: 12px; color: var(--text);">
          ${t("usageUi.details.systemPromptBreakdown")}
        </div>
        ${
          hasMore
            ? html`<button class="context-expand-btn" @click=${onToggleExpanded}>
                ${showAll ? t("usageUi.details.collapse") : t("usageUi.details.expandAll")}
              </button>`
            : nothing
        }
      </div>
      <p class="context-weight-desc">
        ${contextPct || t("usageUi.details.baseContextPerMessage")}
      </p>
      <div class="context-stacked-bar">
        <div class="context-segment system" style="width: ${pct(systemTokens, totalContextTokens).toFixed(1)}%" title=${t("usageUi.details.contextLegend.system", { value: formatTokens(systemTokens) })}></div>
        <div class="context-segment skills" style="width: ${pct(skillsTokens, totalContextTokens).toFixed(1)}%" title=${t("usageUi.details.contextLegend.skills", { value: formatTokens(skillsTokens) })}></div>
        <div class="context-segment tools" style="width: ${pct(toolsTokens, totalContextTokens).toFixed(1)}%" title=${t("usageUi.details.contextLegend.tools", { value: formatTokens(toolsTokens) })}></div>
        <div class="context-segment files" style="width: ${pct(filesTokens, totalContextTokens).toFixed(1)}%" title=${t("usageUi.details.contextLegend.files", { value: formatTokens(filesTokens) })}></div>
      </div>
      <div class="context-legend">
        <span class="legend-item"><span class="legend-dot system"></span>${t("usageUi.details.contextLegend.sysShort", { value: formatTokens(systemTokens) })}</span>
        <span class="legend-item"><span class="legend-dot skills"></span>${t("usageUi.details.contextLegend.skills", { value: formatTokens(skillsTokens) })}</span>
        <span class="legend-item"><span class="legend-dot tools"></span>${t("usageUi.details.contextLegend.tools", { value: formatTokens(toolsTokens) })}</span>
        <span class="legend-item"><span class="legend-dot files"></span>${t("usageUi.details.contextLegend.files", { value: formatTokens(filesTokens) })}</span>
      </div>
      <div class="context-total">
        ${t("usageUi.breakdown.totalApproxValue", { value: formatTokens(totalContextTokens) })}
      </div>
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">
                      ${t("usageUi.details.sections.skills", { count: String(skillsList.length) })}
                    </div>
                    ...
                    ${
                      more > 0
                        ? html`<div class="context-breakdown-more">
                            ${t("usageUi.sessions.moreCount", { count: String(more) })}
                          </div>`
                        : nothing
                    }
                  </div>
          ...
          toolsList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">
                      ${t("usageUi.details.sections.tools", { count: String(toolsList.length) })}
                    </div>
                    <div class="context-breakdown-list">
                      ${toolsTop.map(
                        (t) => html`
          ...
          filesList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">
                      ${t("usageUi.details.sections.files", { count: String(filesList.length) })}
                    </div>
                    <div class="context-breakdown-list">
                      ${filesTop.map(
                        (f) => html`
...
function renderSessionLogsCompact(
  ...
  if (loading) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">${t("usageUi.details.conversation")}</div>
        <div class="muted" style="padding: 20px; text-align: center">${t("states.loading")}</div>
      </div>
    `;
  }
  ...
  const displayedCount =
    hasActiveFilters || hasCursorFilter
      ? t("usageUi.details.filteredMessagesCount", {
          filtered: String(filteredEntries.length),
          total: String(logs.length),
          suffix: hasCursorFilter ? ` (${t("usageUi.details.timelineFiltered")})` : "",
        })
      : `${logs.length}`;
  ...
  return html`
    <div class="session-logs-compact">
      <div class="session-logs-header">
        <span>
          ${t("usageUi.details.conversation")}
          <span style="font-weight: normal; color: var(--muted);">
            (${t("usageUi.details.messagesWithCount", { count: displayedCount })})
          </span>
        </span>
        <button class="btn btn-sm usage-action-btn usage-secondary-btn" @click=${onToggleExpandedAll}>
          ${expandedAll ? t("usageUi.details.collapseAll") : t("usageUi.details.expandAll")}
        </button>
      </div>
      <div class="usage-filters-inline" style="margin: 10px 12px;">
        ...
        >
          <option value="user" ?selected=${roleSelected.has("user")}>${t("usageUi.details.roles.user")}</option>
          <option value="assistant" ?selected=${roleSelected.has("assistant")}>${t("usageUi.details.roles.assistant")}</option>
          <option value="tool" ?selected=${roleSelected.has("tool")}>${t("usageUi.details.roles.tool")}</option>
          <option value="toolResult" ?selected=${roleSelected.has("toolResult")}>${t("usageUi.details.roles.toolResult")}</option>
        </select>
        <select
          multiple
          size="4"
        ...
        <label class="usage-filters-inline" style="gap: 6px;">
          ...
          />
          ${t("usageUi.details.hasTools")}
        </label>
        <input
          type="text"
          placeholder=${t("usageUi.details.searchConversation")}
          .value=${filters.query}
      ...
      <div class="session-logs-list">
        ${filteredEntries.map((entry) => {
          ...
          const roleLabel =
            log.role === "user"
              ? t("usageUi.details.roles.you")
              : log.role === "assistant"
                ? t("usageUi.details.roles.assistant")
                : t("usageUi.details.roles.tool");
          return html`
          <div class="session-log-entry ${roleClass}">
            <div class="session-log-meta">
              <span class="session-log-role">${roleLabel}</span>
              <span>${new Date(log.timestamp).toLocaleString(getUiLocale())}</span>
          ...
          filteredEntries.length === 0
            ? html`
                <div class="muted" style="padding: 12px">
                  ${t("usageUi.details.noMessagesMatchFilters")}
                </div>
              `
            : nothing
        }
