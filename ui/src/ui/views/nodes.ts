    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">${t("nodesUi.nodes.subtitle")}</div>
        </div>
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          props.nodes.length === 0
            ? html`
                <div class="muted">${t("nodesUi.nodes.none")}</div>
              `
...
function renderDevices(props: NodesProps) {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("nodesUi.devices.title")}</div>
          <div class="card-sub">${t("nodesUi.devices.subtitle")}</div>
        </div>
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? t("states.loading") : t("common.refresh")}
        </button>
      </div>
      ${
        props.devicesError
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          pending.length > 0
            ? html`
              <div class="muted" style="margin-bottom: 8px;">${t("nodesUi.devices.pending")}</div>
        ...
        ${
          paired.length > 0
            ? html`
              <div class="muted" style="margin-top: 12px; margin-bottom: 8px;">${t("nodesUi.devices.paired")}</div>
        ...
        ${
          pending.length === 0 && paired.length === 0
            ? html`
                <div class="muted">${t("nodesUi.devices.none")}</div>
              `
            : nothing
        }
      </div>
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const role = req.role?.trim()
    ? t("nodesUi.devices.roleWithValue", { value: req.role })
    : t("nodesUi.devices.roleWithValue", { value: "-" });
  const repair = req.isRepair ? ` · ${t("nodesUi.devices.repair")}` : "";
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="muted" style="margin-top: 6px;">
          ${role} · ${t("nodesUi.devices.requestedAt", { value: age })}${repair}
        </div>
      </div>
      <div class="list-meta">
        <div class="row" style="justify-content: flex-end; gap: 8px; flex-wrap: wrap;">
          <button class="btn btn--sm primary" @click=${() => props.onDeviceApprove(req.requestId)}>
            ${t("nodesUi.devices.approve")}
          </button>
          <button class="btn btn--sm" @click=${() => props.onDeviceReject(req.requestId)}>
            ${t("nodesUi.devices.reject")}
          </button>
        </div>
      </div>
    </div>
...
function renderPairedDevice(device: PairedDevice, props: NodesProps) {
  ...
  const roles = t("nodesUi.devices.rolesWithValue", { value: formatList(device.roles) });
  const scopes = t("nodesUi.devices.scopesWithValue", { value: formatList(device.scopes) });
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        ${
          tokens.length === 0
            ? html`
                <div class="muted" style="margin-top: 6px">${t("nodesUi.devices.tokensNone")}</div>
              `
            : html`
              <div class="muted" style="margin-top: 10px;">${t("nodesUi.devices.tokens")}</div>
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? t("nodesUi.devices.revoked") : t("nodesUi.devices.active");
  const scopes = t("nodesUi.devices.scopesWithValue", { value: formatList(token.scopes) });
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">${t("nodesUi.bindings.title")}</div>
          <div class="card-sub">
            ${t("nodesUi.bindings.subtitle")}
          </div>
        </div>
        <button
          class="btn"
          ...
          @click=${state.onSave}
        >
          ${state.configSaving ? t("channels.common.saving") : t("actions.save")}
        </button>
      </div>
        ...
        state.formMode === "raw"
          ? html`
              <div class="callout warn" style="margin-top: 12px">
                ${t("nodesUi.bindings.rawModeHint")}
              </div>
            `
          : nothing
      }
      ...
      ${
        !state.ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t("nodesUi.bindings.loadConfigHint")}</div>
            <button class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
              ${state.configLoading ? t("states.loading") : t("nodesUi.bindings.loadConfig")}
            </button>
          </div>`
          : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                <div class="list-main">
                  <div class="list-title">${t("nodesUi.bindings.defaultBinding")}</div>
                  <div class="list-sub">${t("nodesUi.bindings.defaultBindingSub")}</div>
                </div>
                <div class="list-meta">
                  <label class="field">
                    <span>${t("nodesUi.bindings.node")}</span>
                    <select
                    ...
                    >
                      <option value="" ?selected=${defaultValue === ""}>${t("nodesUi.bindings.anyNode")}</option>
                      ${state.nodes.map(
                        (node) =>
                          html`<option
                            value=${node.id}
                  ...
                  ${
                    !supportsBinding
                      ? html`
                          <div class="muted">${t("nodesUi.bindings.noRunnableNodes")}</div>
                        `
                      : nothing
                  }
                </div>
              ...
              ${
                state.agents.length === 0
                  ? html`
                      <div class="muted">${t("agentsUi.sidebar.noAgents")}</div>
                    `
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="list-sub">
          ${agent.isDefault ? t("nodesUi.bindings.defaultAgent") : t("nodesUi.bindings.agent")} ·
          ${
            bindingValue === "__default__"
              ? t("nodesUi.bindings.usesDefault", {
                  value: state.defaultBinding ?? t("nodesUi.bindings.any"),
                })
              : t("nodesUi.bindings.override", { value: String(agent.binding ?? "") })
          }
        </div>
      </div>
      <div class="list-meta">
        <label class="field">
          <span>${t("nodesUi.bindings.binding")}</span>
          <select
            ?disabled=${state.disabled || !supportsBinding}
            @change=${(event: Event) => {
              const target = event.target as HTMLSelectElement;
...
function renderNode(node: Record<string, unknown>) {
  ...
  const title =
    (typeof node.displayName === "string" && node.displayName.trim()) ||
    (typeof node.nodeId === "string" ? node.nodeId : t("states.unknown"));
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? t("nodesUi.nodes.paired") : t("nodesUi.nodes.unpaired")}</span>
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? t("channels.common.connected") : t("common.offline")}
          </span>
