...
// Section definitions - using i18n for labels
const getSections = (): Array<{ key: string; label: string }> => [
            ...
            ? html`
              <details class="config-diff">
                <summary class="config-diff__summary">
                  <span
                    >${t("config.viewChanges", { count: String(diff.length), plural: diff.length !== 1 ? "s" : "" })}</span
                  >
                  <svg
