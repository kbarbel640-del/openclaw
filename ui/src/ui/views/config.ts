...
// Section definitions — keys match i18n keys under configView.sections.*
const SECTION_KEYS = [
  "env",
  "update",
  "agents",
...
function getSections(): Array<{ key: string; label: string }> {
  return SECTION_KEYS.map((key) => ({
    key,
    label: t(`configView.sections.${key}`),
  }));
}
...
function resolveSectionMeta(
  ...
  const description = t(`configView.sectionMeta.${key}.description`);
  // t() returns the key path itself when no translation is found
  const hasTranslation = label !== `configView.sectionMeta.${key}.label`;
  if (hasTranslation) {
    ...
    <div class="config-layout">
      ...
      <aside class="config-sidebar">
        <div class="config-sidebar__header">
          <div class="config-sidebar__title">${t("configView.sidebar.title")}</div>
          <span
            class="pill pill--sm ${
        ...
        <div class="config-search">
          <div class="config-search__input-row">
            ...
            <input
              ...
              placeholder="${t("configView.search.placeholder")}"
          ...
          <div class="config-search__hint">
            <span class="config-search__hint-label" id="config-tag-filter-label">${t("configView.search.tagFilters")}</span>
            <details class="config-search__tag-picker">
              <summary class="config-search__tag-trigger" aria-labelledby="config-tag-filter-label">
                ${
                  selectedTags.size === 0
                    ? html`
                        <span class="config-search__tag-placeholder">${t("configView.search.addTags")}</span>
                      `
                    : html`
        ...
        <nav class="config-nav">
          ...
          >
            ...
            <span class="config-nav__label">${t("configView.sidebar.allSettings")}</span>
          </button>
          ${allSections.map(
            (section) => html`
              <button
        ...
        <div class="config-sidebar__footer">
          <div class="config-mode-toggle">
            ...
            >
              ${t("configView.mode.form")}
            </button>
            <button
              class="config-mode-toggle__btn ${props.formMode === "raw" ? "active" : ""}"
              @click=${() => props.onFormModeChange("raw")}
            >
              ${t("configView.mode.raw")}
            </button>
          </div>
        </div>
      </aside>
      ...
      <main class="config-main">
        ...
        <div class="config-actions">
          <div class="config-actions__left">
            ${
              hasChanges
                ...
                `
                : html`
                    <span class="config-status muted">${t("configView.actions.noChanges")}</span>
                  `
            }
          </div>
          <div class="config-actions__right">
            ...
            >
              ${props.loading ? t("configView.actions.loading") : t("configView.actions.reload")}
            </button>
            <button
              ...
              @click=${props.onSave}
            >
              ${props.saving ? t("configView.actions.saving") : t("configView.actions.save")}
            </button>
            <button
              class="btn btn--sm"
              ?disabled=${!canApply}
              @click=${props.onApply}
            >
              ${props.applying ? t("configView.actions.applying") : t("configView.actions.apply")}
            </button>
            <button
              class="btn btn--sm"
              ?disabled=${!canUpdate}
            ...
            >
              ${props.updating ? t("configView.actions.updating") : t("configView.actions.update")}
            </button>
          </div>
        </div>
              ...
              <details class="config-diff">
                <summary class="config-diff__summary">
                  <span
                    >${
                      diff.length === 1
                        ? t("configView.diff.viewPending", { count: String(diff.length) })
                        : t("configView.diff.viewPendingPlural", { count: String(diff.length) })
                    }</span
                  >
                  <svg
            ...
            props.formMode === "form"
                  ...
                  props.schemaLoading
                    ? html`
                        <div class="config-loading">
                          ...
                          <span>${t("configView.actions.loadingSchema")}</span>
                        </div>
                      `
                    : renderConfigForm({
              ...
              : html`
                <label class="field config-raw-field">
                  <span>${t("configView.mode.rawJson5")}</span>
                  <textarea
                    .value=${props.raw}
                    @input=${(e: Event) =>
