...
export function renderCron(props: CronProps) {
  ...
  return html`
      ...
      <div class="card">
            ...
            ? html`
                <div class="form-grid" style="margin-top: 12px;">
                  <label class="field">
                    ...
                    >
                      <option value="announce">${t("cron.announce")}</option>
                      <option value="none">${t("cron.none")}</option>
                    </select>
                  </label>
                  <label class="field">
        ...
        <div class="row" style="margin-top: 14px;">
          <button class="btn primary" ?disabled=${props.busy} @click=${props.onAdd}>
            ${props.busy ? t("config.saving") : t("cron.addJob")}
          </button>
        </div>
      </div>
    </section>
    ...
    <section class="card" style="margin-top: 18px;">
      <div class="card-title">${t("cron.allJobs")}</div>
      <div class="card-sub">${t("cron.allJobsSubtitle")}</div>
      ${
        props.jobs.length === 0
          ? html`
    ...
    <section class="card" style="margin-top: 18px;">
      ...
      <div class="card-sub">${t("cron.runHistorySubtitle", { job: selectedRunTitle })}</div>
      ${
        props.runsJobId == null
          ? html`
              <div class="muted" style="margin-top: 12px">${t("cron.selectJobHelp")}</div>
            `
  ...
  return html`
    <div class=${itemClass} @click=${() => props.onLoadRuns(job.id)}>
      <div class="list-main">
        ...
        ${job.agentId ? html`<div class="muted cron-job-agent">${t("cron.agent", { id: job.agentId })}</div>` : nothing}
      </div>
      <div class="list-meta">
        ${renderJobState(job)}
      </div>
...
function renderJobState(job: CronJob) {
  ...
  return html`
    <div class="cron-job-state">
      ...
      <div class="cron-job-state-row">
        <span class="cron-job-state-key">${t("cron.lastLabel")}</span>
