...
export function renderLogs(props: LogsProps) {
  ...
  const exportButtonText =
    exportLabel === "filtered" ? t("logsUi.exportFiltered") : t("logsUi.exportVisible");

  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("logsUi.title")}</div>
          <div class="card-sub">${t("logsUi.subtitle")}</div>
        </div>
      ...
      <div class="filters" style="margin-top: 14px;">
        <label class="field" style="min-width: 220px;">
          <span>${t("logsUi.filter")}</span>
          <input
            .value=${props.filterText}
            @input=${(e: Event) => props.onFilterTextChange((e.target as HTMLInputElement).value)}
            placeholder=${t("logsUi.filterPlaceholder")}
          />
        </label>
        <label class="field checkbox">
          <span>${t("logsUi.autoFollow")}</span>
          <input
            type="checkbox"
      ...
      ${
        props.file
          ? html`<div class="muted" style="margin-top: 10px;">${t("logsUi.file", { path: props.file })}</div>`
          : nothing
      }
      ${
        props.truncated
          ? html`
              <div class="callout" style="margin-top: 10px">${t("logsUi.truncated")}</div>
            `
          : nothing
      }
      ${
      ...
      <div class="log-stream" style="margin-top: 12px;" @scroll=${props.onScroll}>
        ${
          filtered.length === 0
            ? html`
                <div class="muted" style="padding: 12px">${t("logsUi.noEntries")}</div>
              `
            : filtered.map(
                (entry) => html`
                <div class="log-row">
