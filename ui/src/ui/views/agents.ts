      ...
      <section class="card agents-sidebar">
        <div class="row" style="justify-content: space-between;">
          <div>
            ...
            <div class="card-sub">${t("agentsUi.sidebar.configuredCount", { count: String(agents.length) })}</div>
          </div>
      ...
      <section class="agents-main">
        ${
          !selectedAgent
            ? html`
                <div class="card">
                  <div class="card-title">${t("agentsUi.empty.title")}</div>
                  <div class="card-sub">${t("agentsUi.empty.subtitle")}</div>
                </div>
              `
            : html`
                ${renderAgentHeader(
...
function renderAgentHeader(
  ...
  const subtitle = agent.identity?.theme?.trim() || t("agentsUi.header.subtitleFallback");
...
function renderAgentTabs(active: AgentsPanel, onSelect: (panel: AgentsPanel) => void) {
  const tabs: Array<{ id: AgentsPanel; label: string }> = [
    ...
    { id: "files", label: t("agentsUi.tabs.files") },
    { id: "tools", label: t("agentsUi.tabs.tools") },
...
function renderAgentOverview(params: {
  ...
  const workspace =
    ...
    t("agentsUi.values.defaultWorkspace");
  ...
  return html`
    <section class="card">
      ...
      <div class="card-sub">${t("agentsUi.overview.subtitle")}</div>
      <div class="agents-overview-grid" style="margin-top: 16px;">
        ...
        <div class="agent-kv">
          ...
          <div>${isDefault ? t("states.yes") : t("states.no")}</div>
        </div>
        <div class="agent-kv">
        ...
        <div class="agent-kv">
          ...
          <div>${
            skillFilter
              ? t("agentsUi.values.selectedCount", { count: String(skillCount ?? 0) })
              : t("agentsUi.values.allSkills")
          }</div>
        </div>
      </div>

      <div class="agent-model-select" style="margin-top: 20px;">
        <div class="label">${t("agentsUi.overview.modelSelection")}</div>
        <div class="row" style="gap: 12px; flex-wrap: wrap;">
          <label class="field" style="min-width: 260px; flex: 1;">
            <span>${t("agentsUi.overview.primaryModelField", {
              suffix: isDefault ? t("agentsUi.overview.defaultSuffix") : "",
            })}</span>
            <select
                ...
                isDefault
                  ...
                  : html`
                      <option value="">
                        ${
                          defaultPrimary
                            ? t("agentsUi.overview.inheritDefaultWithValue", {
                                value: defaultPrimary,
                              })
                            : t("agentsUi.overview.inheritDefault")
                        }
                      </option>
                    `
              }
          ...
          <label class="field" style="min-width: 260px; flex: 1;">
            <span>${t("agentsUi.overview.fallbacksLabel")}</span>
            <input
              .value=${fallbackText}
              ?disabled=${!configForm || configLoading || configSaving}
              placeholder=${t("agentsUi.overview.fallbacksPlaceholder")}
              @input=${(e: Event) =>
                onModelFallbacksChange(
                  agent.id,
        ...
        <div class="row" style="justify-content: flex-end; gap: 8px;">
          ...
          <button
            ...
            @click=${onConfigSave}
          >
            ${configSaving ? t("channels.common.saving") : t("actions.save")}
          </button>
        </div>
      </div>
    </section>
