...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("AMIGO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.AMIGO_STATE_DIR = path.join(base, ".amigo");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "amigo-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".amigo", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
