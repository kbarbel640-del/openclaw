...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `amigo-e2e-${name}-`));
  const configDir = path.join(homeDir, ".amigo");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          AMIGO_SKIP_CHANNELS: "1",
          AMIGO_SKIP_BROWSER_CONTROL_SERVER: "1",
          AMIGO_SKIP_CANVAS_HOST: "1",
        },
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        AMIGO_GATEWAY_TOKEN: inst.gatewayToken,
        AMIGO_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          AMIGO_GATEWAY_PORT: String(gwA.port),
          AMIGO_GATEWAY_TOKEN: gwA.gatewayToken,
          AMIGO_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          AMIGO_GATEWAY_PORT: String(gwB.port),
          AMIGO_GATEWAY_TOKEN: gwB.gatewayToken,
          AMIGO_GATEWAY_PASSWORD: "",
        }),
