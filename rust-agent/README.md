...
- Gateway RPC parity scaffold for `sessions.list`, `sessions.patch`, and `session.status`.
...
- Responds to core session RPCs (`sessions.list`, `sessions.patch`, `session.status`) with typed `resp` frames.
