...
| Gateway protocol connectivity | WS control plane (`connect`, events, session/gateway methods) | `Partial` | Rust bridge uses typed frame helpers (`req`/`resp`/`event`), method-family classification, known-method registry, session-kind parsing, and RPC dispatcher coverage for `sessions.list`, `sessions.patch`, `sessions.history`, `sessions.send`, and `session.status`; full RPC dispatch parity still pending. |
...
| Session model | `main` session, group isolation, activation/queue policies, reply-back | `Partial` | First-pass per-session scheduler now supports `followup`/`steer`/`collect` queue modes plus group activation gating (`mention`/`always`), with state counters + bounded in-memory session transcript (`sessions.history`/`sessions.send`) + last-decision persistence (JSON default, optional SQLite WAL); advanced routing/reply-back parity still pending. |
