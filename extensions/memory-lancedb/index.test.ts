...
const describeLiveOpenAI =
  HAS_OPENAI_KEY && process.env.OPENCLAW_LIVE_TEST === "1" ? describe : describe.skip;
...
describe("memory plugin e2e", () => {
  ...
  test("shouldCapture applies real capture rules", async () => {
    ...
    const customTooLong = `I always prefer this style. ${"x".repeat(1200)}`;
    expect(shouldCapture(customTooLong)).toBe(false);
  });
