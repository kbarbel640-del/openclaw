...
export const msteamsPlugin: ChannelPlugin<ResolvedMSTeamsAccount> = {
  ...
  agentPrompt: {
    messageToolHints: () => [
      ...
      "- MSTeams targeting: omit `target` to reply to the current conversation (auto-inferred). Explicit targets: `user:ID` or `user:Display Name` (requires Graph API) for DMs, `conversation:19:[redacted-email]2` for groups/channels. Prefer IDs over display names for speed.",
    ],
  },
  threading: {
