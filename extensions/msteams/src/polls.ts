...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["amigoPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["amigo", "pollId"]) ??
    readNestedString(value, ["amigo", "poll", "id"]) ??
    readNestedString(value, ["data", "amigoPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "amigo", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "amigo poll vote",
          displayText: "Vote recorded",
          value: { amigoPollId: pollId, pollId },
        },
      },
    ],
  };
