...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as AmigoConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as AmigoConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as AmigoConfig);
