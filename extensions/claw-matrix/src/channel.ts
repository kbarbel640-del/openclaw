...
// The ChannelPlugin type from OpenClaw — we type it loosely here
// to avoid import resolution issues. OpenClaw validates at runtime.
type AnyConfig = any;
...
export const matrixChannelPlugin = {
  ...
  // ═══════════════════════════════════════════════
  config: {
    listAccountIds(cfg: AnyConfig): string[] {
      const matrixCfg = cfg?.channels?.matrix;
      if (!matrixCfg) return [];
      if (matrixCfg.accounts && typeof matrixCfg.accounts === "object") {
        return Object.keys(matrixCfg.accounts);
      }
      return matrixCfg.userId ? ["default"] : [];
    },
    ...
    resolveAllowFrom(params: {
      ...
      return params.cfg?.channels?.matrix?.dm?.allowFrom;
    },
  },
  ...
  outbound: {
    ...
    async sendText(ctx: {
      ...
      const account = resolveMatrixAccount(ctx.cfg);
  ...
  groups: {
    resolveRequireMention(params: {
      ...
      const matrixCfg = params.cfg?.channels?.matrix;
      const groupId = params.groupId;
      if (groupId && matrixCfg?.groups?.[groupId]) {
        return matrixCfg.groups[groupId].requireMention;
      }
      return undefined;
    },
  },
  ...
  actions: {
    supportsAction(params: { action: string }): boolean {
      return ["send", "read", "channel-list"].includes(params.action);
    },
