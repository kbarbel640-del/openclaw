...
export function createMattermostInteractionHandler(params: {
  ...
  return async (req: IncomingMessage, res: ServerResponse) => {
    ...
    // Fetch the post and build the updated attachments BEFORE responding,
    // then return the update in the response (for immediate UI refresh)
    // AND update via REST API (as backup for WebSocket-connected clients).
    let updatedMessage = "";
    let updatedAttachments: any[] = [];

    try {
      const post = await fetchMattermostPost(client, payload.post_id);
      const postProps = (post as any).props as Record<string, unknown> | undefined;
      const attachments = Array.isArray(postProps?.attachments) ? postProps!.attachments : [];
      updatedMessage = (post as any).message ?? "";

      updatedAttachments = attachments
        .map((att: any) => {
          if (!Array.isArray(att.actions) || att.actions.length === 0) {
            return att;
          }
          const clicked = att.actions.find((a: any) => a.id === actionId);
          const clickedName = clicked?.name ?? actionId;
          const selectedLine = `\u2713 **${clickedName}** selected by @${payload.user_name ?? payload.user_id}`;
          const newText = [att.text, selectedLine].filter(Boolean).join("\n");
          return { ...att, text: newText, actions: undefined };
        })
        .filter((att: any) => att.text);
    } catch (err) {
      log?.(`mattermost interaction: failed to fetch post: ${String(err)}`);
    }

    // Return update in the callback response for immediate UI refresh
    ...
    try {
      ...
      // Best-effort — the response update should have handled it
    }
  };
}
