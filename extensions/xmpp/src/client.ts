...
export class XmppClient {
  ...
  async sendMessage(to: string, body: string, options?: XmppSendOptions): Promise<string> {
    ...
    if (options?.chatState) {
      children.push(
        xml(options.chatState, { xmlns: "http://jabber.org/protocol/chatstates" }),
      );
    }
  ...
  private handleStanza(stanza: any): void {
    ...
    const bodyText = typeof stanza.getChildText === "function" ? stanza.getChildText("body") : null;
        ...
        if (hasUploadFeature) {
          ...
          for (const x of xData as any[]) {
            ...
            for (const field of fields) {
              ...
              const valueEl =
                typeof field.getChild === "function" ? field.getChild("value") : null;
              const text =
  ...
  private async requestHttpUploadSlot(params: {
    ...
    const putUrlText =
      typeof putEl?.getText === "function" ? String(putEl.getText() ?? "") : "";
    const getUrlText =
      typeof getEl?.getText === "function" ? String(getEl.getText() ?? "") : "";
        ...
        const req = transport.request(options, (res) => {
          ...
          res.on("end", () => {
            if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
              ...
              reject(
                new Error(
                  `HTTP upload failed with status ${res.statusCode ?? 0}: ${combined}`,
                ),
              );
            }
          });
