...
export async function startGateway(ctx: GatewayContext): Promise<void> {
  ...
  if (savedSession) {
    ...
    log?.info(
      `[qqbot:${account.accountId}] Restored session: sessionId=${sessionId}, lastSeq=${lastSeq}, intentLevel=${intentLevelIndex}`,
    );
  }
  ...
  const enqueueMessage = (msg: InboundMessageEvent): void => {
    if (messageQueue.length >= MESSAGE_QUEUE_SIZE) {
      ...
      log?.error(
        `[qqbot:${account.accountId}] Message queue full, dropping oldest from ${dropped?.senderId}`,
      );
    }
  ...
  const startMessageProcessor = (
    handleMessageFn: (msg: InboundMessageEvent) => Promise<void>,
  ): void => {
    ...
    try {
      ...
      ws.on("open", () => {
        ...
        startBackgroundTokenRefresh(account.appId, account.clientSecret, {
          log: log as { info: (msg: string) => void; error: (msg: string) => void },
        });
      });
          ...
          switch (op) {
            case 10: {
              ...
              if (sessionId && lastSeq !== null) {
                ...
                log?.info(
                  `[qqbot:${account.accountId}] Identify with intents: ${intentLevel.intents} (${intentLevel.description})`,
                );
                ws.send(
                  JSON.stringify({
                    op: 2,
            ...
            case 0: // Dispatch
              if (t === "READY") {
                ...
                log?.info(
                  `[qqbot:${account.accountId}] Ready (${INTENT_LEVELS[intentLevelIndex].description}), session: ${sessionId}`,
                );
                saveSession({
                  sessionId,
                  lastSeq,
                ...
                recordKnownUser({
                  openid: evt.author.user_openid,
                  type: "c2c",
                  accountId: account.accountId,
                });
                enqueueMessage({
                  type: "c2c",
                  senderId: evt.author.user_openid,
                  content: evt.content,
                  messageId: evt.id,
                ...
                recordKnownUser({
                  openid: evt.author.id,
                  type: "c2c",
                  nickname: evt.author.username,
                  accountId: account.accountId,
                });
                enqueueMessage({
                  type: "guild",
                ...
                recordKnownUser({
                  openid: evt.author.member_openid,
                  type: "group",
                  groupOpenid: evt.group_openid,
                  accountId: account.accountId,
                });
                enqueueMessage({
                  type: "group",
                  senderId: evt.author.member_openid,
                  content: evt.content,
                  messageId: evt.id,
                  timestamp: evt.timestamp,
                  groupOpenid: evt.group_openid,
            ...
            case 9: {
              ...
              if (!canResume) {
                ...
                if (intentLevelIndex < INTENT_LEVELS.length - 1) {
                  intentLevelIndex++;
                  log?.info(
                    `[qqbot:${account.accountId}] Downgrading intents to: ${INTENT_LEVELS[intentLevelIndex].description}`,
                  );
                } else {
