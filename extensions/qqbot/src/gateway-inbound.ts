...
请根据图片内容进行回复。`);
        ...
        log?.info(
          `[qqbot:${accountId}] Voice attachment detected: ${att.filename}, converting SILK to WAV...`,
        );
        try {
          ...
          if (result) {
            ...
            log?.info(
              `[qqbot:${accountId}] Voice converted: ${result.wavPath} (duration: ${durationStr})`,
            );
...
- 发送时间：${ts}`);
            ...
            log?.info(
              `[qqbot:${accountId}] Voice file is not SILK format, keeping original: ${localPath}`,
            );
            voiceDescriptions.push(`
...
export async function buildInboundMessage(
  ...
  const commandAuthorized =
    allowAll ||
    allowFromList.some((entry: string) => entry.toUpperCase() === event.senderId.toUpperCase());
