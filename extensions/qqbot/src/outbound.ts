    ...
    for (const item of sendQueue) {
      try {
        if (item.type === "text") {
          ...
          const imgResult = await sendImageToTarget(
            accessToken,
            target,
            imageUrl,
            replyToId ?? undefined,
          ...
          if (imgResult) {
            lastResult = {
              channel: "qqbot",
              messageId: imgResult.id,
              timestamp: imgResult.timestamp,
            };
          }
          console.log(
            `[qqbot] sendText: Sent image via <qqimg> tag: ${item.content.slice(0, 60)}...`,
          );
        }
      } catch (err) {
...
export async function sendMedia(ctx: MediaOutboundContext): Promise<OutboundResult> {
  ...
  try {
    ...
    // 发送图片
    const imageResult = await sendImageToTarget(
      accessToken,
      target,
      processedMediaUrl,
      replyToId ?? undefined,
...
function resolveImageUrl(imagePath: string): string | null {
  ...
  if (isLocalPath(imagePath)) {
    ...
    console.log(
      `[qqbot] resolveImageUrl: Converted local image to Base64 (size: ${result.sizeBytes} bytes)`,
    );
