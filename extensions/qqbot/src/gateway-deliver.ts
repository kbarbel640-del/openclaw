 ...
 * 辅助：发送错误提示给用户
 */
async function sendErrorMessage(ctx: DeliverContext, errorText: string): Promise<void> {
 ...
 */
async function sendImage(ctx: DeliverContext, imagePath: string): Promise<boolean> {
  ...
  if (isLocal) {
    ...
    log?.info(
      `[qqbot:${account.accountId}] Converted local image to Base64 (size: ${result.sizeBytes} bytes)`,
    );
  ...
  if (payloadResult.isPayload) {
    ...
    if (payloadResult.payload) {
      ...
      if (isCronReminderPayload(parsed)) {
        ...
        log?.error(
          `[qqbot:${account.accountId}] Unknown payload type: ${(parsed as Record<string, unknown>).type}`,
        );
        await sendErrorMessage(
          ctx,
          `[QQBot] 不支持的载荷类型: ${(parsed as Record<string, unknown>).type}`,
        );
        return;
      }
    }
 ...
 * 处理媒体消息载荷
 */
async function handleMediaPayload(ctx: DeliverContext, parsed: unknown): Promise<void> {
  const { event, account, log } = ctx;
  const target = targetFromEvent(event);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  log?.info(
    `[qqbot:${account.accountId}] Processing media payload, mediaType: ${mediaPayload.mediaType}`,
  );
    ...
    if (isHttpUrl(imgUrl) && !hasQQBotImageSize(fullMatch)) {
      try {
        ...
        textWithoutImages = textWithoutImages.replace(
          fullMatch,
          formatQQBotMarkdownImage(imgUrl, size),
        );
      } catch {
        textWithoutImages = textWithoutImages.replace(
          fullMatch,
          formatQQBotMarkdownImage(imgUrl, null),
        );
      }
    }
  }
