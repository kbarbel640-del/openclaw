...
class EntityExtractor {
  ...
  extractTopics(text) {
    ...
    const words = text
      ...
      .filter(
        (w) =>
          w.length >= this.config.minWordLength && !CLARITY_STOP_WORDS.has(w) && !/^\d+$/.test(w),
      );
...
class EntityScorer {
  ...
  computeTF(entity) {
    ...
    const recentMentions = history.filter((t) => (entity.currentTurn || 10) - t <= 10).length;
  ...
  computeRelationshipBoost(entity) {
    ...
    const avgStrength =
      entity.relationships.reduce((sum, r) => sum + (r.strength || 0), 0) /
...
class EntityStore {
  ...
  getEntitiesAboveThreshold(scorer, threshold) {
    return this.getTopEntities(scorer, 1000).filter((e) => e.totalScore >= threshold);
  }
}
...
class OutputFormatter {
  formatContextBlock(entities, relationships) {
    ...
    for (const entity of entities) {
      const bullet = entity.totalScore >= 40 ? "●" : entity.totalScore >= 25 ? "○" : "◐";
      const anchor = entity.isAnchor ? "⚓ " : "";

      let relInfo = "";
      if (entity.relationships && entity.relationships.length > 0) {
        const topRels = entity.relationships
          .slice(0, 2)
          .map((r) => r.targetId)
          .join(", ");
      ...
      lines.push(
        `  ${bullet} ${anchor}${entity.id} (score: ${Math.round(entity.totalScore)})${relInfo}`,
      );
    }
    ...
    if (strongRelationships.length > 0) {
      ...
      for (const rel of strongRelationships.slice(0, 5)) {
        lines.push(
          `  • ${rel.sourceId.split(":")[1]} → ${rel.type} → ${rel.targetId.split(":")[1]} (strength: ${rel.strength.toFixed(1)})`,
        );
      }
    }
...
class ClarityV2 {
  constructor(config = {}) {
    ...
    this.semanticMatcher = new SemanticMatcher({
      mergeThreshold: this.config.semanticMergeThreshold,
    });
    this.turn = 0;
  }
  ...
  processMessage(text, metadata = {}) {
    // Extract then delegate to processBatch which manages turns and dedup
    const { entities: extracted, composites } = this.extractor.extract(text);
    return this.processBatch(extracted, composites, { text, metadata });
  }

  /**
   * Process a batch of already-extracted entities (useful for testing and
   * programmatic ingestion). This centralizes turn advancement and
   * semantic deduplication logic.
   */
  processBatch(extracted, composites = [], metadata = {}) {
    // Advance turn once per batch
    this.turn++;
    this.store.turn = this.turn;

    // Prepare extracted entities with canonical IDs and minimal metadata
    const prepared = (extracted || []).map((e) => {
      const canonical =
        e.canonical ||
        e.id ||
        `${e.type}:${e.normalized || (e.name || "").toLowerCase().replace(/[^a-z0-9]+/g, "_")}`;
      return {
        id: canonical,
        canonical,
        type: e.type,
        name: e.name || (e.normalized ? e.normalized.replace(/_/g, " ") : canonical),
        normalized: e.normalized || (e.name || "").toLowerCase().replace(/[^a-z0-9]+/g, "_"),
        mentionCount: e.mentionCount || 1,
        firstMentionTurn: this.turn,
        lastMentionTurn: this.turn,
        mentionHistory: e.mentionHistory ? [...e.mentionHistory, this.turn] : [this.turn],
        contexts: e.contexts || (metadata && metadata.text ? [metadata.text] : []),
        relationships: e.relationships || [],
        isAnchor: e.isAnchor || false,
      };
    });

    let finalEntities = prepared;
    let merged = new Map();
    let suggestedRelationships = [];

    if (this.config.semanticDeduplication) {
      // Candidates are existing tracked entities + newly extracted
      const candidates = [...this.store.getAll(), ...prepared];

      const result = this.semanticMatcher.deduplicateEntities(candidates);
      finalEntities = result.entities;
      merged = result.merged || new Map();
      suggestedRelationships = result.relationships || [];

      // Rebuild store with deduplicated canonical entities
      this.store.entities.clear();
      for (const ent of finalEntities) {
        // Ensure canonical fields exist
        ent.id = ent.id || ent.canonical;
        ent.canonical = ent.canonical || ent.id;
        ent.relationships = ent.relationships || [];
        ent.contexts = ent.contexts || [];
        ent.mentionHistory = ent.mentionHistory || [];
        ent.mentionCount = ent.mentionCount || 0;
        this.store.entities.set(ent.id, ent);
      }

      // Integrate suggested semantic relationships
      for (const rel of suggestedRelationships) {
        this.relationships.addRelationship(
          rel.sourceId,
          rel.targetId,
          rel.type || RelType.RELATED,
          this.turn,
        );
        // Overwrite strength with semantic suggestion when present
        const key = this.relationships._makeKey(rel.sourceId, rel.targetId);
        const existing = this.relationships.relationships.get(key);
        if (existing) {
          existing.strength = rel.strength || existing.strength;
          existing.semanticSimilarity = rel.semanticSimilarity || existing.semanticSimilarity;
          existing.lastCooccurrence = this.turn;
        }
      }
    } else {
      // No semantic dedup: ingest extracted items directly
      for (const entity of prepared) {
        // Ingest expects { canonical }
        this.store.ingest(
          {
      ...
      finalEntities = this.store.getAll();
    }

    // Entities that were mentioned in this turn (post-dedup)
    const presentEntities = finalEntities.filter((e) =>
      (e.mentionHistory || []).includes(this.turn),
    );

    // Track co-occurrence for mentions in this turn
    if (presentEntities.length > 0) {
      this.relationships.trackCooccurrence(presentEntities, this.turn);
    }

    // Update document frequency (IDF) only for entities present this turn
    if (presentEntities.length > 0) {
      this.scorer.updateDocumentFrequency(presentEntities);
    }

    // Update stored entity relationship lists from tracker
    for (const entity of presentEntities) {
      const entityId = entity.canonical || entity.id;
    ...
    return {
      turn: this.turn,
      extracted: presentEntities,
      composites,
      entityCount: presentEntities.length,
      merged: merged,
    };
  }
