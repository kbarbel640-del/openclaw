...
metadata: { "amigo": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Amigo `sessions_spawn`
- **File I/O** == Amigo `read`/`write`
- **Remote fetch** == Amigo `web_fetch` (or `exec` with curl when POST is required)
