...
export async function uploadDeviceSignature(
  ...
  try {
    ...
    if (error && typeof error === "object" && "statusCode" in error) {
      ...
      if (statusCode === 429) {
        throw new Error("Rate limited by homeserver - please try again later", { cause: error });
      }
    ...
    if (error instanceof Error) {
      throw new Error(`Failed to upload device signature: ${error.message}`, { cause: error });
    }

    throw new Error("Failed to upload device signature: unknown error", { cause: error });
  }
}
