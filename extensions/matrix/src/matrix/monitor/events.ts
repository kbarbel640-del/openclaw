...
export function registerMatrixMonitorEvents(params: {
  ...
  client.on("room.message", (roomId: string, event: MatrixRawEvent) => {
    void deduplicatedHandler(roomId, event, "room.message");
  });
  ...
  client.on("room.decrypted_event", (roomId: string, event: MatrixRawEvent) => {
    ...
    console.log(
      `[MATRIX-E2EE-DEBUG] decrypted_event room=${roomId} type=${eventType} id=${eventId} hasFile=${hasFile} hasUrl=${hasUrl}`,
    );
    ...
    void deduplicatedHandler(roomId, event, "room.decrypted_event");
  });
  ...
  client.on(
    ...
    async (roomId: string, event: MatrixRawEvent, error: Error) => {
      ...
      console.log(
        `[MATRIX-E2EE-DEBUG] FAILED_DECRYPTION room=${roomId} id=${event.event_id ?? "unknown"} error=${error.message}`,
      );
      logger.warn(
