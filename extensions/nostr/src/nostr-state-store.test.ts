import type { PluginRuntime } from "amigo/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.AMIGO_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-nostr-"));
  process.env.AMIGO_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        ...
        return path.join(homedir(), ".amigo");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.AMIGO_STATE_DIR = previous;
    }
