...
describe("normalizeGoogleChatTarget", () => {
  it("normalizes provider prefixes", () => {
    ...
    expect(normalizeGoogleChatTarget("gchat:user:[redacted-email]")).toBe("users/[redacted-email]");
  });

  it("normalizes email targets to users/<email>", () => {
    expect(normalizeGoogleChatTarget("[redacted-email]")).toBe("users/[redacted-email]");
    expect(normalizeGoogleChatTarget("users/[redacted-email]")).toBe("users/[redacted-email]");
  });
