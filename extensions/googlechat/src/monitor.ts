...
function warnDeprecatedUsersEmailEntries(
  ...
  logVerbose(
    core,
    runtime,
    `Deprecated allowFrom entry detected: "users/<email>" is no longer treated as an email allowlist. Use raw email ([redacted-email]) or immutable user id (users/<id>). entries=${deprecated.join(", ")}`,
  );
}
