...
export const createSmartRouterTool = (api: MoltbotPluginApi) => {
  return {
    name: "smart_router",
    description: "Manage model routing, check usage, and switch models. Use this to optimize cost and performance.",
    schema: {
      type: "object",
      properties: {
        action: {
        ...
        taskType: {
          type: "string",
          description: "For 'route': The type of task to optimize for (e.g. 'coding', 'fast'). Customizable via config."
        },
        forceModel: {
          type: "string",
          description: "For 'route': Explicitly switch to ANY model ID (e.g., 'openai/gpt-4o', 'custom/my-model')."
        },
        profileId: {
          type: "string",
          description: "For 'reset_limit': The profile ID to reset (optional, resets all if omitted)."
        }
      },
      required: ["action"]
    },
    func: async (args: { action: string; taskType?: string; forceModel?: string; profileId?: string }) => {
