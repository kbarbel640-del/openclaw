  ...
  api.registerTool(
    {
      ...
      parameters: Type.Object({
        ...
        category: Type.Optional(stringEnum(MEMORY_CATEGORIES)),
        scope: Type.Optional(
          Type.String({ description: "Memory scope (optional, defaults to agent scope)" }),
        ),
      }),
      async execute(_toolCallId, params) {
        const {
  ...
  api.registerTool(
    {
      ...
      parameters: Type.Object({
        ...
        scope: Type.Optional(
          Type.String({ description: "Scope to search/delete from (optional)" }),
        ),
      }),
      async execute(_toolCallId, params) {
  ...
  api.registerTool(
    {
      ...
      parameters: Type.Object({
        ...
        text: Type.Optional(
          Type.String({ description: "New text content (triggers re-embedding)" }),
        ),
  ...
  api.registerTool(
    {
      ...
      parameters: Type.Object({
        scope: Type.Optional(
          Type.String({ description: "Specific scope to get stats for (optional)" }),
        ),
      }),
      async execute(_toolCallId, params) {
  ...
  api.registerTool(
    {
      ...
      parameters: Type.Object({
        limit: Type.Optional(
          Type.Number({ description: "Max memories to list (default: 10, max: 50)" }),
        ),
        scope: Type.Optional(Type.String({ description: "Filter by specific scope (optional)" })),
        category: Type.Optional(stringEnum(MEMORY_CATEGORIES)),
        offset: Type.Optional(
          Type.Number({ description: "Number of memories to skip (default: 0)" }),
        ),
      }),
      async execute(_toolCallId, params) {
        const {
        ...
        try {
          ...
          return {
            content: [
              { type: "text", text: `Recent memories (showing ${entries.length}):\n\n${text}` },
            ],
            details: {
