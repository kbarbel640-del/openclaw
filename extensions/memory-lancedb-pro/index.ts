    ...
    if (config.sessionMemory?.enabled === true) {
      ...
      api.registerHook(
        ...
        },
        {
          ...
          description: "Persist the previous session summary into memory-lancedb-pro after /new.",
        },
      );
...
function parsePluginConfig(value: unknown): PluginConfig {
  ...
  const apiKey =
    typeof embedding.apiKey === "string" ? embedding.apiKey : process.env.OPENAI_API_KEY || "";
