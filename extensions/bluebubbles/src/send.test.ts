...
describe("send", () => {
  ...
  describe("resolveChatGuidForTarget", () => {
    ...
    it("normalizes handle addresses for matching", async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        json: () =>
          Promise.resolve({
            data: [
              {
                guid: "iMessage;-;[redacted-email]",
                participants: [{ address: "[redacted-email]" }],
              },
            ],
          }),
      });
      ...
      expect(result).toBe("iMessage;-;[redacted-email]");
    });
