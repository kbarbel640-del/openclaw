...
function resolveAmigoRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.AMIGO_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set AMIGO_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveAmigoRoot(), "dist", "extensionAPI.js");
