...
`amigo.invoke --each`) without adding new Amigo code per workflow.
...
This extension depends on Amigo internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Amigo extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.amigo/extensions` as a standalone plugin directory.
