...
// NOTE: This extension is intended to be bundled with Amigo.
// When running from source (tests/dev), Amigo internals live under src/.
...
export function createLlmTaskTool(api: AmigoPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via amigo.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "amigo-llm-task-"));
