/**
 * Twitch channel plugin for Amigo.
 *
...
import { buildChannelConfigSchema } from "amigo/plugin-sdk";
import type {
 ...
 * for Amigo. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: AmigoConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: AmigoConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: AmigoConfig): boolean => {
