import type { AmigoPluginApi, AmigoPluginService } from "amigo/plugin-sdk";
...
function normalizeDenyList(cfg: AmigoPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AmigoPluginApi["config"]): string[] {
...
export default function register(api: AmigoPluginApi) {
  ...
  const timerService: AmigoPluginService = {
