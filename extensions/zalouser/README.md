...
Amigo extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
amigo channels login --channel zalouser
...
```bash
amigo message send --channel zalouser --target <threadId> --message "Hello from Amigo!"
```
...
```bash
amigo channels login --channel zalouser              # Login via QR
amigo channels login --channel zalouser --account work
amigo channels status --probe
amigo channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
amigo directory self --channel zalouser
amigo directory peers list --channel zalouser --query "name"
amigo directory groups list --channel zalouser --query "work"
amigo directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
amigo message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
amigo message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `amigo channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
amigo channels login --channel zalouser --account work
amigo message send --channel zalouser --account work --target <id> --message "Hello"
