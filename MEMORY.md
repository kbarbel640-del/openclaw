...
## Рабочие паттерны
- **PREFLIGHT enforcement** — SOUL.md + PREFLIGHT.md содержат обязательные PRE/POST чеклисты. Следовать строго.
- **Видимая валидация** — в конце сложных ответов `[PRE: ✓] [POST: ✓]` — Влад видит что проверено
- **Checkpoint коммиты** — перед большими изменениями делать коммит для отката (текущий: `0fc085b11`)
...
### Дублирование Telegram сообщений (2026-02-03)
- **Проблема:** Одно сообщение (id:1197 "ну как") доставлено ~40+ раз подряд (08:27-09:49)
- **Вероятная причина:** retry логика или polling баг при восстановлении после сна
- **Статус:** Не исследовано. Нужно проверить дедупликацию по message_id.
