# 🐶 cdog — Personal AI Assistant

<p align="center">
    <picture>
        <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/cdog/cdog/main/docs/assets/cdog-logo-text-dark.png">
        <img src="https://raw.githubusercontent.com/cdog/cdog/main/docs/assets/cdog-logo-text.png" alt="cdog" width="500">
    </picture>
</p>
...
<p align="center">
  <a href="https://github.com/cdog/cdog/actions/workflows/ci.yml?branch=main"><img src="https://img.shields.io/github/actions/workflow/status/cdog/cdog/ci.yml?branch=main&style=for-the-badge" alt="CI status"></a>
  <a href="https://github.com/cdog/cdog/releases"><img src="https://img.shields.io/github/v/release/cdog/cdog?include_prereleases&style=for-the-badge" alt="GitHub release"></a>
  <a href="LICENSE"><img src="https://img.shields.io/badge/License-MIT-blue.svg?style=for-the-badge" alt="MIT License"></a>
</p>

**cdog** is a _personal AI assistant_ you run on your own devices.
It answers you on the channels you already use (WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, Microsoft Teams, WebChat), plus extension channels like BlueBubbles, Matrix, Zalo, and Zalo Personal. It can speak and listen on macOS/iOS/Android, and can render a live Canvas you control. The Gateway is just the control plane — the product is the assistant.

If you want a personal, single-user assistant that feels local, fast, and always-on, this is it.

[Website](https://cdog.ai) · [Docs](https://docs.cdog.ai) · [DeepWiki](https://deepwiki.com/cdog/cdog) · [Getting Started](https://docs.cdog.ai/start/getting-started) · [Updating](https://docs.cdog.ai/install/updating) · [Showcase](https://docs.cdog.ai/start/showcase) · [FAQ](https://docs.cdog.ai/start/faq) · [Wizard](https://docs.cdog.ai/start/wizard) · [Nix](https://github.com/cdog/nix-cdog) · [Docker](https://docs.cdog.ai/install/docker) · [Discord](https://discord.gg/cdog)

Preferred setup: run the onboarding wizard (`cdog onboard`) in your terminal.
The wizard guides you step by step through setting up the gateway, workspace, channels, and skills. The CLI wizard is the recommended path and works on **macOS, Linux, and Windows (via WSL2; strongly recommended)**.
Works with npm, pnpm, or bun.
New install? Start here: [Getting started](https://docs.cdog.ai/start/getting-started)
...
Model note: while any model is supported, I strongly recommend **Anthropic Pro/Max (100/200) + Opus 4.6** for long‑context strength and better prompt‑injection resistance. See [Onboarding](https://docs.cdog.ai/start/onboarding).

## Models (selection + auth)

- Models config + CLI: [Models](https://docs.cdog.ai/concepts/models)
- Auth profile rotation (OAuth vs API keys) + fallbacks: [Model failover](https://docs.cdog.ai/concepts/model-failover)
...
Full beginner guide (auth, pairing, channels): [Getting started](https://docs.cdog.ai/start/getting-started)

```bash
cdog onboard --install-daemon

cdog gateway --port 18789 --verbose

cdog message send --to +1234567890 --message "Hello from CDOG"

cdog agent --message "Ship checklist" --thinking high
```

Upgrading? [Updating guide](https://docs.cdog.ai/install/updating) (and run `cdog doctor`).
...
Switch channels (git + npm): `cdog update --channel stable|beta|dev`.
Details: [Development channels](https://docs.cdog.ai/install/development-channels).
...
```bash
git clone https://github.com/cdog/cdog.git
cd cdog
...
pnpm cdog onboard --install-daemon

# Dev loop (auto-reload on TS changes)
pnpm gateway:watch
```

Note: `pnpm cdog ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `cdog` binary.

## Security defaults (DM access)

OpenClaw connects to real messaging surfaces. Treat inbound DMs as **untrusted input**.

Full security guide: [Security](https://docs.cdog.ai/gateway/security)

Default behavior on Telegram/WhatsApp/Signal/iMessage/Microsoft Teams/Discord/Google Chat/Slack:

- **DM pairing** (`dmPolicy="pairing"` / `channels.discord.dmPolicy="pairing"` / `channels.slack.dmPolicy="pairing"`; legacy: `channels.discord.dm.policy`, `channels.slack.dm.policy`): unknown senders receive a short pairing code and the bot does not process their message.
- Approve with: `cdog pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`).

Run `cdog doctor` to surface risky/misconfigured DM policies.

## Highlights

- **[Local-first Gateway](https://docs.cdog.ai/gateway)** — single control plane for sessions, channels, tools, and events.
- **[Multi-channel inbox](https://docs.cdog.ai/channels)** — WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, BlueBubbles (iMessage), iMessage (legacy), Microsoft Teams, Matrix, Zalo, Zalo Personal, WebChat, macOS, iOS/Android.
- **[Multi-agent routing](https://docs.cdog.ai/gateway/configuration)** — route inbound channels/accounts/peers to isolated agents (workspaces + per-agent sessions).
...
OpenClaw was built for **Molty**, a space dog AI assistant. 🐶
...
Special thanks to Adam Doppelt for his contributions.
